-- ============================================================
-- zenhubXsteal - Merged Build (raw4 + zenhubX + Anti-Cheat)
-- ============================================================
--[[
    zenhubXsteal - Steal an Egg
    Merged: AutoSteal (TP/Speed), ServerHop, ESP, Pets, Treadmill,
            Discord Gatekeeper, Anti-Ragdoll, Fix Lag, Anti Trap,
            Anti-Cheat Metatable Patch
--]]

-- ============================================================
-- ANTI-CHEAT (metatable guard) - chạy trước mọi thứ
-- ============================================================
pcall(function()
...
local genv = _G_ENV
if type(genv.__ZENHUBXSTEAL_SHUTDOWN) == "function" then
    pcall(genv.__ZENHUBXSTEAL_SHUTDOWN); task.wait(0.1)
end
if genv.__ZENHUBXSTEAL_RUNNING then return end
genv.__ZENHUBXSTEAL_RUNNING = true
...
local TOGGLE_ICON  = "rbxassetid://12861923680"
local HEADER_TEXT  = "zenhubXsteal"
...
do
    ...
    Gui.Name = "zenhubX_VerifyGui"
    Gui.ResetOnSpawn = false
    Gui.Parent = LocalPlayer:WaitForChild("PlayerGui", 10) or CoreGui
...
-- Anti-Cheat / Anti-Ragdoll / Anti-Trap state
local PeteSettings = {
    AntiRagdoll = true,
...
if getgenv then
    local history = getgenv().zenhubXstealHopHistory
    if typeof(history) ~= "table" then history = {}; getgenv().zenhubXstealHopHistory = history end
...
EspFolder.Name = "zenhubXstealEggEsp"
...
function H.bypassMoveTo(position, keepGoing, speed)
    ...
    bv.Name = "zenhubXstealBypassMove"; bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
    bv.P = 1250; bv.Velocity = Vector3.zero; bv.Parent = root
    local bg = Instance.new("BodyGyro")
    bg.Name = "zenhubXstealBypassGyro"; bg.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
...
-- ============================================================
-- ANTI-RAGDOLL / ANTI-TRAP / FIX LAG (từ zenhubX)
-- ============================================================
...
H.track(Workspace.DescendantAdded:Connect(function(d)
    if IsTrapImmune then task.wait(); DeleteAndDisableTrap(d) end
end))
...
H.track(RunService.Stepped:Connect(function()
    ...
    if PeteSettings.AntiRagdoll then
...
local function ApplyClayAndClean(obj)
    ...
    if obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Sparkles") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Highlight") then
        obj:Destroy(); return
    end
...
H.track(Workspace.DescendantAdded:Connect(function(obj)
...
-- ============================================================
-- PLACE / HATCH / SELL / FUSE / UPGRADE (rút gọn - giữ nguyên logic)
-- ============================================================
...
function H.runAutoSellPets()
    ...
    for uid, asset in pairs(inventory) do
        ...
        if itemData and itemData.IsFavorite ~= true and itemData.InFuse ~= true then
...
function H.runAutoSellEggs()
    ...
    for uid, egg in pairs(eggInventory) do
        ...
        if typeof(egg) == "table" and egg.Placement == nil then
            if EggApi.RequestEquipTool then pcall(EggApi.RequestEquipTool, uid) end
            task.wait(0.15)
            H.sellUid(uid); task.wait(0.15)
        end
...
function H.runAutoUpgrades()
    ...
    if BaseUpgradeModule and typeof(BaseUpgradeModule.IsNextTierAffordable) == "function" then
    ...
    if TreadmillsData and typeof(TreadmillsData.GetByUpgradeLevel) == "function" then
...
local COLORS = {
    panelBg       = Color3.fromRGB(14, 8, 22),
    panelBg2      = Color3.fromRGB(22, 12, 34),
    panelBorder   = Color3.fromRGB(80, 40, 130),
    panelBorderHi = Color3.fromRGB(170, 80, 255),
    sidebarBg     = Color3.fromRGB(12, 6, 20),
    sectionBg     = Color3.fromRGB(28, 16, 42),
    sectionBorder = Color3.fromRGB(90, 50, 140),
    text          = Color3.fromRGB(245, 235, 255),
    textDim       = Color3.fromRGB(190, 165, 225),
    textMuted     = Color3.fromRGB(130, 105, 165),
    accent        = Color3.fromRGB(160, 32, 240),
    accentHi      = Color3.fromRGB(210, 90, 255),
    ...
    info          = Color3.fromRGB(150, 130, 255),
}
...
-- Toggle button: icon Z (vẽ bằng TextLabel)
...
toggleBtn.TextColor3 = Color3.fromRGB(210, 90, 255)
...
panel.Name = "Panel"
panel.Size = UDim2.fromOffset(640, 440)
...
addStroke(discordBtn, COLORS.accentHi, 1)
discordBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard(DISCORD_LINK) end)
    H.notify(HEADER_TEXT, "Discord link copied", "Success", 3)
end)

local closeBtn = Instance.new("TextButton")
closeBtn.Size = UDim2.fromOffset(28, 28)
closeBtn.Position = UDim2.new(1, -38, 0, 12)
closeBtn.BackgroundColor3 = Color3.fromRGB(60, 15, 90)
...
function H.getState(id, fallback)
    local v = UI.GetState(id); if v ~= nil then return v end
    return fallback
end
function H.isOn(id) return UI.GetState(id) == true end
function H.optionValue(id, fallback)
    local v = UI.GetState(id); if v == nil then return fallback end
    return v
end
...
local function makeTabButton(tabId, tabTitle)
    ...
    local function setActive(active)
        if active then
            TweenService:Create(btn, TweenInfo.new(0.15), { BackgroundColor3 = COLORS.sectionBg, BackgroundTransparency = 0.1 }):Play()
...
function UI.AddSlider(parent, opts)
    ...
    knob.BackgroundColor3 = COLORS.accentHi
    knob.BorderSizePixel = 0
    knob.Parent = bar
    addCorner(knob, 7)
...
function UI.AddStatus(parent, opts)
    ...
    function api.SetStatus(status)
        ...
        else value.TextColor3 = COLORS.accentHi end
    end
    return api
end
...
function H.notify(title, content, variant, duration)
    ...
    addCorner(note, 8)
    local strokeColor = COLORS.accentHi
    ...
    t.Text = tostring(title or HEADER_TEXT)
    t.Parent = note
