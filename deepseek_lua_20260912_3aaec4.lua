...
-- ============================================================
-- EGG / STEAL (giữ nguyên logic file 3)
-- ============================================================
...
-- Toggle button as "Z" avatar (purple)
...
-- Panel size adjusted to fit nicely
...
do
    ...
    -- EXTRAS (tính năng từ 2 file đầu)
    local ExtraTab = UI.AddTab({ Id = "extras", Title = "Extras" })

    local CombatSec = UI.AddSection(ExtraTab, { Title = "Combat / Survival" })
    UI.AddToggle(CombatSec, { Id = "AntiRagdoll", Title = "Anti-Ragdoll / Anti-Knock", Default = true,
        Callback = function(v) PeteSettings.AntiRagdoll = v end })
    UI.AddDropdown(CombatSec, { Id = "AntiRagdollMode", Title = "Camera Mode",
    ...
    UI.AddToggle(CombatSec, { Id = "AntiTrap", Title = "Super Anti-Trap", Default = false,
        Callback = function(v) H.setTrapImmune(v) end })

    local InteractSec = UI.AddSection(ExtraTab, { Title = "Interaction" })
    UI.AddToggle(InteractSec, { Id = "InstantHold", Title = "Instant Interact (No Hold)", Default = false,
        Callback = function(v) H.setInstantHold(v) end })
    UI.AddToggle(InteractSec, { Id = "FreeAutoSteal", Title = "Auto Steal Free (Proximity)", Default = false,
        Callback = function(v) FreeAutoSteal.Enabled = v end })

    local SpeedSec = UI.AddSection(ExtraTab, { Title = "Speed Changer" })
    UI.AddToggle(SpeedSec, { Id = "SpeedChanger", Title = "Enable WalkSpeed Lock", Default = false,
        Callback = function(v) SpeedConfig.Enabled = v end })
    UI.AddSlider(SpeedSec, { Id = "SpeedValue", Title = "WalkSpeed Value", Min = 16, Max = 900, Default = 300, Step = 10,
        Callback = function(v) SpeedConfig.Value = v end })

    local PerfSec = UI.AddSection(ExtraTab, { Title = "Performance" })
    UI.AddButton(PerfSec, { Title = "Fix Lag (Clay + Remove Foliage)", Text = "Apply",
        Callback = function() H.enableFixLag(); H.notify("Fix Lag", "Applied", "Success", 3) end })

    local VisualSec = UI.AddSection(ExtraTab, { Title = "Visual Pets (Client Only)" })
    UI.AddInput(VisualSec, { Id = "VisualPetName", Title = "Pet Name", Placeholder = "Dragon", Default = "" })
    UI.AddSlider(VisualSec, { Id = "VisualPetRadius", Title = "Orbit Radius", Min = 2, Max = 20, Default = 4, Step = 1,
        Callback = function(v) VisualPets.orbitRadius = v end })
    UI.AddSlider(VisualSec, { Id = "VisualPetSpeed", Title = "Orbit Speed", Min = 0, Max = 10, Default = 4, Step = 1,
        Callback = function(v) VisualPets.orbitSpeed = v * 0.2 end })
    UI.AddButton(VisualSec, { Title = "Spawn Visual Pet", Text = "Spawn", Callback = function()
    ...
    UI.AddButton(VisualSec, { Title = "Clear All Visual Pets", Text = "Clear", Callback = function() H.clearVisualPets() end })
    ...
    UI.AddParagraph(AboutSec, { Title = "Script Dev", Content = "zen hubXsteal" })
