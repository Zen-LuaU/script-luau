...
local RAGDOLL_WAIT_TIMEOUT = 8      -- thời gian tối đa chờ quái đánh gây ragdoll
local RAGDOLL_END_TIMEOUT  = 2      -- thời gian tối đa chờ hết ragdoll
...
-- Lấy instance slot trứng theo Uid
...
-- Chọn record trứng bất kỳ trong Forest còn ở Slot/Dropped
...
function H.stealEgg(slotEgg)
    ...
    -- ========================================================
    -- BƯỚC 0: FOREST WARM-UP
    -- 1. Chọn 1 quả trứng ngẫu nhiên trong Forest
    -- 2. Tele tới và nhặt nó (guard sẽ chú ý và đánh chúng ta)
    -- 3. Chờ guard đánh → nhận ragdoll
    -- 4. Chờ hết ragdoll → drop trứng forest → reset state
    -- ========================================================
    do
        ...
        if forestRecord then
            ...
            if forestSlot then
                ...
                if forestPos then
                    ...
                    -- Cố gắng nhặt trứng Forest → guard sẽ phản ứng
                    ...
                    -- Chờ guard đánh → ragdoll (tối đa RAGDOLL_WAIT_TIMEOUT giây)
                    H.waitForRagdoll(RAGDOLL_WAIT_TIMEOUT)

                    -- Chờ hết ragdoll
                    H.waitRagdollEnd(RAGDOLL_END_TIMEOUT)

                    -- Nếu đang mang trứng Forest thì drop (không cần mang về)
