--[[
    ...
    [MOD] Forest FLY-in → wait guard ragdoll → tele target → walk back
    [MOD2] Anti-Ragdoll/Knock không desy, không xung đột với về base
--]]
...
function H.waitForRagdoll(timeout)
    ...
    while running and os.clock() < deadline do
        if H.isRagdoll() then return true end
        RunService.Heartbeat:Wait()
    end
    return false
end

function H.waitRagdollEnd(timeout)
    timeout = tonumber(timeout) or RAGDOLL_END_TIMEOUT
    local deadline = os.clock() + timeout
    while running and os.clock() < deadline do
        if not H.isRagdoll() then return true end
        RunService.Heartbeat:Wait()
    end
    -- Nếu quá hạn, tự ép đứng dậy
    local hum = H.getHumanoid()
    if hum then
        pcall(function()
...
-- ============================================================
-- [MOD2] ANTI-KNOCK KHÔNG DESY — chỉ clamp velocity, không phá ragdoll,
--        KHÔNG can thiệp khi return-to-base (bypassMoveTo)
-- ============================================================
local AntiKnock = {
    Enabled = false,
    MaxHorizontal = 22,
    MaxVertical = 8,
    DriftLimit = 6,     -- khoảng cách tối đa được phép xê dịch so với anchor
    AnchorCF = nil,
    AnchorSetAt = 0,
    ResetAfter = 2.0,
}
...
H.track(RunService.Stepped:Connect(function()
    ...
    -- 1. Bật lại Motor6D nếu bị disable (giữ limbs dính nhau, không bị rụng)
    ...
    -- 2. Xoá các constraint/force gây văng xa khi guard đánh
    ...
    -- 3. Clamp vận tốc để không nẩy xa / cao — KHÔNG gọi ChangeState(GettingUp)
    --    để ragdoll diễn ra tự nhiên cho isRagdoll detect được.
    ...
    -- 4. Anchor clamp: nếu bị văng khỏi vị trí neo quá xa thì kéo về
    if AntiKnock.AnchorCF then
        ...
        if age > AntiKnock.ResetAfter then
            ...
            if horizontalDist > AntiKnock.DriftLimit then
...
-- ============================================================
-- [MOD] stealEgg:
--   B0: FLY tới Forest → nhặt trứng forest → chờ guard đánh → ragdoll
...
function H.stealEgg(slotEgg)
    ...
    -- ========================================================
    -- BƯỚC 0: FOREST WARM-UP (BAY tới — KHÔNG tele)
    -- 1. Bay tới 1 quả trứng ngẫu nhiên trong Forest
    -- 2. Nhặt nó lên → guard chú ý và đánh
    -- 3. Chờ ragdoll (AntiKnock clamp velocity, không desy)
    -- 4. Hết ragdoll → drop trứng forest → reset
    -- ========================================================
    do
        ...
        if forestRecord then
            ...
            if forestSlot then
                ...
                if forestPos then
                    -- AntiKnock OFF khi đang bay (bypassMoveTo cần speed cao)
                    AntiKnock.Enabled = false

                    -- BAY tới trứng Forest (không teleport)
                    H.bypassMoveTo(forestPos, H.stealingEnabled, BYPASS_SPEED)
                    task.wait(0.15)

                    -- Bật lại ragdoll states để guard có thể ragdoll
                    H.setRagdollStatesAllowed(true)

                    -- Bật AntiKnock (chỉ clamp velocity) + neo vị trí tại Forest egg
                    AntiKnock.Enabled = true
                    root = H.getRoot()
                    if root then
                        AntiKnock.SetAnchor(root.Position)
                    end

                    -- Nhặt trứng Forest → guard sẽ phản ứng
                    ...
                    -- Drop trứng Forest nếu đang mang
                    ...
                    -- Tắt AntiKnock + khoá lại ragdoll states cho bước 1-4 an toàn
                    AntiKnock.Enabled = false
    ...
    -- ========================================================
    -- BƯỚC 5: ĐI BỘ về base (AntiKnock đã OFF — không xung đột)
    -- ========================================================
