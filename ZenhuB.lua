...
local genv = _G_ENV
if type(genv.__ZENHUBX_SHUTDOWN) == "function" then pcall(genv.__ZENHUBX_SHUTDOWN); task.wait(0.1) end
...
local function ZenHubXKeyGate()
    ...
    gui.Name = "ZenHubXKeySystem"; gui.ResetOnSpawn = false; gui.IgnoreGuiInset = true
    gui.DisplayOrder = 100000; gui.Parent = parent
    local frame = Instance.new("Frame")
    frame.Size = UDim2.fromOffset(360, 230); frame.Position = UDim2.new(0.5, -180, 0.5, -115)
    frame.BackgroundColor3 = Color3.fromRGB(14, 10, 20); frame.BorderSizePixel = 0; frame.Parent = gui
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 12)
    local stroke = Instance.new("UIStroke", frame); stroke.Color = Color3.fromRGB(170, 70, 255); stroke.Thickness = 2
    local title = Instance.new("TextLabel")
    title.BackgroundTransparency = 1; title.Position = UDim2.fromOffset(18, 15); title.Size = UDim2.new(1, -36, 0, 28)
    title.Font = Enum.Font.GothamBold; title.TextSize = 20; title.TextColor3 = Color3.fromRGB(220, 150, 255)
    title.Text = "zen hubX • KEY SYSTEM"; title.Parent = frame
    local desc = Instance.new("TextLabel")
    desc.BackgroundTransparency = 1; desc.Position = UDim2.fromOffset(18, 48); desc.Size = UDim2.new(1, -36, 0, 45)
    desc.Font = Enum.Font.Gotham; desc.TextSize = 11; desc.TextWrapped = true; desc.TextColor3 = Color3.fromRGB(195, 180, 210)
    desc.Text = "Tham gia nhóm Discord để nhận key.\nSau đó nhập key vào ô bên dưới để xác minh."; desc.Parent = frame
    local discord = Instance.new("TextButton")
    discord.Size = UDim2.new(1, -36, 0, 32); discord.Position = UDim2.fromOffset(18, 96)
    discord.BackgroundColor3 = Color3.fromRGB(85, 35, 125); discord.BorderSizePixel = 0
    discord.Font = Enum.Font.GothamBold; discord.TextSize = 12; discord.TextColor3 = Color3.new(1, 1, 1)
    discord.Text = "Tham gia Discord • Sao chép link"; discord.Parent = frame
    Instance.new("UICorner", discord).CornerRadius = UDim.new(0, 7)
    discord.MouseButton1Click:Connect(function()
        pcall(function() setclipboard(DISCORD_LINK) end)
        discord.Text = "Đã sao chép link Discord"
        task.delay(1.5, function() if discord.Parent then discord.Text = "Tham gia Discord • Sao chép link" end end)
    end)
    local box = Instance.new("TextBox")
    box.Size = UDim2.new(1, -36, 0, 32); box.Position = UDim2.fromOffset(18, 137)
    box.BackgroundColor3 = Color3.fromRGB(28, 20, 38); box.BorderSizePixel = 0; box.Font = Enum.Font.Gotham
    box.TextSize = 12; box.TextColor3 = Color3.new(1, 1, 1); box.PlaceholderColor3 = Color3.fromRGB(130, 110, 145)
    box.PlaceholderText = "Nhập key..."; box.ClearTextOnFocus = false; box.Parent = frame
    Instance.new("UICorner", box).CornerRadius = UDim.new(0, 7)
    local bp = Instance.new("UIPadding", box); bp.PaddingLeft = UDim.new(0, 10)
    local verify = Instance.new("TextButton")
    verify.Size = UDim2.new(1, -36, 0, 32); verify.Position = UDim2.fromOffset(18, 178)
    verify.BackgroundColor3 = Color3.fromRGB(145, 55, 220); verify.BorderSizePixel = 0
    verify.Font = Enum.Font.GothamBold; verify.TextSize = 12; verify.TextColor3 = Color3.new(1, 1, 1)
    verify.Text = "Xác minh key"; verify.Parent = frame
    Instance.new("UICorner", verify).CornerRadius = UDim.new(0, 7)
    local verified = false
    verify.MouseButton1Click:Connect(function()
        if box.Text == ZENHUBX_KEY then
            verified = true; verify.Text = "✓ Key hợp lệ"; verify.BackgroundColor3 = Color3.fromRGB(65, 160, 105)
        else
            verify.Text = "✗ Key không đúng"; verify.BackgroundColor3 = Color3.fromRGB(180, 55, 80)
            task.delay(1.2, function()
                if verify.Parent then verify.Text = "Xác minh key"; verify.BackgroundColor3 = Color3.fromRGB(145, 55, 220) end
            end)
        end
    end)
...
-- ============================================================
local H = {}
function H.cloneList(src) local out = {}; if type(src) ~= "table" then return out end
    for i = 1, #src do out[i] = src[i] end return out end
function H.clearTable(tbl) if type(tbl) ~= "table" then return end
    for k in pairs(tbl) do tbl[k] = nil end end
local running = true
function H.waitFor(timeout, interval, check)
    local deadline = os.clock() + (tonumber(timeout) or 1); local gap = tonumber(interval) or 0.05
    local passed = false
    repeat if check and check() == true then passed = true
...
function H.findModule(name)
    for _, d in ipairs(ReplicatedStorage:GetDescendants()) do
        if d:IsA("ModuleScript") and d.Name == name then
            local ok, mod = pcall(require, d); if ok then return mod end
        end
    end
    return nil
end
...
function H.findRemoteContains(...)
    ...
    for _, d in ipairs(ReplicatedStorage:GetDescendants()) do
        if d:IsA("RemoteEvent") or d:IsA("RemoteFunction") then
            local ok = true
            for _, part in ipairs(parts) do if not string.find(d.Name, part, 1, true) then ok = false; break end end
...
function H.pickFromTable(tbl, ...)
    ...
    local names = { ... }; local cur = tbl
    for _, name in ipairs(names) do if typeof(cur) ~= "table" then return nil end cur = cur[name] end
    return cur
end
function H.pickFn(mod, ...)
    if typeof(mod) ~= "table" then return nil end
    for i = 1, select("#", ...) do
        local key = select(i, ...); local fn = mod[key]
...
local EspFolder = Instance.new("Folder"); EspFolder.Name = "ZenHubXEggEsp"; EspFolder.Parent = Workspace
...
function H.getSave()
    ...
    local ok, save = pcall(SaveModule.Get); return ok and save or nil
end
function H.netInvoke(remote, ...)
    if typeof(remote) ~= "Instance" then return nil end
    local args = table.pack(...); local result, done = nil, false
    task.spawn(function()
...
function H.countTable(value) if typeof(value) ~= "table" then return 0 end
    local count = 0; for _ in pairs(value) do count = count + 1 end return count end
function H.formatNumber(value)
    local number = tonumber(value) or 0
    local suffixes = { "", "K", "M", "B", "T", "Qa", "Qi" }; local suffixIndex = 1
...
function H.formatElapsed(seconds)
    ...
    local hours = math.floor(total / 3600); local minutes = math.floor((total % 3600) / 60)
...
function H.resolveRarity(assetCategory)
    ...
    local rarity = entry and entry.Rarity; if not rarity then return nil end
...
function H.recordMutations(asset)
    local mutations = {}; if typeof(asset) ~= "table" then return mutations end
...
function H.getZoneLaneCenter(zoneName)
    local zone = H.getZoneModel(zoneName); if not zone then return nil end
...
function H.placeRoot(root, cf)
    ...
    else safePivot(root, cf) end
end
function H.groundedY(x, z, fallbackY)
    local laneY = H.getLaneY(); local root = H.getRoot(); local humanoid = H.getHumanoid()
    local hipHeight = 2; if humanoid and humanoid.HipHeight > 0 then hipHeight = humanoid.HipHeight end
...
function H.prepareStealHumanoid()
    ...
    local camera = Workspace.CurrentCamera; local cameraCFrame = camera and camera.CFrame or nil
    local newHumanoid = nil
    local ok, clone = pcall(function() humanoid.Archivable = true; return humanoid:Clone() end)
    if ok and clone then
        newHumanoid = clone; newHumanoid.Parent = character
    ...
    if newHumanoid then
        newHumanoid.Sit = false; newHumanoid.PlatformStand = false
        newHumanoid.WalkSpeed = H.stealSpeed(); newHumanoid.AutoRotate = true
    end
...
function H.buildStealPath(startPosition, targetPosition)
    local laneZ, laneY = H.getLaneZ(), H.getLaneY(); local waypoints = {}
...
function H.humanoidStealMoveTo(position, keepGoing)
    ...
    humanoid.AutoRotate = true; humanoid.WalkSpeed = H.stealSpeed()
    ...
    for _, point in ipairs(route) do
        ...
        while running and os.clock() < deadline do
            ...
            root = H.getRoot(); if not root then return false end
            local offset = waypoint - root.Position; local distance = offset.Magnitude
...
function H.getBasePosition()
    if PlotApi.GetRespawnPointCFrame then
        local respawn = PlotApi.GetRespawnPointCFrame(); if respawn then return respawn.Position end
    end
    if not PlotApi.GetPlotData then return nil end
    local plot = PlotApi.GetPlotData(); if not plot then return nil end
...
function H.bypassMoveTo(position, keepGoing, speed)
    ...
    while running and os.clock() < deadline do
        ...
        local offset = dest - root.Position; local dist = offset.Magnitude
...
function H.holdAtPosition(seconds, keepGoing)
    ...
    local root = H.getRoot(); if not root then return false end
    local anchorCF = root.CFrame; local deadline = os.clock() + seconds; local frameCounter = 0
    while running and os.clock() < deadline do
        if keepGoing and not keepGoing() then return false end
        root = H.getRoot()
        if root then
...
function H.returnToBaseBypass(keepGoing)
    local basePosition = H.getBasePosition(); if not basePosition then return false end
    if keepGoing and not keepGoing() then return false end
...
function H.ensureAtPlot(keepGoing)
    ...
    local standPosition = H.getPetAreaStandPosition(); if not standPosition then return false end
...
-- ============================================================
local ExtrasState = {
    AntiRagdoll = false, InstantInteract = false, FixLag = false, AntiTrap = false,
    LookRocket = false, SpeedBypass = false, SpeedValue = 300, PromptAutoSteal = false,
}
...
H.track(RunService.RenderStepped:Connect(function()
    ...
    local ch = LocalPlayer.Character; local hrp = H.getRoot(); local hum = H.getHumanoid()
    ...
    if ExtrasState.LookRocket then
        ...
        if horizontal.Magnitude > 15 and oldHorizontal.Magnitude < 10 and os.clock() - lastRocket > 0.08 then
            ...
            if cam then
                ...
                hrp.AssemblyLinearVelocity = lookDir * (horizontal.Magnitude * THROW_MULTIPLIER)
                lastRocket = os.clock()
            end
        end
    end
...
H.track(Workspace.DescendantAdded:Connect(function(d)
    if ExtrasState.InstantInteract and d:IsA("ProximityPrompt") then task.defer(applyInstantHold, d) end
end))
function ExtrasState.SetInstantHold(on)
    ExtrasState.InstantInteract = on
    if on then for _, p in ipairs(Workspace:GetDescendants()) do
...
local function isFoliage(obj) local n = string.lower(obj.Name)
    for _, w in ipairs(FOLIAGE_WORDS) do if string.find(n, w, 1, true) then return true end end return false end
...
function ExtrasState.EnableFixLag()
    ...
    pcall(function()
        ...
        for _, v in ipairs(Lighting:GetChildren()) do
            ...
            or v:IsA("SunRaysEffect") or v:IsA("BloomEffect") or v:IsA("Atmosphere") then v:Destroy() end
        end
    end)
...
local function isTrap(obj) local n = string.lower(obj.Name)
    for _, w in ipairs(TRAP_WORDS) do if string.find(n, w, 1, true) then return true end end return false end
...
function ExtrasState.EnableAntiTrap()
    ...
    if ch and not ch:FindFirstChild("ZenHubXTrapField") then
        local ff = Instance.new("ForceField"); ff.Name = "ZenHubXTrapField"; ff.Visible = false; ff.Parent = ch
    end
...
function ExtrasState.ApplySpeedBypass(targetSpeed)
    ...
    local oldHum = ch:FindFirstChildOfClass("Humanoid"); if not oldHum then return false end
    local cam = Workspace.CurrentCamera; local camCF = cam and cam.CFrame or nil
    local clone = oldHum:Clone(); clone.Parent = ch; oldHum:Destroy()
    task.wait(0.15)
    local newHum = ch:FindFirstChildOfClass("Humanoid")
    if newHum then
        if cam and camCF then pcall(function() cam.CameraSubject = newHum; cam.CFrame = camCF end) end
...
H.track(RunService.Stepped:Connect(function()
    ...
    if hum and hum.WalkSpeed ~= ExtrasState.SpeedValue then hum.WalkSpeed = ExtrasState.SpeedValue end
end))
function ExtrasState.VoidReset()
    local hrp = H.getRoot(); if not hrp then return false end
    pcall(function() hrp.CFrame = CFrame.new(hrp.Position.X, -350, hrp.Position.Z) end)
    return true
end
...
H.track(Workspace.DescendantRemoving:Connect(function(d)
    if d:IsA("ProximityPrompt") then
        local idx = table.find(PromptCache, d); if idx then table.remove(PromptCache, idx) end
    end
end))
local function FindNearestStealOptimized()
    local hrp = H.getRoot(); if not hrp or not hrp.Parent then return nil end
    ...
    for i = #PromptCache, 1, -1 do
        ...
        else if not obj or not obj.Parent then table.remove(PromptCache, i) end end
    end
    return closest
end
...
H.track(RunService.Heartbeat:Connect(function()
    ...
    local prompt = FindNearestStealOptimized(); if prompt then ActivatePrompt(prompt) end
end))
task.spawn(function()
    RefreshPromptCache()
    while running do
...
function H.getSlotEggPosition(eggInstance)
    local part = eggInstance:FindFirstChild("Hitbox") or eggInstance:FindFirstChild("CustomBoundingBox")
...
function H.isBigEgg(egg)
    ...
    local scale = tonumber(egg.AssetScale); if not scale then return false end
...
function H.pickStealTarget()
    ...
    local root = H.getRoot(); local priority = H.optionValue("StealPriority", "Rarest")
...
function H.eggInventoryCount()
    local save = H.getSave(); local inventory = save and save.EggInventory
...
function H.tryCarryEgg(eggInstance)
    ...
    local uid = eggInstance.Name; local slotKey = nil
    if SlotIdentityApi.IsFirstAreaUid and SlotIdentityApi.IsFirstAreaUid(uid) then
        for _, record in ipairs(H.getAreaEggs()) do
            if record.Uid == uid and SlotIdentityApi.BuildSlotKey then
                slotKey = SlotIdentityApi.BuildSlotKey(record.AreaId, record.NestId); break
            end
        end
    end
...
function H.stealEgg(slotEgg)
    ...
    while running and H.stealingEnabled() and not IsCarryingEgg and os.clock() < carryDeadline do
        ...
        H.tryCarryEgg(slotEgg); if IsCarryingEgg then break end; task.wait(0.05)
    end
...
function H.runAutoSteal()
    ...
    local target = H.pickStealTarget(); if not target then return false end
...
function H.getUnplacedEggUids()
    local save = H.getSave(); local eggInventory = save and save.EggInventory; local result = {}
...
function H.getPlacementLocalCFrames()
    ...
    local size = petArea.Size; local placements = {}
...
function H.runAutoPlaceEggs(forceRun)
    ...
    local placements = H.getPlacementLocalCFrames(); if #placements == 0 then return end
...
function H.runAutoOpenReadyEggs()
    local save = H.getSave(); local eggInventory = save and save.EggInventory
...
function H.holdUid(uid)
    local character = LocalPlayer.Character; local humanoid = H.getHumanoid()
    if not character or not humanoid then return false end
    local tool = H.findToolByUid(uid); if not tool then return false end
...
function H.sellUid(uid)
    ...
    return H.waitFor(2, 0.1, function()
        ...
        local inventory = save.Inventory or {}; local eggInventory = save.EggInventory or {}
        return inventory[uid] == nil and eggInventory[uid] == nil
    end)
end

-- Pet Sell System (Whitelist/Blacklist)
local PetSellConfig = {
    AutoSellPet = false, SellPetWhitelist = {}, SellPetBlacklist = {},
    UseWhitelist = true, SellPetInterval = 15, KeepBestPet = true,
}
local PetSellCache = { petList = {}, petListAt = 0 }
function H.getPetListForSell(force)
    if not force and os.clock() - PetSellCache.petListAt < 2 then return PetSellCache.petList end
    local save = H.getSave(); local inventory = save and save.Inventory; local list = {}
    if typeof(inventory) == "table" then
        for uid, asset in pairs(inventory) do
            if typeof(uid) == "string" and typeof(asset) == "table" then
                local category = asset.Category
                list[#list + 1] = {
                    uid = uid, name = H.assetName(category),
                    rarity = H.resolveRarity(category) or "Common",
                    scale = tonumber(asset.Scale) or 0, category = category,
                    mutations = H.recordMutations(asset),
                }
            end
        end
    end
    PetSellCache.petList = list; PetSellCache.petListAt = os.clock()
    return list
end
    ...
    if PetSellConfig.UseWhitelist then
        ...
        for k, v in pairs(PetSellConfig.SellPetWhitelist) do
            if v == true then
                ...
                if name:find(key, 1, true) or rarity:find(key, 1, true) then return true end
            end
        end
        return false
    else
        for k, v in pairs(PetSellConfig.SellPetBlacklist) do
            if v == true then
                local key = tostring(k):lower()
                if name:find(key, 1, true) or rarity:find(key, 1, true) then return false end
            end
        end
        return true
    end
...
function H.findBestPetPerType(list)
    ...
    for _, pet in ipairs(list) do
        ...
        if not best[key] or (pet.scale or 0) > (best[key].scale or 0) then best[key] = pet end
    end
    local keep = {}
...
function H.runAutoSellPetsAdvanced()
    ...
    for _, pet in ipairs(list) do
        ...
        if keepBest[pet.uid] then skipped = skipped + 1
        elseif H.isPetInSellList(pet) then
            H.sellUid(pet.uid); sold = sold + 1; task.wait(0.08)
...
function H.fuseGroups(save)
    local inventory = save and save.Inventory; local groups = {}
    if typeof(inventory) ~= "table" then return groups end
    local equipped = save.EquippedAssets or {}
    local keepEquipped = H.isOn("FuseKeepEquipped"); local keepMutated = H.isOn("FuseKeepMutated")
    local maxScale = tonumber(H.optionValue("FuseMaxScale", 10)) or 10
    local filterMutations = H.multiHasAny("FuseMutations"); local selectedMutations = H.multiSelected("FuseMutations")
    for uid, pet in pairs(inventory) do
        if typeof(uid) == "string" and typeof(pet) == "table" then
            local category = pet.Category; local selectable = false
            ...
            if selectable and not (keepEquipped and table.find(equipped, uid) ~= nil) then
                ...
                local rarity = H.resolveRarity(category); local scale = tonumber(pet.Scale) or 0
...
function H.fusePrice(save, petUids)
    ...
    for index, uid in ipairs(petUids) do
        local pet = inventory[uid]; local decoded = pet and H.getPetItemData(pet)
...
function H.runAutoFusePets(forceRun)
    ...
    local petUids = H.pickFuseGroup(save); if not petUids then return end
    ...
    for _, uid in ipairs(petUids) do
        ...
        H.netInvoke(Remotes.FuseMachine.INSERT_MOB, uid); task.wait(0.2)
    end
...
function H.runAutoEquipBestTrail()
    local save = H.getSave(); local ownedTrails = save and save.TrailInventory
...
function H.runAutoEquipBestGear()
    local character = LocalPlayer.Character; local backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
    local humanoid = H.getHumanoid()
...
function H.runAutoBuyTrail()
    ...
    local wanted = H.multiSelected("TrailWanted"); local owned = save.TrailInventory or {}
        ...
        if wanted[trailName] then
            ...
            if trailId and not owned[trailId] then
                ...
                if save.Money >= price then
                    ...
                    save = H.getSave() or save; owned = save.TrailInventory or owned
                end
            end
        end
    end
...
function H.runAutoUpgrades()
    ...
    if selected.Base and BaseUpgradeModule and typeof(BaseUpgradeModule.IsNextTierAffordable) == "function" then
        if BaseUpgradeModule.IsNextTierAffordable(save) then
            H.netCall(Remotes.Plots.REQUEST_BASE_UPGRADE); upgraded = true; task.wait(0.35)
        end
    end
    if selected.Treadmill and TreadmillsData and typeof(TreadmillsData.GetByUpgradeLevel) == "function" then
        ...
        if nextUpgrade then
            ...
            if save.Money >= price then
                H.netCall(Remotes.Treadmills.REQUEST_UPGRADE, nextUpgrade._id); upgraded = true; task.wait(0.35)
            end
        end
    end
    return upgraded
...
function H.runAutoClaimGroupReward()
    ...
    H.netInvoke(Remotes.GroupReward.CLAIM_REWARD, isInGroup); return true
end
...
function H.getTreadmillStand()
    if not PlotApi.GetPlotData then return nil end
    local plot = PlotApi.GetPlotData(); local plotFolder = plot and plot.PlotFolder
...
function H.dismountTreadmill()
    pcall(function()
        ...
        input:SendKeyEvent(true, Enum.KeyCode.Space, false, game); task.wait(0.05)
...
function H.runAutoTreadmillTraining()
    local treadmillPosition = H.getTreadmillStand(); if not treadmillPosition then return end
...
function H.ensureEspEntry(id, color)
    local existing = EspEntries[id]; if existing then return existing end
    ...
    EspEntries[id] = entry; return entry
end
...
function H.collectEggEsp()
    local showWorldEggs = H.isOn("EspWorldEggs"); local showCarriedEggs = H.isOn("EspCarriedEggs")
...
function H.collectPetEsp()
    ...
    local renderedAssets = Workspace:FindFirstChild("ClientRenderedAssets"); if not renderedAssets then return end
    local save = H.getSave(); local inventory = save and save.Inventory or {}
    local runtimeByUid = {}
    pcall(function()
...
function H.collectMachineEsp()
    ...
    local objects = Workspace:FindFirstChild("__OBJECTS"); local machines = objects and objects:FindFirstChild("Machines")
...
function H.collectPlotEsp()
    ...
    for _, plot in ipairs(plots:GetChildren()) do
        ...
        if anchor and anchor:IsA("BasePart") and H.withinEspRange(anchor.Position) then
            ...
            local ownerText = "Empty"; local numericUserId = tonumber(ownerUserId)
...
function H.pickHopTargets()
    local cursor = nil; local candidates = {}
    for _ = 1, 4 do
        local page = H.fetchServerPage(cursor); if not page then break end
...
function H.tryTeleportTo(serverId)
    LastTeleportFailure = nil; task.wait(1)
...
function H.serverHop(reason)
    ...
    for attempt = 1, 3 do
        ...
        for index = 1, math.min(#targets, 10) do
            ...
            if H.tryTeleportTo(target.id) then ServerHopInProgress = false; return true end
            task.wait(0.5)
        end
    end
    ServerHopRetryAfter = os.clock() + 10; ServerHopInProgress = false; return false
...
function H.runServerHop()
    ...
    elseif now - NoMatchingEggsSince >= threshold then
        NoMatchingEggsSince = 0; H.serverHop("No matching eggs in this server")
    end
end
...
function H.hopLowPop()
    task.spawn(function()
        ...
        if not page then H.notify("Hop", "Không lấy được danh sách server", "Error", 3); return end
        local candidates = {}
        for _, s in ipairs(page.data or {}) do
            if typeof(s) == "table" and typeof(s.id) == "string" and s.id ~= game.JobId then
                ...
                if playing > 0 and playing <= 5 and playing < maxP then table.insert(candidates, s.id) end
            end
        end
        if #candidates == 0 then H.notify("Hop", "Không tìm thấy server ít người", "Warning", 3); return end
...
function H.openServerHopGUI()
    ...
    local old = pg:FindFirstChild("ZenHubX_ServerHopGUI"); if old then old:Destroy() end
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "ZenHubX_ServerHopGUI"; ScreenGui.ResetOnSpawn = false; ScreenGui.Parent = pg
    local MainFrame = Instance.new("Frame")
    MainFrame.Parent = ScreenGui; MainFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 30)
    MainFrame.BorderSizePixel = 0; MainFrame.Position = UDim2.new(0.5, -145, 0.5, -110)
    MainFrame.Size = UDim2.new(0, 290, 0, 220); MainFrame.Active = true; MainFrame.Draggable = true
    Instance.new("UICorner", MainFrame).CornerRadius = UDim.new(0, 10)
    local Stroke = Instance.new("UIStroke", MainFrame); Stroke.Color = Color3.fromHex("#8A2BE2"); Stroke.Thickness = 2
    local Title = Instance.new("TextLabel", MainFrame)
    Title.Size = UDim2.new(1, 0, 0, 36); Title.BackgroundColor3 = Color3.fromRGB(30, 25, 45)
    Title.BorderSizePixel = 0; Title.Font = Enum.Font.GothamBold; Title.Text = "ZEN HUB X - HOP SERVER"
    Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.TextSize = 13
    Instance.new("UICorner", Title).CornerRadius = UDim.new(0, 10)
    local CloseBtn = Instance.new("TextButton", MainFrame)
    CloseBtn.BackgroundTransparency = 1; CloseBtn.Position = UDim2.new(1, -28, 0, 6)
    CloseBtn.Size = UDim2.new(0, 22, 0, 22); CloseBtn.Font = Enum.Font.GothamBold
    ...
    NowLabel.TextXAlignment = Enum.TextXAlignment.Left; NowLabel.TextColor3 = Color3.fromRGB(100, 220, 100); NowLabel.TextSize = 12
    ...
    StatusLabel.TextXAlignment = Enum.TextXAlignment.Left; StatusLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
    StatusLabel.TextSize = 11; StatusLabel.TextWrapped = true
    ...
    MaxLabel.TextXAlignment = Enum.TextXAlignment.Left; MaxLabel.TextColor3 = Color3.fromRGB(160, 160, 160)
    MaxLabel.TextSize = 11; MaxLabel.Text = "Hop nếu số player LỚN HƠN hoặc BẰNG:"
    local MaxBox = Instance.new("TextBox", MainFrame)
    MaxBox.Position = UDim2.new(0, 10, 0, 126); MaxBox.Size = UDim2.new(1, -20, 0, 28)
    MaxBox.BackgroundColor3 = Color3.fromRGB(35, 30, 50); MaxBox.BorderSizePixel = 0
    MaxBox.Font = Enum.Font.GothamBold; MaxBox.Text = "5"; MaxBox.TextColor3 = Color3.fromRGB(255, 255, 255)
    MaxBox.TextSize = 14; MaxBox.ClearTextOnFocus = false
    ...
    HopBtn.BackgroundColor3 = Color3.fromHex("#8A2BE2"); HopBtn.BorderSizePixel = 0
    HopBtn.Font = Enum.Font.GothamBold; HopBtn.Text = "Hop 1 Lần"; HopBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    HopBtn.TextSize = 13; Instance.new("UICorner", HopBtn).CornerRadius = UDim.new(0, 6)
    local AutoBtn = Instance.new("TextButton", MainFrame)
    AutoBtn.Position = UDim2.new(0, 145, 0, 162); AutoBtn.Size = UDim2.new(0, 135, 0, 45)
    AutoBtn.BackgroundColor3 = Color3.fromRGB(40, 120, 70); AutoBtn.BorderSizePixel = 0
    AutoBtn.Font = Enum.Font.GothamBold; AutoBtn.Text = "Auto Hop: OFF"; AutoBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    AutoBtn.TextSize = 13; Instance.new("UICorner", AutoBtn).CornerRadius = UDim.new(0, 6)
    local autoEnabled = false; local autoThread = nil
    task.spawn(function()
        while ScreenGui.Parent do
            NowLabel.Text = "Player ở server hiện tại: " .. #Players:GetPlayers(); task.wait(1)
        end
    end)
    local function getRandomServer()
        ...
        else pcall(function() raw = game:HttpGet(url) end) end
        if not raw or raw == "" then return nil end
        local ok2, data = pcall(function() return HttpService:JSONDecode(raw) end)
        if not ok2 or not data or not data.data or #data.data == 0 then return nil end
        local targetMax = tonumber(MaxBox.Text) or 5; local candidates = {}
    ...
    local function hopOnce()
        ...
        if currentCount < threshold then
            ...
            StatusLabel.TextColor3 = Color3.fromRGB(100, 220, 100); return false
        end
        StatusLabel.Text = "Server đang có " .. currentCount .. " player. Đang tìm server nhỏ hơn..."
        StatusLabel.TextColor3 = Color3.fromRGB(255, 200, 50); task.wait(0.5)
        local serverId = getRandomServer()
        if not serverId then
            StatusLabel.Text = "Không lấy được danh sách Server! Vui lòng thử lại..."
            StatusLabel.TextColor3 = Color3.fromRGB(255, 80, 80); return false
        end
        StatusLabel.Text = "Đang kết nối sang Server mới..."; StatusLabel.TextColor3 = Color3.fromRGB(120, 180, 255)
        task.wait(0.5)
    ...
    AutoBtn.MouseButton1Click:Connect(function()
        ...
        if autoEnabled then
            AutoBtn.Text = "Auto Hop: ON"; AutoBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
            autoThread = task.spawn(function()
                while autoEnabled and ScreenGui.Parent do
                    ...
                    if count < threshold then
                        ...
                        StatusLabel.TextColor3 = Color3.fromRGB(100, 220, 100); task.wait(3)
                    else hopOnce(); task.wait(5) end
                end
            end)
        else
            AutoBtn.Text = "Auto Hop: OFF"; AutoBtn.BackgroundColor3 = Color3.fromRGB(40, 120, 70)
            if autoThread then task.cancel(autoThread); autoThread = nil end
            StatusLabel.Text = "Đã dừng Auto Hop."; StatusLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
        end
    end)
    StatusLabel.Text = "Hãy nhập số người tối đa & ấn Hop."; StatusLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
end
...
function H.httpPost(payload)
    ...
    local encodedBody; local encoded = pcall(function() encodedBody = HttpService:JSONEncode(payload) end)
...
function H.trackWebhookEvents()
    ...
    for uid in pairs(save.Inventory or {}) do
        if KnownInventoryUids[uid] == nil then
            KnownInventoryUids[uid] = true; SummaryPetsObtained = SummaryPetsObtained + 1
        end
    end
    ...
    local currentWorldEggs = {}; local logSpawns = H.isOn("WebhookEggSpawns")
    for _, egg in ipairs(H.getAreaEggs()) do
        ...
        if KnownWorldEggUids[egg.Uid] == nil then
            ...
            if logSpawns and H.selectionAllows("WebhookRarities", rarity or "") and #SpawnedEggLog < 60 then
                table.insert(SpawnedEggLog, {
                    rank = RarityWeight[rarity or ""] or 0, order = #SpawnedEggLog,
...
function H.buildSummaryEmbed()
    ...
    return {
        ...
        color = 10033663, fields = fields, footer = { text = "ZenHubX | " .. DISCORD_LINK },
...
function H.enableFpsBoost()
    ...
    local terrain = Workspace:FindFirstChildOfClass("Terrain"); local qualityLevel = nil
...
-- ============================================================
local UI = {}
UI.__state = {}; UI.__callbacks = {}; UI.__tabs = {}; UI.__activeTab = nil; UI.__connections = {}
...
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "ZenHubXGUI"; screenGui.ResetOnSpawn = false; screenGui.IgnoreGuiInset = true
screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; screenGui.DisplayOrder = 9999
screenGui.Parent = getGuiParent()

local toggleGui = Instance.new("ScreenGui")
toggleGui.Name = "ZenHubXToggle"; toggleGui.ResetOnSpawn = false; toggleGui.IgnoreGuiInset = true
toggleGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; toggleGui.DisplayOrder = 9998
toggleGui.Parent = getGuiParent()

local toggleBtn = Instance.new("ImageButton")
toggleBtn.Name = "ToggleBtn"; toggleBtn.Size = UDim2.fromOffset(52, 52)
toggleBtn.Position = UDim2.new(0, 20, 0.4, 0); toggleBtn.BackgroundColor3 = Color3.fromRGB(120, 40, 190)
toggleBtn.BackgroundTransparency = 0.05; toggleBtn.BorderSizePixel = 0
toggleBtn.Image = "rbxassetid://72639013832251"; toggleBtn.ImageColor3 = Color3.fromRGB(255, 255, 255)
toggleBtn.ScaleType = Enum.ScaleType.Crop; toggleBtn.AutoButtonColor = false
toggleBtn.Active = true; toggleBtn.Draggable = false; toggleBtn.Parent = toggleGui
Instance.new("UICorner", toggleBtn).CornerRadius = UDim.new(1, 0)
local toggleStroke = Instance.new("UIStroke", toggleBtn)
toggleStroke.Thickness = 2; toggleStroke.Color = Color3.fromRGB(210, 160, 255); toggleStroke.Transparency = 0.15

local PANEL_W, PANEL_H = 480, 340
local panel = Instance.new("Frame")
panel.Name = "Panel"; panel.Size = UDim2.fromOffset(PANEL_W, PANEL_H)
panel.Position = UDim2.new(0.5, -PANEL_W / 2, 0.5, -PANEL_H / 2)
panel.BackgroundColor3 = COLORS.panelBg; panel.BackgroundTransparency = 0.08
panel.BorderSizePixel = 0; panel.ClipsDescendants = true; panel.Active = true
panel.Draggable = false; panel.Visible = false; panel.Parent = screenGui
Instance.new("UICorner", panel).CornerRadius = UDim.new(0, 12)
local panelStroke = Instance.new("UIStroke", panel)
panelStroke.Thickness = 1.5; panelStroke.Color = COLORS.panelBorder; panelStroke.Transparency = 0.15
...
header.Name = "Header"; header.Size = UDim2.new(1, 0, 0, 44)
header.BackgroundColor3 = COLORS.panelBg2; header.BackgroundTransparency = 0.05
...
headerLine.BackgroundColor3 = COLORS.panelBorder; headerLine.BorderSizePixel = 0; headerLine.Parent = header
local title = Instance.new("TextLabel")
title.BackgroundTransparency = 1; title.Position = UDim2.fromOffset(16, 5)
title.Size = UDim2.new(0, 240, 0, 20); title.Font = Enum.Font.GothamBold; title.TextSize = 16
title.TextColor3 = COLORS.text; title.TextXAlignment = Enum.TextXAlignment.Left
title.Text = HEADER_TEXT; title.Parent = header
local subtitle = Instance.new("TextLabel")
subtitle.BackgroundTransparency = 1; subtitle.Position = UDim2.fromOffset(16, 24)
subtitle.Size = UDim2.new(0, 320, 0, 14); subtitle.Font = Enum.Font.Gotham; subtitle.TextSize = 10
subtitle.TextColor3 = COLORS.textDim; subtitle.TextXAlignment = Enum.TextXAlignment.Left
subtitle.Text = SUBTITLE; subtitle.Parent = header

local discordBtn = Instance.new("TextButton")
discordBtn.Size = UDim2.fromOffset(84, 22); discordBtn.Position = UDim2.new(1, -152, 0, 11)
discordBtn.BackgroundColor3 = Color3.fromRGB(72, 48, 105); discordBtn.BackgroundTransparency = 0.15
discordBtn.BorderSizePixel = 0; discordBtn.AutoButtonColor = false
discordBtn.Font = Enum.Font.GothamSemibold; discordBtn.TextSize = 11; discordBtn.TextColor3 = COLORS.text
discordBtn.Text = "Discord"; discordBtn.Parent = header
addCorner(discordBtn, 5); addStroke(discordBtn, Color3.fromRGB(110, 80, 155), 1)
...
discordBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard(DISCORD_LINK) end)
    H.notify("ZenHubX", "Đã sao chép link Discord", "Success", 3)
end)

local closeBtn = Instance.new("TextButton")
closeBtn.Size = UDim2.fromOffset(24, 24); closeBtn.Position = UDim2.new(1, -32, 0, 10)
closeBtn.BackgroundColor3 = Color3.fromRGB(52, 26, 72); closeBtn.BackgroundTransparency = 0.2
closeBtn.BorderSizePixel = 0; closeBtn.AutoButtonColor = false; closeBtn.Font = Enum.Font.GothamBold
closeBtn.TextSize = 13; closeBtn.TextColor3 = COLORS.text; closeBtn.Text = "X"; closeBtn.Parent = header
addCorner(closeBtn, 5); addStroke(closeBtn, Color3.fromRGB(120, 60, 160), 1)
...
sidebar.Position = UDim2.fromOffset(0, 44); sidebar.Size = UDim2.new(0, SIDEBAR_W, 1, -44)
sidebar.BackgroundColor3 = COLORS.sidebarBg; sidebar.BackgroundTransparency = 0.1
sidebar.BorderSizePixel = 0; sidebar.Parent = panel
local sidebarScroll = Instance.new("ScrollingFrame")
sidebarScroll.BackgroundTransparency = 1; sidebarScroll.BorderSizePixel = 0
sidebarScroll.Size = UDim2.new(1, 0, 1, 0); sidebarScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
sidebarScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; sidebarScroll.ScrollBarThickness = 3
sidebarScroll.ScrollBarImageColor3 = COLORS.accent; sidebarScroll.Parent = sidebar
local sidebarList = Instance.new("UIListLayout")
sidebarList.Padding = UDim.new(0, 3); sidebarList.SortOrder = Enum.SortOrder.LayoutOrder
sidebarList.Parent = sidebarScroll
local sidebarPad = Instance.new("UIPadding")
sidebarPad.PaddingTop = UDim.new(0, 8); sidebarPad.PaddingLeft = UDim.new(0, 6)
sidebarPad.PaddingRight = UDim.new(0, 6); sidebarPad.Parent = sidebarScroll
...
contentScroll.Size = UDim2.new(1, 0, 1, 0); contentScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
contentScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; contentScroll.ScrollBarThickness = 4
contentScroll.ScrollBarImageColor3 = COLORS.accent; contentScroll.Parent = content
...
function UI.OnChange(id, callback)
    UI.__callbacks[id] = UI.__callbacks[id] or {}; table.insert(UI.__callbacks[id], callback)
end
function H.getState(id, fallback)
    local v = UI.GetState(id); if v ~= nil then return v end; return fallback
end
function H.isOn(id) return UI.GetState(id) == true end
function H.optionValue(id, fallback)
    local v = UI.GetState(id); if v == nil then return fallback end; return v
end
function H.multiSelected(id)
    local value = UI.GetState(id); local selected = {}
...
local function makeTabButton(tabId, tabTitle)
    ...
    btn.Name = "Tab_" .. tabId; btn.Size = UDim2.new(1, 0, 0, 28)
    btn.BackgroundColor3 = COLORS.sectionBg; btn.BackgroundTransparency = 0.5
    btn.BorderSizePixel = 0; btn.AutoButtonColor = false
    btn.Font = Enum.Font.GothamMedium; btn.TextSize = 12; btn.TextColor3 = COLORS.textDim
    btn.TextXAlignment = Enum.TextXAlignment.Left; btn.Text = tabTitle; btn.Parent = sidebarScroll
    addCorner(btn, 5)
    local pad = Instance.new("UIPadding", btn); pad.PaddingLeft = UDim.new(0, 10)
    local indicator = Instance.new("Frame")
    indicator.Size = UDim2.new(0, 3, 0, 0); indicator.Position = UDim2.new(0, 0, 0.5, 0)
    indicator.AnchorPoint = Vector2.new(0, 0.5); indicator.BackgroundColor3 = COLORS.accent
    indicator.BorderSizePixel = 0; indicator.Parent = btn
...
function UI.AddTab(opts)
    ...
    page.AutomaticCanvasSize = Enum.AutomaticSize.Y; page.ScrollBarThickness = 4
    page.ScrollBarImageColor3 = COLORS.accent; page.Visible = false; page.Parent = contentScroll
    local pageList = Instance.new("UIListLayout")
    pageList.Padding = UDim.new(0, 8); pageList.SortOrder = Enum.SortOrder.LayoutOrder; pageList.Parent = page
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for tid, other in pairs(UI.__tabs) do
            other.Page.Visible = (tid == id); other.SetActive(tid == id)
        end
        UI.__activeTab = id
    end)
    UI.__tabs[id] = tab
...
function UI.AddSection(tab, opts)
    ...
    section.BackgroundColor3 = COLORS.sectionBg; section.BackgroundTransparency = 0.35
    section.BorderSizePixel = 0; section.Size = UDim2.new(1, 0, 0, 0)
    section.AutomaticSize = Enum.AutomaticSize.Y; section.Parent = tab.Page
    addCorner(section, 6); addStroke(section, COLORS.sectionBorder, 1)
    local sectionList = Instance.new("UIListLayout")
    sectionList.Padding = UDim.new(0, 5); sectionList.SortOrder = Enum.SortOrder.LayoutOrder; sectionList.Parent = section
    local sectionPad = Instance.new("UIPadding")
    sectionPad.PaddingTop = UDim.new(0, 8); sectionPad.PaddingBottom = UDim.new(0, 8)
    sectionPad.PaddingLeft = UDim.new(0, 10); sectionPad.PaddingRight = UDim.new(0, 10); sectionPad.Parent = section
    if opts.Title then
        local h = Instance.new("TextLabel")
        h.BackgroundTransparency = 1; h.Size = UDim2.new(1, 0, 0, 18)
        h.Font = Enum.Font.GothamBold; h.TextSize = 12; h.TextColor3 = COLORS.text
        h.TextXAlignment = Enum.TextXAlignment.Left; h.Text = opts.Title; h.Parent = section
    end
    if opts.Description then
        local d = Instance.new("TextLabel")
        d.BackgroundTransparency = 1; d.Size = UDim2.new(1, 0, 0, 14)
        d.Font = Enum.Font.Gotham; d.TextSize = 10; d.TextColor3 = COLORS.textMuted
        d.TextXAlignment = Enum.TextXAlignment.Left; d.TextWrapped = true
        d.Text = opts.Description; d.Parent = section
    end
    return section
end
...
local function makeRow(parent, height)
    ...
    row.BackgroundColor3 = COLORS.panelBg2; row.BackgroundTransparency = 0.4
    row.BorderSizePixel = 0; row.Size = UDim2.new(1, 0, 0, height or 28); row.Parent = parent
    addCorner(row, 5)
...
function UI.AddToggle(parent, opts)
    ...
    name.BackgroundTransparency = 1; name.Size = UDim2.new(1, -55, 0, 18)
    name.Font = Enum.Font.GothamMedium; name.TextSize = 12; name.TextColor3 = COLORS.text
    name.TextXAlignment = Enum.TextXAlignment.Left; name.Text = opts.Title or "Toggle"; name.Parent = row
    if opts.Description then
        local d = Instance.new("TextLabel")
        d.BackgroundTransparency = 1; d.Position = UDim2.fromOffset(0, 17)
        d.Size = UDim2.new(1, -55, 0, 12); d.Font = Enum.Font.Gotham
        d.TextSize = 10; d.TextColor3 = COLORS.textMuted
        d.TextXAlignment = Enum.TextXAlignment.Left; d.Text = opts.Description; d.Parent = row
    end
    local switch = Instance.new("TextButton")
    switch.Size = UDim2.fromOffset(36, 20); switch.Position = UDim2.new(1, -36, 0.5, -10)
    switch.BackgroundColor3 = COLORS.toggleOff; switch.BorderSizePixel = 0
    switch.Text = ""; switch.AutoButtonColor = false; switch.Parent = row
    addCorner(switch, 10)
    local knob = Instance.new("Frame")
    knob.Size = UDim2.fromOffset(16, 16); knob.Position = UDim2.fromOffset(2, 2)
    knob.BackgroundColor3 = COLORS.text; knob.BorderSizePixel = 0; knob.Parent = switch
    addCorner(knob, 8)
...
function UI.AddSlider(parent, opts)
    ...
    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1; label.Size = UDim2.new(1, -70, 0, 18)
    label.Font = Enum.Font.GothamMedium; label.TextSize = 12; label.TextColor3 = COLORS.text
    label.TextXAlignment = Enum.TextXAlignment.Left; label.Text = opts.Title or "Slider"; label.Parent = row
    local valueLabel = Instance.new("TextLabel")
    valueLabel.BackgroundTransparency = 1; valueLabel.Position = UDim2.new(1, -70, 0, 0)
    valueLabel.Size = UDim2.fromOffset(70, 18); valueLabel.Font = Enum.Font.GothamBold
    valueLabel.TextSize = 11; valueLabel.TextColor3 = COLORS.accentHi
    valueLabel.TextXAlignment = Enum.TextXAlignment.Right; valueLabel.Parent = row
    local bar = Instance.new("Frame")
    bar.Position = UDim2.fromOffset(0, 24); bar.Size = UDim2.new(1, 0, 0, 6)
    bar.BackgroundColor3 = COLORS.toggleOff; bar.BorderSizePixel = 0; bar.Parent = row
    addCorner(bar, 3)
    ...
    knob.Position = UDim2.new(0, 0, 0.5, 0); knob.BackgroundColor3 = COLORS.text
    knob.BorderSizePixel = 0; knob.Parent = bar
    addCorner(knob, 6)
    ...
    local function render()
        ...
        fill.Size = UDim2.new(alpha, 0, 1, 0); knob.Position = UDim2.new(alpha, 0, 0.5, 0)
        valueLabel.Text = tostring(value) .. suffix
    end
    render()
    local dragging = false
    local function updateFromX(x)
        local absX = bar.AbsolutePosition.X; local absW = bar.AbsoluteSize.X
        ...
        if snapped ~= value then
            value = snapped
            UI.SetState(opts.Id, value, true); render()
    ...
    conn(UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement
        or input.UserInputType == Enum.UserInputType.Touch) then updateFromX(input.Position.X) end
    end))
...
-- ⭐ UI.AddDropdown — PATCHED (trả về api với SetOptions / Refresh / Set)
function UI.AddDropdown(parent, opts)
    local isMulti = opts.Multi == true
    local row = makeRow(parent, 30)
    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1; label.Size = UDim2.new(1, -110, 1, 0)
    label.Font = Enum.Font.GothamMedium; label.TextSize = 12; label.TextColor3 = COLORS.text
    label.TextXAlignment = Enum.TextXAlignment.Left; label.Text = opts.Title or "Dropdown"; label.Parent = row
    local valueLabel = Instance.new("TextLabel")
    valueLabel.BackgroundTransparency = 1; valueLabel.Position = UDim2.new(1, -190, 0, 0)
    valueLabel.Size = UDim2.fromOffset(155, 30); valueLabel.Font = Enum.Font.Gotham
    valueLabel.TextSize = 11; valueLabel.TextColor3 = COLORS.textDim
    valueLabel.TextXAlignment = Enum.TextXAlignment.Right; valueLabel.TextTruncate = Enum.TextTruncate.AtEnd
    valueLabel.Parent = row
    local arrowBtn = Instance.new("TextButton")
    arrowBtn.Size = UDim2.fromOffset(26, 22); arrowBtn.Position = UDim2.new(1, -30, 0.5, -11)
    arrowBtn.BackgroundColor3 = COLORS.toggleOff; arrowBtn.BackgroundTransparency = 0.4
    arrowBtn.BorderSizePixel = 0; arrowBtn.AutoButtonColor = false; arrowBtn.Font = Enum.Font.GothamBold
    arrowBtn.TextSize = 11; arrowBtn.TextColor3 = COLORS.text; arrowBtn.Text = "v"; arrowBtn.Parent = row
    addCorner(arrowBtn, 4)
    local dropdown = Instance.new("Frame")
    dropdown.BackgroundColor3 = COLORS.panelBg2; dropdown.BorderSizePixel = 0
    dropdown.Size = UDim2.new(1, 0, 0, 0); dropdown.AutomaticSize = Enum.AutomaticSize.Y
    dropdown.Visible = false; dropdown.Parent = parent
    addCorner(dropdown, 5); addStroke(dropdown, COLORS.panelBorder, 1)
    local dropdownList = Instance.new("UIListLayout")
    dropdownList.Padding = UDim.new(0, 2); dropdownList.SortOrder = Enum.SortOrder.LayoutOrder
    ...
    local function addEntry(option)
        ...
        entry.Size = UDim2.new(1, 0, 0, 22); entry.BackgroundColor3 = COLORS.sectionBg
        entry.BackgroundTransparency = 0.6; entry.BorderSizePixel = 0; entry.AutoButtonColor = false
        entry.Font = Enum.Font.Gotham; entry.TextSize = 11; entry.TextColor3 = COLORS.text
        entry.TextXAlignment = Enum.TextXAlignment.Left; entry.Text = "  " .. tostring(option)
        entry.Parent = dropdown; addCorner(entry, 4); entries[option] = entry
    ...
    for _, option in ipairs(opts.Options or {}) do addEntry(option) end
    ...
    function api:Refresh(newOptions) api:SetOptions(newOptions or opts.Options or {}) end
    function api:Set(newSelected)
        if isMulti then
            selected = {}
            if type(newSelected) == "table" then
                for k, v in pairs(newSelected) do if v == true then selected[k] = true end end
            end
        else selected = newSelected end
        UI.SetState(opts.Id, selected, true)
        for optName, e in pairs(entries) do
            local active = isMulti and selected[optName] == true or (not isMulti and selected == optName)
            e.TextColor3 = active and COLORS.accentHi or COLORS.text
...
function UI.AddButton(parent, opts)
    ...
    btn.BorderSizePixel = 0; btn.AutoButtonColor = false; btn.Font = Enum.Font.GothamSemibold
    btn.TextSize = 12; btn.TextColor3 = COLORS.text; btn.Text = opts.Title or "Button"
    btn.TextXAlignment = Enum.TextXAlignment.Left; btn.Parent = row
    local action = Instance.new("TextLabel")
    action.BackgroundColor3 = COLORS.accent; action.BackgroundTransparency = 0.2
    action.Size = UDim2.fromOffset(62, 22); action.Position = UDim2.new(1, -62, 0.5, -11)
    action.Font = Enum.Font.GothamBold; action.TextSize = 11; action.TextColor3 = COLORS.text
    action.Text = opts.Text or "Run"; action.Parent = row
    addCorner(action, 4)
...
function UI.AddParagraph(parent, opts)
    ...
    title.Font = Enum.Font.GothamBold; title.TextSize = 11; title.TextColor3 = COLORS.text
    title.TextXAlignment = Enum.TextXAlignment.Left; title.Text = opts.Title or ""; title.Parent = row
    local body = Instance.new("TextLabel")
    body.BackgroundTransparency = 1; body.Position = UDim2.fromOffset(0, 16)
    body.Size = UDim2.new(1, 0, 0, 24); body.Font = Enum.Font.Gotham
    body.TextSize = 10; body.TextColor3 = COLORS.textDim
    body.TextXAlignment = Enum.TextXAlignment.Left; body.TextWrapped = true
    body.TextYAlignment = Enum.TextYAlignment.Top; body.Text = opts.Content or ""; body.Parent = row
    return row
end

function UI.AddDivider(parent, opts)
    local holder = Instance.new("Frame")
    holder.BackgroundTransparency = 1; holder.Size = UDim2.new(1, 0, 0, 10); holder.Parent = parent
    local line = Instance.new("Frame")
    line.BackgroundColor3 = COLORS.panelBorder; line.BorderSizePixel = 0
    line.Position = UDim2.new(0, 0, 0.5, 0); line.Size = UDim2.new(1, 0, 0, 1); line.Parent = holder
    if opts and opts.Title then
        local lbl = Instance.new("TextLabel")
        lbl.BackgroundTransparency = 1; lbl.Size = UDim2.new(1, 0, 1, 0)
        lbl.Font = Enum.Font.Gotham; lbl.TextSize = 10; lbl.TextColor3 = COLORS.textMuted
        lbl.TextXAlignment = Enum.TextXAlignment.Center; lbl.Text = opts.Title; lbl.Parent = holder
    end
    return holder
end
...
function UI.AddStatus(parent, opts)
    ...
    local name = Instance.new("TextLabel")
    name.BackgroundTransparency = 1; name.Size = UDim2.new(0.6, 0, 1, 0)
    name.Font = Enum.Font.Gotham; name.TextSize = 11; name.TextColor3 = COLORS.textDim
    name.TextXAlignment = Enum.TextXAlignment.Left; name.Text = opts.Title or "Status"; name.Parent = row
    local value = Instance.new("TextLabel")
    value.BackgroundTransparency = 1; value.Size = UDim2.new(0.4, 0, 1, 0)
    value.Position = UDim2.fromScale(0.6, 0); value.Font = Enum.Font.GothamBold
    value.TextSize = 11; value.TextColor3 = COLORS.text; value.TextXAlignment = Enum.TextXAlignment.Right
    value.Text = tostring(opts.Value or ""); value.Parent = row
    local api = {}
...
function UI.AddInput(parent, opts)
    ...
    label.Font = Enum.Font.GothamMedium; label.TextSize = 12; label.TextColor3 = COLORS.text
    label.TextXAlignment = Enum.TextXAlignment.Left; label.Text = opts.Title or "Input"; label.Parent = row
    local box = Instance.new("TextBox")
    box.BackgroundColor3 = COLORS.toggleOff; box.BackgroundTransparency = 0.2
    box.BorderSizePixel = 0; box.Position = UDim2.fromScale(0.42, 0.5)
    box.AnchorPoint = Vector2.new(0, 0.5); box.Size = UDim2.new(0.58, -10, 0, 22)
    box.Font = Enum.Font.Gotham; box.TextSize = 11; box.TextColor3 = COLORS.text
    box.PlaceholderColor3 = COLORS.textMuted; box.PlaceholderText = opts.Placeholder or ""
    box.Text = tostring(opts.Default or ""); box.ClearTextOnFocus = false
    box.TextXAlignment = Enum.TextXAlignment.Left; box.Parent = row
    addCorner(box, 4)
...
NotifyHolder.BackgroundTransparency = 1; NotifyHolder.AnchorPoint = Vector2.new(1, 1)
NotifyHolder.Position = UDim2.new(1, -16, 1, -16); NotifyHolder.Size = UDim2.fromOffset(260, 380)
NotifyHolder.Parent = screenGui
local NotifyList = Instance.new("UIListLayout")
NotifyList.Padding = UDim.new(0, 6); NotifyList.SortOrder = Enum.SortOrder.LayoutOrder
NotifyList.VerticalAlignment = Enum.VerticalAlignment.Bottom
NotifyList.HorizontalAlignment = Enum.HorizontalAlignment.Right; NotifyList.Parent = NotifyHolder

function H.notify(title, content, variant, duration)
    local note = Instance.new("Frame")
    note.BackgroundColor3 = COLORS.panelBg; note.BackgroundTransparency = 0.05
    note.BorderSizePixel = 0; note.Size = UDim2.fromOffset(250, 50); note.Parent = NotifyHolder
    addCorner(note, 8)
    ...
    t.BackgroundTransparency = 1; t.Position = UDim2.fromOffset(12, 6)
    t.Size = UDim2.new(1, -24, 0, 18); t.Font = Enum.Font.GothamBold
    t.TextSize = 12; t.TextColor3 = strokeColor; t.TextXAlignment = Enum.TextXAlignment.Left
    t.Text = tostring(title or "ZenHubX"); t.Parent = note
    local c = Instance.new("TextLabel")
    c.BackgroundTransparency = 1; c.Position = UDim2.fromOffset(12, 22)
    c.Size = UDim2.new(1, -24, 0, 22); c.Font = Enum.Font.Gotham
    c.TextSize = 10; c.TextColor3 = COLORS.textDim; c.TextXAlignment = Enum.TextXAlignment.Left
    c.TextWrapped = true; c.TextYAlignment = Enum.TextYAlignment.Top
    c.Text = tostring(content or ""); c.Parent = note
...
do
    ...
    UI.AddButton(QuickSec, { Title = "Về căn cứ", Text = "Về", Callback = function()
        task.spawn(function()
            if not H.getBasePosition() or not H.returnToBaseBypass(nil) then
                H.notify("Về căn cứ", "Không tìm thấy căn cứ", "Warning", 3)
            end
        end)
    end })
    ...
    UI.AddButton(QuickSec, { Title = "Về Void (Reset nhanh)", Text = "Void", Callback = function()
        task.spawn(function()
            if ExtrasState.VoidReset() then H.notify("Void", "Đã thả xuống Void", "Success", 3)
            else H.notify("Void", "Không tìm thấy nhân vật", "Error", 3) end
        end)
    end })
    ...
    VisualPetFolder.Name = "ZenHubXVisualPets"; VisualPetFolder.Parent = Workspace
    ...
    local function findVisualPet(name)
        local q = tostring(name or ""):lower(); if q == "" then return nil end
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("Model") and not obj:IsDescendantOf(VisualPetFolder)
                and obj.Name:lower():find(q, 1, true)
                and obj:FindFirstChildWhichIsA("BasePart", true) then return obj end
        end
        return nil
    end

    local function removeAllVisualPets()
        for _, data in ipairs(VisualPetState.spawned) do pcall(function() data.model:Destroy() end) end
    ...
    local function spawnVisualPet(name)
        ...
        if not source then H.notify("Visual Pet", "Không tìm thấy pet: " .. tostring(name), "Warning", 4); return end
        local clone = source:Clone()
        for _, obj in ipairs(clone:GetDescendants()) do
            if obj:IsA("Script") or obj:IsA("LocalScript") or obj:IsA("ModuleScript") then obj:Destroy()
            elseif obj:IsA("BasePart") then
                obj.Anchored = true; obj.CanCollide = false; obj.CanTouch = false; obj.CastShadow = false
            end
        end
        clone.Name = "VP_" .. tostring(name); clone.Parent = VisualPetFolder
        table.insert(VisualPetState.spawned, { model = clone })
        H.notify("Visual Pet", "Đã spawn " .. tostring(name) .. " (chỉ client).", "Success", 3)
    end
    ...
    UI.AddButton(PetsOverview, {
        ...
        Callback = function()
            ...
            if #list == 0 then H.notify("Visual Pet", "Chưa tìm thấy model pet.", "Warning", 4); return end
            pcall(function() VisualPetDropdown:SetOptions(list) end)
            VisualPetState.inputName = list[1]
            UI.SetState("VisualPetName", list[1], true)
            H.notify("Visual Pet", "Đã tìm thấy " .. #list .. " pet.", "Success", 3)
        end
    })
    ...
    UI.AddButton(PetsOverview, {
        ...
        Callback = function()
            if VisualPetState.inputName == "" or VisualPetState.inputName == "Đang quét..." then
                H.notify("Visual Pet", "Hãy chọn hoặc nhập tên pet.", "Warning", 4); return
            end
    ...
    task.spawn(function()
        while screenGui.Parent do
            ...
            if root and count > 0 then
                local center = root.Position + Vector3.new(0, 1, 0); local t = os.clock()
                for i, data in ipairs(VisualPetState.spawned) do
                    if data.model and data.model.Parent then
                        ...
                        local pos = center + Vector3.new(
                            math.cos(angle) * VisualPetState.orbitRadius, 0,
    ...
    UI.AddToggle(SellPetSec, {
        ...
        Callback = function(v)
            ...
            if v then H.notify("Auto Sell Pet", "Đã bật. Quét pet mỗi " .. PetSellConfig.SellPetInterval .. "s.", "Success", 3)
            else H.notify("Auto Sell Pet", "Đã tắt.", "Info", 3) end
        end
    })
    UI.AddToggle(SellPetSec, {
        Id = "UseWhitelist", Title = "Dùng Whitelist",
        Description = "BẬT = chỉ bán pet trong list | TẮT = bán tất cả trừ pet trong list",
        Default = true, Callback = function(v) PetSellConfig.UseWhitelist = v end
    })
    UI.AddToggle(SellPetSec, {
        Id = "KeepBestPet", Title = "Giữ pet mạnh nhất mỗi loại",
        Description = "Không bán pet có scale cao nhất của mỗi tên",
        Default = true, Callback = function(v) PetSellConfig.KeepBestPet = v end
    })
    ...
    local PetNameDropdown = UI.AddDropdown(SellPetSec, {
        ...
        Options = { "Đang quét..." }, Default = {}, Multi = true,
        Callback = function()
    ...
    UI.AddDropdown(SellPetSec, {
        ...
        Options = RARITIES, Default = {}, Multi = true,
        Callback = function()
    ...
    UI.AddButton(SellPetSec, {
        ...
        Callback = function()
            task.spawn(function()
                ...
                table.sort(names)
                if #names == 0 then H.notify("Auto Sell Pet", "Không tìm thấy pet nào.", "Warning", 3); return end
                pcall(function() PetNameDropdown:SetOptions(names) end)
                H.notify("Auto Sell Pet", "Đã quét " .. #names .. " loại pet.", "Success", 3)
            end)
        end
    })
    UI.AddButton(SellPetSec, {
    ...
    UI.AddButton(SellPetSec, {
        ...
        Callback = function()
            ...
            H.notify("Auto Sell Pet", "Đã xoá danh sách.", "Info", 3)
        end
    })
    task.delay(5, function()
        pcall(function()
            ...
            table.sort(names)
            if #names > 0 then PetNameDropdown:SetOptions(names) end
        end)
    end)
    ...
    UI.AddToggle(MoveSec, { Id = "Fly", Title = "Bay", Default = false, Callback = function(v)
        if not v then
            ...
            local root = H.getRoot(); local lv = root and root:FindFirstChild("ZenHubXFlyLV")
    ...
    UI.AddButton(TeleportSec, { Title = "Dịch chuyển tới điểm", Text = "Đi", Callback = function()
        task.spawn(function()
            ...
            if not position then H.notify("Waypoint", "Không khả dụng", "Warning", 3); return end
            if not H.bypassMoveTo(position, nil, BYPASS_SPEED) then H.notify("Waypoint", "Thất bại", "Error", 3) end
        end)
    end })
    ...
    UI.AddButton(SpeedSec2, { Title = "Áp dụng Bypass tốc độ", Text = "Áp dụng",
        Callback = function()
            task.spawn(function()
                ...
                H.notify("Tốc độ", ok and ("Đã bypass " .. sp .. " thành công") or "Thất bại",
    ...
    UI.AddButton(SessionSysSec, { Title = "Sao chép script vào server", Text = "Copy", Callback = function()
        ...
        H.notify("Đã sao chép", "Đã sao chép script vào server", "Success", 3)
    end })
    ...
    UI.AddButton(WebhookSec, { Title = "Gửi tổng kết ngay", Text = "Gửi", Callback = function()
        task.spawn(function()
            ...
            H.notify("Webhook", sent and "Đã gửi" or "Thất bại", sent and "Success" or "Error", 3)
        end)
    end })
...
local function refreshHomeDashboard()
    ...
    pcall(function()
        if refs.carryingRow then
            refs.carryingRow:SetValue(IsCarryingEgg and "Có" or "Không")
...
H.track(RunService.RenderStepped:Connect(function(dt)
    ...
    if not lv then
        lv = Instance.new("LinearVelocity"); lv.Name = "ZenHubXFlyLV"; lv.MaxForce = 1e6
        lv.RelativeTo = Enum.ActuatorRelativeTo.World
        local att = Instance.new("Attachment"); att.Name = "ZenHubXFlyAttachment"; att.Parent = root
    ...
    if direction.Magnitude > 0 then lv.VectorVelocity = direction.Unit * speed
    else lv.VectorVelocity = Vector3.zero end
end))
...
H.track(RunService.Heartbeat:Connect(function()
    ...
    if schedulerDue("dashboard", 2) then
        ...
        if H.isOn("WalkSpeedEnabled") and not ExtrasState.SpeedBypass then
            local humanoid = H.getHumanoid()
            if humanoid then humanoid.WalkSpeed = math.min(tonumber(H.optionValue("WalkSpeed", 32)) or 32, MAX_SAFE_SPEED) end
        end
        if H.isOn("JumpPowerEnabled") then
            local humanoid = H.getHumanoid()
            if humanoid then humanoid.UseJumpPower = true; humanoid.JumpPower = tonumber(H.optionValue("JumpPower", 50)) or 50 end
        end
    ...
    if schedulerDue("sellPets", tonumber(H.optionValue("SellPetInterval", 15)) or 15) then
        if H.isOn("AutoSellPets") and not AutomationBusy and not IsCarryingEgg then
            task.spawn(function() pcall(H.runAutoSellPetsAdvanced) end)
        end
    end
    ...
    if schedulerDue("session", 4) then
        if H.isOn("AntiAfk") then
            ...
            if (idleFor >= 300 and sinceLastTap >= 60) or (idleFor < 300 and sinceLastTap >= 300) then
                pcall(function()
                    local humanoid = H.getHumanoid()
                    if humanoid then humanoid.Jump = true; LastAntiAfkAt = tick() end
                end)
            end
        end
...
H.notify("ZenHubX", "Sẵn sàng - ấn avatar để mở", "Success", 5)
