...
-- ProximityPrompt Auto Steal (from script 2)
...
-- Detailed Server Hop GUI (from script 2)
...
do
    ...
    UI.AddButton(QuickSec, { Title = "Về Void (Reset nhanh)", Text = "Void", Callback = function()
        task.spawn(function()
            if ExtrasState.VoidReset() then
                H.notify("Void", "Đã thả xuống Void", "Success", 3)
            else
                H.notify("Void", "Không tìm thấy nhân vật", "Error", 3)
            end
        end)
    end })
    ...
    local SellPetSec = UI.AddSection(PetsTab, { Title = "Tự bán pet" })
    ...
    UI.AddToggle(SellPetSec, { Id = "AutoSellPets", Title = "Tự bán pet", Default = false })
    UI.AddDropdown(SellPetSec, { Id = "SellRarities", Title = "Độ hiếm bán", Options = RARITIES, Multi = true, Default = {} })
    UI.AddDropdown(SellPetSec, { Id = "SellMutations", Title = "Biến thể bán", Options = MUTATIONS, Multi = true, Default = {} })
    UI.AddToggle(SellPetSec, { Id = "SellKeepMutated", Title = "Không bán pet biến thể", Default = true })
    UI.AddToggle(SellPetSec, { Id = "SellKeepEquipped", Title = "Không bán pet đang trang bị", Default = true })
    UI.AddSlider(SellPetSec, { Id = "SellMaxScale", Title = "Scale tối đa để bán", Min = 0, Max = 10, Default = 10, Step = 0.1 })
    UI.AddSlider(SellPetSec, { Id = "SellInterval", Title = "Chu kỳ bán", Min = 1, Max = 120, Default = 6, Step = 1, Suffix = " giây" })
    ...
    UI.AddButton(SpeedSec2, { Title = "Áp dụng Bypass tốc độ", Text = "Áp dụng",
        Callback = function()
            task.spawn(function()
                ...
                H.notify("Tốc độ", ok and ("Đã bypass " .. sp) or "Thất bại",
...
-- FLY (đã fix typo VectorVelocity)
