--[[
    Apex Hub - Steal an Egg (Combined Script: File 1 + File 2)
    Standalone UI (No external library)
    Discord: https://discord.gg/kptjwzKWgX
--]]

-- ============================================================
-- COMPAT SHIMS & INITIALIZATION
-- ============================================================
...
function H.isOn(name) return SettingsState.Toggles[name] == true end
function H.optionValue(name, default)
    local val = SettingsState.Options[name]
    return val ~= nil and val or default
end
function H.selectionAllows(name, key)
    local sel = SettingsState.Selections[name]
    if not sel or not next(sel) then return true end
    return sel[key] == true
end
function H.multiSelected(name) return SettingsState.Selections[name] or {} end
function H.multiHasAny(name)
    local sel = SettingsState.Selections[name]
    if not sel then return false end
    for _, v in pairs(sel) do if v then return true end end
    return false
end
function H.matchesEggFilters(eggRecord, zoneOption, rarityOption, mutationOption)
    if not eggRecord then return false end
    if zoneOption and not H.selectionAllows(zoneOption, eggRecord.AreaId) then return false end
    local rarity = H.resolveRarity(eggRecord.AssetCategory)
    if rarityOption and H.multiHasAny(rarityOption) and not H.selectionAllows(rarityOption, rarity) then return false end
    if mutationOption and H.multiHasAny(mutationOption) then
        local eggMutations = H.recordMutations(eggRecord)
        local hasMut = false
        for _, mut in ipairs(eggMutations) do
            if H.selectionAllows(mutationOption, mut) then hasMut = true; break end
        end
...
-- ============================================================
-- GAME HELPERS & ANTI-CHEAT / BYPASS
-- ============================================================
...
-- TELEPORT & SERVER HOP
-- ============================================================
function H.teleportToWaypoint(waypointName)
    local pos = H.resolveWaypoint(waypointName)
    if not pos then return false end
    local root = H.getRoot()
    if root then H.placeRoot(root, CFrame.new(pos)) end
    return true
end
    ...
    while running do
        ...
        if not AutomationBusy then
            ...
            for _, taskName in ipairs(order) do
                local job = AutomationJobs[taskName]
                local lastRun = AutomationLastRunAt[taskName] or 0
                if job and (os.clock() - lastRun >= job.Interval) and job.Ready() then
                    AutomationBusy = true
                    AutomationLastRunAt[taskName] = os.clock()
                    pcall(job.Run)
        ...
        if H.isOn("AutoReturn") then pcall(H.runAutoReturn) end
        if H.isOn("AutoSellPets") then pcall(H.runAutoSellPets) end
        if H.isOn("AutoSellEggs") then pcall(H.runAutoSellEggs) end
        if H.isOn("AutoFusePets") then pcall(H.runAutoFusePets) end
        if H.isOn("AutoEquipBest") then pcall(H.runAutoEquipBest) end
        ...
        if H.isOn("AutoBuyTrail") then pcall(H.runAutoBuyTrail) end
        if H.isOn("AutoUpgrades") then pcall(H.runAutoUpgrades) end
        ...
        if H.isOn("AutoClaimGroup") then pcall(H.runAutoClaimGroupReward) end
        if H.isOn("DeletePetRenders") then pcall(H.deleteOwnPetRenders) end
        if H.isOn("AutoServerHop") then pcall(H.runServerHop) end

        -- Webhook Trackers & Anti AFK
        pcall(H.trackWebhookEvents)
        if H.isOn("WebhookEnabled") then pcall(H.runWebhookSummary) end

        if H.isOn("AntiAfk") and (tick() - LastAntiAfkAt > 60) then
            LastAntiAfkAt = tick()
            pcall(function()
...
print("[Apex Hub] Loaded successfully with all features combined!")