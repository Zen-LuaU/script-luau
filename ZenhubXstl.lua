--[[
    ...
    [MOD] Fly-in Forest grab → RAGDOLL → TELE NGAY (không chờ hết ragdoll) → grab target → walk back
    [MOD2] Anti-Knock clamp velocity không desy, không xung đột về base
--]]
...
-- Chờ ragdoll với callback — trả về true NGAY KHI phát hiện ragdoll
...
-- ============================================================
-- [MOD2] ANTI-KNOCK: clamp velocity, không desy, không xung đột về base
-- Chỉ bật khi ở Forest warm-up. TẮT hoàn toàn khi bypassMoveTo / return base.
-- ============================================================
local AntiKnock = {
    Enabled = false,
    MaxHorizontal = 22,
...
H.track(RunService.Stepped:Connect(function()
    ...
    -- Re-enable Motor6D (giữ limbs dính)
    ...
    -- Clamp velocity — KHÔNG đổi state để ragdoll tự nhiên
...
-- ============================================================
-- [MOD] stealEgg:
--   B0: BAY tới Forest → nhặt trứng → CHỜ RAGDOLL → TELE NGAY target → grab
...
function H.stealEgg(slotEgg)
            ...
            if forestSlot then
                ...
                if forestPos then
                    ...
                    H.waitForRagdoll(RAGDOLL_WAIT_TIMEOUT, function()
                        ...
                        -- TELE NGAY tới trứng đích khi vừa phát hiện ragdoll
                        AntiKnock.Enabled = false
                    ...
                    -- Nếu ragdoll không kích hoạt trong timeout, vẫn tele tới trứng
                    ...
                    -- Ngay sau tele, khoá lại ragdoll states để grab target an toàn
                    H.setRagdollStatesAllowed(false)

                    -- Ép đứng dậy nếu còn state ragdoll
                    local hum = H.getHumanoid()
                    if hum then
                        pcall(function()
                    ...
                    -- Drop trứng Forest nếu đang cầm (không mang về)
    ...
    -- ========================================================
    -- BƯỚC 1: Đảm bảo đã ở vị trí trứng (tele phòng trường hợp chưa)
    -- ========================================================
    do
        root = H.getRoot()
        if root then
