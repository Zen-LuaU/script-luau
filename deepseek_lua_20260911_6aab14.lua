--[[
    ...
    Farm Egg + Visual Pet + Spy ESP + Teleport Step + Anti-Cheat
]]

-- ============================================================
-- ANTI-CHEAT (kích hoạt ngay khi load script)
-- ============================================================
pcall(function()
    ...
    if type(dir) == "table" then
        ...
        for id, item in pairs(dir) do
            if type(id) == "string" and type(item) == "table" then
                ...
                entries[#entries+1] = {id = id, label = label, rarity = rarity}
            end
        end
...
WindUI:Notify({Title = "zen hubX steal an egg", Content = "Đã load "..#AreaList.." khu vực! Anti-cheat active.", Duration = 6})