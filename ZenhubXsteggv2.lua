--[[
    ...
    [MOD] Teleport-to-egg + Forest warm-up ragdoll wait
--]]
...
-- ============================================================
-- [MOD] RAGDOLL HELPERS + FOREST WARM-UP
-- ============================================================
...
-- Chọn 1 record trứng bất kỳ trong Forest còn ở Slot/Dropped
...
-- Lấy vị trí world của record (dùng AreaEggSlotsClient theo Uid)
function H.getRecordWorldPosition(record)
    if typeof(record) ~= "table" or typeof(record.Uid) ~= "string" then return nil end
    local slots = AreaEggSlotsClient and AreaEggSlotsClient:GetChildren() or {}
    for _, slot in ipairs(slots) do
        if slot.Name == record.Uid then
            return H.getSlotEggPosition(slot)
        end
    end
    if record.BottomCFrame then return record.BottomCFrame.Position end
    if record.BoundsCFrame then return record.BoundsCFrame.Position end
    return nil
end

-- Teleport tới 1 vị trí ngẫu nhiên trong zone Forest
function H.teleportToRandomForestSpot()
    local root = H.getRoot()
    ...
    local rx = center.X + (math.random() - 0.5) * math.max(4, size.X * 0.6)
    local rz = H.getLaneZ() + (math.random() - 0.5) * 4
    local ry = H.groundedY(rx, rz, center.Y)
    H.placeRoot(root, CFrame.new(rx, ry, rz))
    return true
end

-- ============================================================
-- [MOD] stealEgg: warm-up Forest → chờ ragdoll → tele trứng → grab → đi bộ về
-- ============================================================
function H.stealEgg(slotEgg)
    ...
    -- === BƯỚC 0: WARM-UP TẠI FOREST ===
    -- Tele tới 1 chỗ ngẫu nhiên trong Forest, chờ ragdoll xảy ra
    -- (do anti-cheat tele detect) rồi mới tele tới trứng để tránh bị giật.
    do
        local warmupOk = H.teleportToRandomForestSpot()
        if warmupOk then
            task.wait(0.05)
            -- Chờ ragdoll bật lên (tối đa 2.5s)
            local gotRagdoll = H.waitForRagdoll(2.5)
            if gotRagdoll then
                -- Chờ ragdoll kết thúc để character ổn định
                H.waitRagdollEnd(1.2)
            end
            -- Reset PlatformStand để chuẩn bị tele
            local hum = H.getHumanoid()
            if hum then
                pcall(function()
    ...
    -- === BƯỚC 5: ĐI BỘ về base (không tele) ===
