--[[
    ZenHubX v3.1 - Steal an Egg
    Standalone UI - Delta Safe
    Discord: https://discord.gg/vggkAdBcuJ
    TikTok: zenhub.z
--]]

-- ============================================================
-- MASTER SAFE WRAPPER - Chống lỗi "attempt to call a nil value"
-- ============================================================
local ok, err = xpcall(function()

-- ============================================================
-- COMPAT SHIMS (không sửa global, chỉ dùng local)
-- ============================================================
local _getgenv = (rawget and rawget(_G, "getgenv")) or getgenv
local _gethui  = (rawget and rawget(_G, "gethui")) or gethui
local _setfpscap = (rawget and rawget(_G, "setfpscap")) or setfpscap
local _getconnections = (rawget and rawget(_G, "getconnections")) or getconnections

local _unpack = table.unpack or unpack
local _pack = table.pack or function(...) return { n = select("#", ...), ... } end
local _typeof = typeof or type
local function _clamp(v, mn, mx)
    if v < mn then return mn elseif v > mx then return mx end
    return v
end
local function _find(t, v, init)
    if type(t) ~= "table" then return nil end
    for i = tonumber(init) or 1, #t do if t[i] == v then return i end end
    return nil
end

local genv = (type(_getgenv) == "function" and _getgenv()) or _G
...
local LANG = {
    ...
    Strings = {
        vi = {
            ...
            copy = "Chép", ready = "Sẵn sàng - ấn avatar mở menu",
        },
        en = {
            ...
            copy = "Copy", ready = "Ready - press avatar to open",
        },
    },
}
local function L(key)
    local s = LANG.Strings[LANG.Current]
    return (s and s[key]) or key
end
...
function H.requirePath(root, timeout, ...)
    ...
    for _, name in ipairs(names) do
        ...
        if not child then
            local ok2, result = pcall(function() return cur:WaitForChild(name, timeout or 4) end)
            child = ok2 and result or nil
        end
        cur = child
    end
    if not cur then return nil end
    local ok2, mod = pcall(require, cur)
    ...
    for _, d in ipairs(ReplicatedStorage:GetDescendants()) do
        if d:IsA("RemoteEvent") or d:IsA("RemoteFunction") then
            ...
            for _, part in ipairs(parts) do
                if not string.find(d.Name, part, 1, true) then ok2 = false; break end
            end
...
function H.pickFromTable(tbl, ...)
    if _typeof(tbl) ~= "table" then return nil end
    local names = { ... }
    local cur = tbl
    for _, name in ipairs(names) do
        if _typeof(cur) ~= "table" then return nil end
        cur = cur[name]
    end
    return cur
end
...
function H.pickFn(mod, ...)
    if _typeof(mod) ~= "table" then return nil end
    for i = 1, select("#", ...) do
        local key = select(i, ...)
        local fn = mod[key]
        if _typeof(fn) == "function" then return fn end
    end
    return nil
end
...
function H.remoteFrom(mod, ...)
    ...
    if _typeof(inst) == "Instance" then return inst end
    return nil
end
...
local AreaNames = {}
if AreasModule and _typeof(AreasModule.Directory) == "table" then
    for areaName in pairs(AreasModule.Directory) do table.insert(AreaNames, areaName) end
    table.sort(AreaNames)
else
    for i = 1, #AREA_ORDER do AreaNames[i] = AREA_ORDER[i] end
end

local TrailNames, TrailIdByName, TrailPriceByName = {}, {}, {}
if TrailsModule and _typeof(TrailsModule.Directory) == "table" then
    local trailEntries = {}
...
if GearsModule then
    ...
    if _typeof(gearDirectory) == "table" then
        for _, gearData in pairs(gearDirectory) do
            if _typeof(gearData) == "table" and _typeof(gearData.DisplayName) == "string" then
...
local StealConfig = { GrabDelay = 0.55, ArriveDistance = 1.35, MoveTimeout = 14 }
...
if genv then
    local history = genv.ZenHubXHopHistory
    if _typeof(history) ~= "table" then history = {}; genv.ZenHubXHopHistory = history end
...
if not AreasFolder then
    ...
    if not objects then
        local ok2, result = pcall(function() return Workspace:WaitForChild("__OBJECTS", 8) end)
        objects = ok2 and result or nil
    end
    AreasFolder = objects and objects:FindFirstChild("Areas")
end
...
function H.getSave()
    if not SaveModule or _typeof(SaveModule.Get) ~= "function" then return nil end
    local ok2, save = pcall(SaveModule.Get)
    return ok2 and save or nil
end
function H.netInvoke(remote, ...)
    if _typeof(remote) ~= "Instance" then return nil end
    local args = _pack(...)
    local result, done = nil, false
    task.spawn(function()
        if remote:IsA("RemoteFunction") then
            result = _pack(pcall(function() return remote:InvokeServer(_unpack(args, 1, args.n)) end))
        elseif remote:IsA("RemoteEvent") then
            result = _pack(pcall(function() remote:FireServer(_unpack(args, 1, args.n)); return true end))
        else result = _pack(false) end
        done = true
    end)
...
function H.netCall(remote, ...)
    if _typeof(remote) == "Instance" and remote:IsA("RemoteEvent") then
        return pcall(function(...) remote:FireServer(...) end, ...)
    end
    return H.netInvoke(remote, ...)
end
function H.countTable(value)
    if _typeof(value) ~= "table" then return 0 end
    local count = 0
...
function H.resolveRarity(assetCategory)
    if _typeof(assetCategory) ~= "string" or not AssetsData or _typeof(AssetsData.Directory) ~= "table" then return nil end
...
function H.assetName(assetCategory)
    if AssetsData and _typeof(AssetsData.Directory) == "table" then
...
function H.recordMutations(asset)
    local mutations = {}
    if _typeof(asset) ~= "table" then return mutations end
    if _typeof(asset.Mutations) == "table" then
        for _, mutation in pairs(asset.Mutations) do
            if _typeof(mutation) == "string" then table.insert(mutations, mutation) end
        end
    end
    if _typeof(asset.BaseMutation) == "string" then table.insert(mutations, asset.BaseMutation) end
    return mutations
end
function H.getLaneZ()
    if AreasFolder then
...
function H.getZoneLaneCenter(zoneName)
    ...
    local ok2, pivot = pcall(function() return zone:GetBoundingBox() end)
    if ok2 and pivot then return Vector3.new(pivot.Position.X, H.getLaneY(), H.getLaneZ()) end
    return nil
end
...
function H.groundedY(x, z, fallbackY)
    ...
    if hitY then return _clamp(hitY + characterOffset, laneY - 2, laneY + 5) end
    if _typeof(fallbackY) == "number" then return _clamp(fallbackY, laneY - 2, laneY + 5) end
    return laneY + 3
end
...
function H.prepareStealHumanoid()
    ...
    local ok2, clone = pcall(function()
...
function H.humanoidStealMoveTo(position, keepGoing)
    if _typeof(position) ~= "Vector3" or not running then return false end
    local character = LocalPlayer.Character
    ...
    for _, point in ipairs(route) do
        ...
        while running and os.clock() < deadline do
            ...
            local dt = _clamp(RunService.Heartbeat:Wait(), 0, 1 / 30)
            ...
            if flatDir.Magnitude > 0.001 then
                newCF = CFrame.lookAt(nextPos, nextPos + flatDir.Unit)
            else
                newCF = CFrame.new(nextPos)
            end
...
function H.bypassMoveTo(position, keepGoing, speed)
    ...
    while running and os.clock() < deadline do
        ...
        if flatDir.Magnitude > 0.001 then
            bg.CFrame = CFrame.lookAt(root.Position, root.Position + flatDir.Unit)
        end
        RunService.Heartbeat:Wait()
    end
...
function H.getAreaEggs()
    ...
    if _typeof(snapshot) ~= "table" or _typeof(snapshot.Records) ~= "table" then
        if EggApi.RequestAreaEggSnapshot then pcall(EggApi.RequestAreaEggSnapshot) end
        snapshot = EggApi.GetAreaEggSnapshot()
    end
    if _typeof(snapshot) ~= "table" or _typeof(snapshot.Records) ~= "table" then return {} end
    local records = {}
    for _, record in pairs(snapshot.Records) do
        if _typeof(record) == "table" and _typeof(record.Uid) == "string" then table.insert(records, record) end
    end
    return records
end
...
function H.isStealCandidate(record, bypassFilters)
    if _typeof(record) ~= "table" or _typeof(record.Uid) ~= "string" then return false end
...
function H.pickStealTarget()
    ...
    for _, record in ipairs(H.getAreaEggs()) do
        if _typeof(record.Uid) == "string" then recordsByUid[record.Uid] = record end
    end
...
function H.eggInventoryCount()
    ...
    if _typeof(inventory) ~= "table" then return 0 end
...
function H.tryCarryEgg(eggInstance)
    ...
    local ok2, carried = pcall(function() return EggApi.RequestCarryAreaEgg(uid, slotKey) end)
    if ok2 and carried == true then return true end
...
if EggApi.AreaEggCarryStateChanged and _typeof(EggApi.AreaEggCarryStateChanged.Connect) == "function" then
    track(EggApi.AreaEggCarryStateChanged:Connect(function(state)
        local carryingNow = _typeof(state) == "table" and state.IsCarrying == true
...
function H.getUnplacedEggUids()
    ...
    local result = {}
    if _typeof(eggInventory) ~= "table" then return result end
    local bypass = H.isOn("AutoPlaceAll") and not H.isOn("AutoPlaceSelected")
    for uid, egg in pairs(eggInventory) do
        if _typeof(uid) == "string" and _typeof(egg) == "table" and egg.Placement == nil
...
function H.runAutoOpenReadyEggs()
    local save = H.getSave()
    local eggInventory = save and save.EggInventory
    if _typeof(eggInventory) ~= "table" then return end
...
function H.getPetItemData(serializedPet)
    ...
    local ok2, itemData = pcall(AssetItemsApi.Deserialize, serializedPet)
    if not ok2 or _typeof(itemData) ~= "table" then return nil end
    return itemData
end
...
function H.getSellablePets()
    ...
    local result = {}
    if _typeof(inventory) ~= "table" then return result end
    ...
    for uid, asset in pairs(inventory) do
        if _typeof(uid) == "string" and _typeof(asset) == "table" then
            local itemData = H.getPetItemData(asset)
            local isEquipped = _find(equipped, uid) ~= nil
            ...
            if not protected then
                ...
                local rarityAllowed = not filterRarities or (_typeof(rarity) == "string" and selectedRarities[rarity] == true)
...
function H.getSellableEggUids()
    ...
    for uid, egg in pairs(eggInventory) do
        if _typeof(uid) == "string" and _typeof(egg) == "table" and egg.Placement == nil then
            local rarity = H.resolveRarity(egg.AssetCategory)
            if not useRarityFilter or (_typeof(rarity) == "string" and selectedRarities[rarity] == true) then
                table.insert(result, uid)
            end
        end
    end
...
function H.fuseGroups(save)
    local inventory = save and save.Inventory
    local groups = {}
    if _typeof(inventory) ~= "table" then return groups end
    local equipped = save.EquippedAssets or {}
    ...
    for uid, pet in pairs(inventory) do
        if _typeof(uid) == "string" and _typeof(pet) == "table" then
            local category = pet.Category
            local selectable = false
            if _typeof(category) == "string" and FuseApi.CanSelectPet then
                pcall(function() selectable = FuseApi.CanSelectPet(uid, pet, category, false) == true end)
            end
            if selectable and not (keepEquipped and _find(equipped, uid) ~= nil) then
...
function H.fusePrice(save, petUids)
    local inventory = save and save.Inventory
    if _typeof(inventory) ~= "table" or not FuseApi.CalculateFusePrice then return nil end
    local itemData = {}
    ...
    local ok2, price = pcall(FuseApi.CalculateFusePrice, itemData)
    return ok2 and tonumber(price) or nil
end
function H.getFuseMachinePosition()
    ...
    local ok2, pivot = pcall(function() return fuseMachine:GetPivot() end)
    if not ok2 or not pivot then return nil end
...
function H.runAutoEquipBestTrail()
    ...
    if _typeof(ownedTrails) ~= "table" then return false end
    ...
    local wornTrailId = _typeof(wornSnapshot) == "table" and wornSnapshot[tostring(LocalPlayer.UserId)] or nil
...
function H.runAutoUpgrades()
    ...
    if selected.Base and BaseUpgradeModule and _typeof(BaseUpgradeModule.IsNextTierAffordable) == "function" then
    ...
    if selected.Treadmill and TreadmillsData and _typeof(TreadmillsData.GetByUpgradeLevel) == "function" then
...
function H.runClaimOfflineEarnings()
    ...
    if _typeof(summary) ~= "table" then return false end
...
function H.isDoubleSpeedVisible()
    local ok2, visible = pcall(function()
...
function H.resolveWaypoint(name)
    if _typeof(name) ~= "string" then return nil end
...
function H.collectGuardEsp()
    ...
    for _, guardArea in ipairs(GuardAreasFolder:GetChildren()) do
        ...
        local ok2, pivot = pcall(function() return guard and guard:GetPivot() or nil end)
        if ok2 and pivot and H.withinEspRange(pivot.Position) then
...
function H.collectPetEsp()
    ...
    pcall(function()
        if AssetRosterApi.GetRuntimeSnapshot then
            ...
            for _, group in pairs(snapshot) do
                if _typeof(group) == "table" and _typeof(group.Records) == "table" then
    ...
    for _, renderedPet in ipairs(renderedAssets:GetChildren()) do
        ...
        local ok2, pivot = pcall(function() return renderedPet:GetPivot() end)
        if _typeof(uid) == "string" and ok2 and pivot and H.withinEspRange(pivot.Position) then
            local category, moneyPerSecond = nil, nil
            local savedPet = inventory[uid]
            if _typeof(savedPet) == "table" then category = savedPet.Category end
            local runtimeRecord = runtimeByUid[uid]
            if _typeof(runtimeRecord) == "table" then
...
function H.collectMachineEsp()
    ...
    for _, machine in ipairs(machines:GetChildren()) do
        local ok2, pivot = pcall(function() return machine:GetPivot() end)
        if ok2 and pivot and H.withinEspRange(pivot.Position) then
...
function H.rememberVisited(serverId)
    if _typeof(serverId) ~= "string" or serverId == "" then return end
...
function H.fetchServerPage(cursor)
    ...
    if not ok2 or _typeof(body) ~= "string" then return nil end
    local decodedOk, page = pcall(function() return HttpService:JSONDecode(body) end)
    if not decodedOk or _typeof(page) ~= "table" or _typeof(page.data) ~= "table" then return nil end
    return page
end
function H.pickHopTargets()
    ...
    for _ = 1, 4 do
        ...
        for _, server in ipairs(page.data) do
            if _typeof(server) == "table" and _typeof(server.id) == "string"
        ...
        cursor = _typeof(page.nextPageCursor) == "string" and page.nextPageCursor or nil
...
function H.serverHop(reason)
    ...
    if _typeof(targets) ~= "table" or #targets == 0 then
...
function H.rejoinServer()
    local ok2 = pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayer) end)
    if not ok2 then pcall(function() TeleportService:Teleport(game.PlaceId, LocalPlayer) end) end
end
function H.hopToLowPop(maxPlayers)
    ...
    if req then
        ...
        if s and r and r.Body then raw = r.Body end
    else
        pcall(function() raw = game:HttpGet(url) end)
    end
    if not raw then return false end
    local ok2, data = pcall(function() return HttpService:JSONDecode(raw) end)
    if not ok2 or not data or not data.data then return false end
    local candidates = {}
    for _, s in ipairs(data.data) do
        if s.id and s.id ~= game.JobId and s.playing and s.maxPlayers then
            if s.playing > 0 and s.playing <= maxPlayers then
...
function H.httpPost(payload)
    ...
    if _typeof(requestFn) ~= "function" then return false end
...
CarryStartedCallback = function(eventData)
    if _typeof(eventData) ~= "table" then return end
    local record = _typeof(eventData.Uid) == "string" and H.findAreaEggRecord(eventData.Uid) or nil
    ...
    if _typeof(areaId) == "string" then table.insert(parts, areaId) end
    if record then
...
function H.applyFpsCap(value)
    local setCap = _setfpscap or (syn and syn.set_fps_cap)
    if _typeof(setCap) ~= "function" then return false end
    return pcall(setCap, _clamp(tonumber(value) or 60, 15, 360))
end
...
local ExtrasState = {
    ...
    SpeedChanger = false,
    SpeedValue = 60,
    InstantHoldEnabled = false,
}

track(RunService.Stepped:Connect(function()
    if not running or not ExtrasState.AntiRagdoll then return end
    local hum = H.getHumanoid(); local root = H.getRoot()
...
track(RunService.RenderStepped:Connect(function()
    if not running or not ExtrasState.LookRocket then return end
    local root = H.getRoot(); if not root then return end
    ...
    if horizontal.Magnitude > 15 and oldHorizontal.Magnitude < 10 and os.clock() - lastRocket > 0.08 then
        ...
        if cam then
            local lookDirection = cam.CFrame.LookVector.Unit
            local speed = horizontal.Magnitude * 1.8
            root.AssemblyLinearVelocity = lookDirection * speed
            lastRocket = os.clock()
        end
    end
...
local function applyInstantHold(prompt)
    if not ExtrasState.InstantHoldEnabled or not prompt:IsA("ProximityPrompt") then return end
...
track(Workspace.DescendantAdded:Connect(function(d)
    if ExtrasState.InstantHoldEnabled and d:IsA("ProximityPrompt") then task.defer(applyInstantHold, d) end
end))
function ExtrasState.SetInstantHold(on)
    ExtrasState.InstantHoldEnabled = on
    if on then
...
local function clayAndClean(obj)
    if not ExtrasState.FixLagEnabled then return end
...
track(Workspace.DescendantAdded:Connect(function(d)
    if ExtrasState.FixLagEnabled then task.defer(clayAndClean, d) end
end))
function ExtrasState.EnableFixLag()
    ExtrasState.FixLagEnabled = true
    pcall(function()
        Lighting.FogEnd = 1e6
...
local function neutralizeTrap(obj)
    if not ExtrasState.IsTrapImmune or not isTrap(obj) then return end
...
track(Workspace.DescendantAdded:Connect(function(d)
    if ExtrasState.IsTrapImmune then task.defer(neutralizeTrap, d) end
end))
function ExtrasState.EnableAntiTrap()
    ExtrasState.IsTrapImmune = true
    local ch = LocalPlayer.Character
...
function ExtrasState.ApplySpeedBypass(targetSpeed)
    ...
    if newHum then
        ...
        newHum.WalkSpeed = tonumber(targetSpeed) or 300
        pcall(function()
...
track(RunService.Stepped:Connect(function()
    if not running or not ExtrasState.SpeedChanger then return end
    local hum = H.getHumanoid()
    if hum and hum.WalkSpeed ~= ExtrasState.SpeedValue then
        pcall(function() hum.WalkSpeed = ExtrasState.SpeedValue end)
    end
end))

-- ============================================================
-- SPY EGG ESP + VISUAL PETS
-- ============================================================
...
local function getRecords()
    if not EggApi.GetAreaEggSnapshot then return {} end
    local ok2, snap = pcall(function() return EggApi.GetAreaEggSnapshot() end)
    if not ok2 or _typeof(snap) ~= "table" then return {} end
    return snap.Records or {}
end

local function getModelByUid(uid)
...
local function assetDir(cat)
    if not AssetsData or _typeof(AssetsData.Directory) ~= "table" then return nil end
    return AssetsData.Directory[cat]
end
...
local function spyEggContents()
    ...
    for _, rec in ipairs(getRecords()) do
        ...
        if owner and owner ~= "" then
            local pl = Players:GetPlayerByUserId(tonumber(owner))
            local nm = pl and pl.Name or tostring(owner)
            local isMe = tostring(owner) == tostring(LocalPlayer.UserId)
...
local function stopSpyEsp()
    petEspRunning = false
    if petEspThread then task.cancel(petEspThread); petEspThread = nil end
    for uid, data in pairs(petEspSpy) do
        pcall(function() data:Destroy() end)
        petEspSpy[uid] = nil
    end
...
VisualPetsFolder.Name = "ZenHubXVisualPets"
VisualPetsFolder.Parent = Workspace
...
local function scanPetNames()
    ...
    for _, d in ipairs(Workspace:GetDescendants()) do
        if d:IsA("Model") and d:FindFirstChildWhichIsA("BasePart") then
            local n = d.Name
            if not string.find(string.lower(n), "egg") and not string.find(string.lower(n), "humanoid") and not seen[n] then
                seen[n] = true
...
local function findPetModel(name)
    ...
    for _, d in ipairs(Workspace:GetDescendants()) do
        if d:IsA("Model") and d:FindFirstChildWhichIsA("BasePart")
            and string.find(string.lower(d.Name), search, 1, true)
            and not d:IsDescendantOf(VisualPetsFolder) then
            return d
        end
    end
    return nil
...
local function spawnVisualPet(name)
    ...
    clone.Name = "VP_" .. name
    clone.Parent = VisualPetsFolder
    VisualPets.spawned[#VisualPets.spawned + 1] = { model = clone }
    return true
end
...
VisualPets.conn = RunService.Heartbeat:Connect(function()
    if not running or #VisualPets.spawned == 0 then return end
    local root = H.getRoot(); if not root then return end
    local t = tick()
    local n = #VisualPets.spawned
    local center = root.Position + Vector3.new(0, 3, 0)
    for i, item in ipairs(VisualPets.spawned) do
        local angle = (i - 1) * (6.283185307179586 / n) + t * VisualPets.orbitSpeed
        local x = math.cos(angle) * VisualPets.orbitRadius
        local z = math.sin(angle) * VisualPets.orbitRadius
        local pos = center + Vector3.new(x, 0, z)
        pcall(function()
            item.model:PivotTo(CFrame.new(pos) * CFrame.Angles(0, angle, 0))
        end)
    end
end)
...
local function getGuiParent()
    if _typeof(_gethui) == "function" then
        local ok2, parent = pcall(_gethui)
        if ok2 and parent then return parent end
    end
    return CoreGui
end
...
-- Nút avatar (asset 72639013832251)
...
subtitle.Size = UDim2.new(0, 340, 0, 14)
...
discordBtn.Size = UDim2.fromOffset(76, 24)
discordBtn.Position = UDim2.new(1, -144, 0, 11)
...
discordBtn.MouseButton1Click:Connect(function()
    ...
    H.notify("ZenHubX", "Discord link copied", "Success", 3)
end)
...
function H.multiSelected(id)
    ...
    if _typeof(value) ~= "table" then
        if _typeof(value) == "string" and value ~= "" then selected[value] = true end
        return selected
    end
    for key, item in pairs(value) do
        if item == true then selected[key] = true
        elseif _typeof(key) == "number" and _typeof(item) == "string" then selected[item] = true end
    end
    return selected
end
...
function H.matchesEggFilters(egg, areaOptionName, rarityOptionName, mutationOptionName)
    if areaOptionName then
        ...
        if _typeof(areaId) ~= "string" or not H.selectionAllows(areaOptionName, areaId) then return false end
    end
    local rarity = H.resolveRarity(egg.AssetCategory)
    if _typeof(rarity) ~= "string" or not H.selectionAllows(rarityOptionName, rarity) then return false end
...
function UI.AddTab(opts)
    ...
    local pagePad = Instance.new("UIPadding"); pagePad.PaddingRight = UDim.new(0, 4); pagePad.Parent = page
...
function UI.AddDropdown(parent, opts)
    ...
    valueLabel.Size = UDim2.fromOffset(150, 32)
    ...
    arrowBtn.Position = UDim2.new(1, -28, 0.5, -11)
    ...
    if isMulti then
        selected = {}
        if _typeof(opts.Default) == "table" then
...
function UI.AddButton(parent, opts)
    ...
    action.Size = UDim2.fromOffset(60, 22)
    action.Position = UDim2.new(1, -60, 0.5, -11)
...
function UI.AddInput(parent, opts)
    ...
    box.Size = UDim2.new(0.58, -8, 0, 24)
...
NotifyHolder.Size = UDim2.fromOffset(240, 400)
...
function H.notify(title, content, variant, duration)
    ...
    note.Size = UDim2.fromOffset(220, 46)
    ...
    c.Position = UDim2.fromOffset(10, 21)
...
do
    local HomeTab = UI.AddTab({ Id = "home", Title = L("tabHome") })
    ...
    refs.statusRow = UI.AddStatus(SessionSec, { Title = "Automation", Value = "Ready" })
    refs.statusRow:SetStatus("Success")
    refs.stolenRow = UI.AddStatus(SessionSec, { Title = "Stolen Eggs", Value = "0" })
    refs.carryingRow = UI.AddStatus(SessionSec, { Title = "Carrying Egg", Value = "No" })
    refs.runtimeRow = UI.AddStatus(SessionSec, { Title = "Runtime", Value = "0m" })
    UI.AddStatus(SessionSec, { Title = "Server", Value = DisplayJobId })

    refs.inventoryProgress = UI.AddStatus(AccountSec, { Title = "Egg Inventory", Value = tostring(H.eggInventoryCount()) })
    refs.moneyRow = UI.AddStatus(AccountSec, { Title = "Money", Value = "0" })
    refs.speedRow = UI.AddStatus(AccountSec, { Title = "Speed Power", Value = "0" })
    refs.rebirthRow = UI.AddStatus(AccountSec, { Title = "Rebirths", Value = "0" })
    refs.petsOwnedRow = UI.AddStatus(AccountSec, { Title = "Pets Owned", Value = "0" })

    UI.AddButton(QuickSec, { Title = "Return to Base", Text = "Return", Callback = function()
        task.spawn(function()
    ...
    local FarmTab = UI.AddTab({ Id = "farm", Title = L("tabFarm") })
    local StealSec = UI.AddSection(FarmTab, { Title = "Steal Eggs" })
    ...
    UI.AddDropdown(StealSec, { Id = "StealZones", Title = "Areas", Options = AreaNames, Multi = true, Default = {} })
    ...
    UI.AddDropdown(StealSec, { Id = "StealPriority", Title = "Priority", Options = STEAL_PRIORITIES, Default = "Rarest" })
    ...
    UI.AddSlider(LifeSec, { Id = "SellEggInterval", Title = "Sell Interval", Min = 1, Max = 120, Default = 8, Step = 1, Suffix = " s" })
    ...
    UI.AddButton(ServerSec, { Title = "Hop Now", Text = "Hop", Callback = function()
        task.spawn(function() ServerHopRetryAfter = 0; H.serverHop("Manual") end)
    end })
    UI.AddButton(ServerSec, { Title = "Hop Low Pop (1-5)", Text = "Low", Callback = function()
        task.spawn(function() H.hopToLowPop(5) end)
    end })

    local PetsTab = UI.AddTab({ Id = "pets", Title = L("tabPets") })
    ...
    local VisualPetsSec = UI.AddSection(PetsTab, { Title = "Visual Pets" })

    UI.AddToggle(PetsOverview, { Id = "AutoEquipBest", Title = "Auto Equip Best Pets", Default = false })
    UI.AddToggle(PetsOverview, { Id = "AutoDeleteOwnPets", Title = "Hide Own Pet Renders", Default = false })
    ...
    UI.AddButton(FuseSec, { Title = "Fuse Now", Text = "Fuse", Callback = function() task.spawn(function() H.runAutoFusePets(true) end) end })
    ...
    UI.AddDropdown(VisualPetsSec, { Id = "VisualPetName", Title = "Pet Name", Options = { "None" }, Default = "None",
        Callback = function(v) VisualPets.inputName = tostring(v) end })
    UI.AddButton(VisualPetsSec, { Title = "Refresh Pet List", Text = "Scan", Callback = function()
        local names = scanPetNames()
        if #names > 0 then
            H.notify("Visual Pets", #names .. " pets found", "Success", 3)
        end
    end })
    UI.AddButton(VisualPetsSec, { Title = "Spawn Selected Pet", Text = "Spawn", Callback = function()
        if VisualPets.inputName and VisualPets.inputName ~= "None" and VisualPets.inputName ~= "" then
            task.spawn(function()
                if spawnVisualPet(VisualPets.inputName) then
                    H.notify("Visual Pets", "Spawned " .. VisualPets.inputName, "Success", 3)
                else
                    H.notify("Visual Pets", "Not found", "Error", 3)
                end
            end)
        end
    end })
    UI.AddSlider(VisualPetsSec, { Id = "VisualPetRadius", Title = "Orbit Radius", Min = 2, Max = 20, Default = 4, Step = 1,
        Callback = function(v) VisualPets.orbitRadius = v end })
    UI.AddButton(VisualPetsSec, { Title = "Remove All Visual Pets", Text = "Clear", Callback = function()
        clearVisualPets()
        H.notify("Visual Pets", "Cleared", "Info", 3)
    end })

    local ProgressTab = UI.AddTab({ Id = "progress", Title = L("tabProgress") })
    local UpgradesSec = UI.AddSection(ProgressTab, { Title = "Upgrades" })
    local RewardsSec = UI.AddSection(ProgressTab, { Title = "Rewards" })
    local EquipmentSec = UI.AddSection(ProgressTab, { Title = "Equipment" })
    ...
    UI.AddToggle(EquipmentSec, { Id = "AutoBuyTrail", Title = "Auto Buy Trail", Default = false })
    UI.AddDropdown(EquipmentSec, { Id = "TrailWanted", Title = "Trails", Options = TrailNames, Multi = true, Default = {} })
    UI.AddToggle(EquipmentSec, { Id = "AutoEquipBestTrail", Title = "Auto Equip Best Trail", Default = false })
    UI.AddToggle(EquipmentSec, { Id = "AutoEquipBestGear", Title = "Auto Equip Best Gear", Default = false })
    UI.AddToggle(TrainingSec, { Id = "AutoTreadmill", Title = "Auto Treadmill Training", Default = false })

    local PlayerTab = UI.AddTab({ Id = "player", Title = L("tabPlayer") })
    ...
    UI.AddToggle(EspSec, { Id = "EspCarriedEggs", Title = "Carried Egg ESP", Default = false })
    ...
    UI.AddSlider(EspSec, { Id = "EspDistance", Title = "Render Distance", Min = 100, Max = 6000, Default = 2000, Step = 50, Suffix = " s" })
    UI.AddToggle(MoveSec, { Id = "WalkSpeedEnabled", Title = "Walk Speed Override", Default = false })
    UI.AddSlider(MoveSec, { Id = "WalkSpeed", Title = "Walk Speed", Min = 16, Max = 500, Default = 32, Step = 1 })
    UI.AddToggle(MoveSec, { Id = "JumpPowerEnabled", Title = "Jump Power Override", Default = false })
    UI.AddSlider(MoveSec, { Id = "JumpPower", Title = "Jump Power", Min = 10, Max = 500, Default = 50, Step = 1 })
    UI.AddToggle(MoveSec, { Id = "InfJump", Title = "Infinite Jump", Default = false })
    UI.AddToggle(MoveSec, { Id = "NoClip", Title = "NoClip", Default = false })
    ...
    UI.AddSlider(MoveSec, { Id = "FlySpeed", Title = "Fly Speed", Min = 10, Max = 400, Default = 60, Step = 1 })
    ...
    local ExtrasTab = UI.AddTab({ Id = "extras", Title = L("tabExtras") })
    local CharSec = UI.AddSection(ExtrasTab, { Title = "Character" })
    local WorldSec = UI.AddSection(ExtrasTab, { Title = "World" })
    local SpeedSec = UI.AddSection(ExtrasTab, { Title = "Speed / Anti-Cheat" })
    local EggEspSec = UI.AddSection(ExtrasTab, { Title = "Egg ESP Advanced" })

    UI.AddToggle(CharSec, { Id = "AntiRagdoll", Title = "Anti-Ragdoll", Default = false,
        Callback = function(v) ExtrasState.AntiRagdoll = v end })
    UI.AddToggle(CharSec, { Id = "LookRocket", Title = "LookRocket (Tele Guiado)", Default = false,
        Callback = function(v) ExtrasState.LookRocket = v end })
    UI.AddToggle(CharSec, { Id = "InstantInteract", Title = "Instant Interact", Default = false,
        Callback = function(v) ExtrasState.SetInstantHold(v) end })

    UI.AddToggle(WorldSec, { Id = "FixLag", Title = "Fix Lag (Clay)", Default = false,
        Callback = function(v)
            if v then ExtrasState.EnableFixLag()
            else ExtrasState.FixLagEnabled = false end
        end })
    UI.AddToggle(WorldSec, { Id = "AntiTrap", Title = "Anti Trap (Delete)", Default = false,
        Callback = function(v)
            if v then ExtrasState.EnableAntiTrap()
            else ExtrasState.IsTrapImmune = false end
        end })
    UI.AddButton(WorldSec, { Title = "Void Reset (Drop to Void)", Text = "Void",
        Callback = function()
            task.spawn(function()
                if ExtrasState.VoidReset() then
                    H.notify("Extras", "Dropping to void...", "Info", 3)
                else
                    H.notify("Extras", "No character", "Error", 3)
                end
            end)
        end })

    UI.AddToggle(SpeedSec, { Id = "SpeedChanger", Title = "Speed Changer (WalkSpeed)", Default = false,
        Callback = function(v) ExtrasState.SpeedChanger = v end })
    UI.AddSlider(SpeedSec, { Id = "SpeedValue", Title = "Speed Value", Min = 16, Max = 500, Default = 60, Step = 1,
        Callback = function(v) ExtrasState.SpeedValue = v end })
    UI.AddSlider(SpeedSec, { Id = "BypassSpeed", Title = "Speed Bypass Value", Min = 100, Max = 900, Default = 300, Step = 10 })
    UI.AddButton(SpeedSec, { Title = "Apply Bypass (Clone Humanoid)", Text = "Apply",
        Callback = function()
            task.spawn(function()
                local sp = tonumber(H.optionValue("BypassSpeed", 300)) or 300
                local ok2 = ExtrasState.ApplySpeedBypass(sp)
                H.notify("Speed", ok2 and ("Bypass " .. sp .. " applied") or "Failed",
                    ok2 and "Success" or "Error", 3)
            end)
        end })

    UI.AddToggle(EggEspSec, { Id = "SpyEggEsp", Title = "Spy Egg ESP (nội dung trứng)", Default = false,
        Callback = function(v)
            if v then
                if not EggStateModule then
                    H.notify("Spy Egg", "EggState missing", "Error", 3); return
                end
                startSpyEsp()
                H.notify("Spy Egg", "Enabled", "Success", 3)
            else
                stopSpyEsp()
                H.notify("Spy Egg", "Disabled", "Info", 3)
            end
        end })

    local SysTab = UI.AddTab({ Id = "system", Title = L("tabSystem") })
    ...
    local SettingsSec = UI.AddSection(SysTab, { Title = "Settings" })
    local SocialSec = UI.AddSection(SysTab, { Title = "Social" })
    ...
    UI.AddDropdown(SettingsSec, { Id = "Language", Title = L("language"),
        Options = { L("langVN"), L("langEN") }, Default = L("langVN"),
        Callback = function(v)
            LANG.Current = (v == "English") and "en" or "vi"
            H.notify("Language", (LANG.Current == "vi") and "Đã chuyển sang Tiếng Việt" or "Switched to English", "Success", 3)
        end })

    UI.AddButton(SocialSec, { Title = "Discord", Text = L("copy"),
        Callback = function()
            pcall(function() setclipboard(DISCORD_LINK) end)
            H.notify("Discord", "Copied: " .. DISCORD_LINK, "Success", 3)
        end })
    UI.AddButton(SocialSec, { Title = "TikTok ID", Text = L("copy"),
        Callback = function()
            pcall(function() setclipboard(TIKTOK_ID) end)
            H.notify("TikTok", "Copied: " .. TIKTOK_ID, "Success", 3)
        end })

    UI.AddParagraph(AboutSec, { Title = "Script Dev", Content = "ZenHubX" })
...
function H.unload()
    ...
    for _, c in ipairs(conns) do
        pcall(function() if _typeof(c) == "RBXScriptConnection" then c:Disconnect() end end)
    end
    H.clearTable(conns)
...
track(UserInputService.InputBegan:Connect(function() LastInputAt = tick() end))
...
local function runPriorityJobs()
    ...
    for _, jobName in ipairs(order) do
        ...
        if ready then
            ...
            AutomationBusy = true
            local ok2, didWork = pcall(job.Run)
            AutomationBusy = false
            if ok2 and didWork then return end
        end
    end
end
...
-- END OF MAIN
    ...
    warn("[ZenHubX] Error: " .. tostring(err))
    pcall(function()
...
if not ok then
    warn("[ZenHubX] Failed to load: " .. tostring(err))
end