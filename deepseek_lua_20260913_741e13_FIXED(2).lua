...
-- ============================================================
-- TASK LIBRARY SHIM (Fix "attempt to call a nil value")
-- ============================================================
if not task then
    task = {}
    ...
    function task.delay(duration, fn)
        task.spawn(function()
            task.wait(tonumber(duration) or 0)
...
elseif type(task.wait) ~= "function" then
    local originalTask = task
    task = originalTask
    local oldWait = task.wait
    function task.wait(duration)
        if oldWait then return oldWait(tonumber(duration) or 0) end
    end
end
...
toggleGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(185, 110, 255)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(120, 40, 190)),
})
