--[[
    ...
    ANTI-CHEAT BYPASS: AUTO-ACTIVATED ON LOAD
--]]
...
-- ============================================================
-- ANTI-CHEAT BYPASS (AUTO-ACTIVATED)
-- ============================================================
local BypassState = {
    Enabled = true,          -- AUTO-ON
    AutoActivated = true,    -- Đánh dấu đã auto kích hoạt
    KicksBlocked = 0,
    KickReasons = {},
    BlockKickRemote = true,
    ProtectCharacter = true,
    LastReapplyAt = 0,
}

local AC_KEYWORDS = {
    "AntiCheat", "Anti_Cheat", "AntiCheatKick", "AC_", "_AC",
    "Kick", "Ban", "Banned", "Exploit", "Detect", "Detection",
    "CheatDetection", "SecurityCheck", "Violation", "Suspicious",
    "AntiExploit", "AntiTeleport", "AntiSpeed", "AntiFly",
    "Watchdog", "Guardian", "Sentinel", "Protect",
}
...
-- Bộ đếm để tránh hook chồng nhiều lần
...
local function hookNamecall()
    pcall(function()
        ...
        -- Tránh double hook
        if getgenv and getgenv().__ZenHubXNamecallHooked then
            if rawget(mt, "__namecall") == getgenv().__ZenHubXNamecallHooked then return end
        end
        setreadonly(mt, false)
        local newFn = newcclosure(function(self, ...)
            local method = getnamecallmethod and getnamecallmethod() or nil
            if BypassState.Enabled then
                if method == "Kick" and self == LocalPlayer then
                    BypassState.KicksBlocked = BypassState.KicksBlocked + 1
                    table.insert(BypassState.KickReasons, tostring(select(1, ...)))
                    return
                end
                if (method == "Kick" or method == "Ban") and typeof(self) == "Instance" then
                    local n = string.lower(self.Name or "")
                    if isAntiCheatName(n) then
                        BypassState.KicksBlocked = BypassState.KicksBlocked + 1
                        return
                    end
                end
                if method == "Destroy" and typeof(self) == "Instance" then
                    if BypassState.ProtectCharacter then
                        if self == LocalPlayer.Character or self == LocalPlayer then return end
                        if self:IsDescendantOf(LocalPlayer.Character) then return end
                    end
                end
                if (method == "FireServer" or method == "InvokeServer") and BypassState.BlockKickRemote then
                    if typeof(self) == "Instance" and isAntiCheatName(self.Name) then
                        BypassState.KicksBlocked = BypassState.KicksBlocked + 1
                        return
                    end
                end
        ...
        mt.__namecall = newFn
        if getgenv then getgenv().__ZenHubXNamecallHooked = newFn end
        setreadonly(mt, true)
        HookInstalled = true
    end)
end

local function neutralizeAntiCheatRemotes()
    pcall(function()
        for _, d in ipairs(ReplicatedStorage:GetDescendants()) do
            if (d:IsA("RemoteEvent") or d:IsA("RemoteFunction")) and isAntiCheatName(d.Name) then
                pcall(function()
                    if d:IsA("RemoteEvent") then
                        d.OnClientEvent:Connect(function() end)
                    end
                end)
            end
        end
...
local function overrideKickDirect()
    pcall(function()
        if LocalPlayer then
...
local function filterAntiCheatNotifications()
    pcall(function()
        if StarterGui and StarterGui.SetCore then
            local oldSetCore = StarterGui.SetCore
            StarterGui.SetCore = newcclosure(function(self, key, ...)
                if BypassState.Enabled and key == "SendNotification" then
                    local args = { ... }
                    local title = args[1] and args[1].Title or ""
                    if isAntiCheatName(tostring(title)) then return end
                end
...
-- HÀM KÍCH HOẠT TOÀN BỘ BYPASS
local function activateAllBypass()
    BypassState.Enabled = true
    hookNamecall()
    neutralizeAntiCheatRemotes()
    overrideKickDirect()
    filterAntiCheatNotifications()
    pcall(function()
        if print then
            print("[ZenHubX] Anti-Cheat Bypass: ACTIVE (auto)")
        end
    end)
end

-- CHẠY NGAY — tự động bật khi script load
activateAllBypass()
...
function H.bypassMoveTo(position, keepGoing, speed)
    ...
    bv.Name = "ZenHubXBypassMove"; bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
    bv.P = 1250; bv.Velocity = Vector3.zero; bv.Parent = root
    local bg = Instance.new("BodyGyro")
    bg.Name = "ZenHubXBypassGyro"; bg.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
...
do
    ...
    local BypassSec = UI.AddSection(HomeTab, { Title = "Protection", Description = "Anti-cheat bypass (auto-enabled)" })
    ...
    -- Protection (auto-activated)
    refs.antiCheatStatus = UI.AddStatus(BypassSec, { Title = "Anti-Cheat Bypass", Value = "ACTIVE" })
    refs.antiCheatStatus:SetStatus("Success")
    refs.kicksBlocked = UI.AddStatus(BypassSec, { Title = "Kicks Blocked", Value = "0" })
    UI.AddToggle(BypassSec, { Id = "AntiCheatBypass", Title = "Anti-Cheat Bypass", Description = "Auto-enabled on load", Default = true,
        Callback = function(v)
            BypassState.Enabled = v
            if v then pcall(activateAllBypass) end
        end })
    UI.AddToggle(BypassSec, { Id = "BlockKickRemote", Title = "Block Kick Remotes", Default = true,
        Callback = function(v) BypassState.BlockKickRemote = v end })
    UI.AddToggle(BypassSec, { Id = "ProtectCharacter", Title = "Protect Character from Destroy", Default = true,
        Callback = function(v) BypassState.ProtectCharacter = v end })
    UI.AddButton(BypassSec, { Title = "Re-apply Bypass Hooks", Text = "Apply", Callback = function()
        pcall(activateAllBypass)
        H.notify("Bypass", "Hooks re-applied", "Success", 3)
    end })
    ...
    UI.AddButton(AboutSec, { Title = "Copy Discord Link", Text = "Copy", Callback = function()
        pcall(function() setclipboard(DISCORD_LINK) end)
...
local function refreshHomeDashboard()
    ...
    pcall(function()
        ...
        if refs.kicksBlocked then refs.kicksBlocked:SetValue(tostring(BypassState.KicksBlocked)) end
        if refs.antiCheatStatus then
            refs.antiCheatStatus:SetValue(BypassState.Enabled and "ACTIVE" or "Disabled")
            refs.antiCheatStatus:SetStatus(BypassState.Enabled and "Success" or "Warning")
        end
        local save = H.getSave()
        if save then
...
-- ============================================================
-- WATCHDOG: TỰ RE-APPLY BYPASS + BẢO VỆ
-- ============================================================
task.spawn(function()
    while running and BypassState.Enabled do
        task.wait(5)
        pcall(function()
            -- Re-hook metatable nếu bị gỡ
            local mt = getrawmetatable and getrawmetatable(game)
            if mt then
                local nc = rawget(mt, "__namecall")
                local tag = getgenv and getgenv().__ZenHubXNamecallHooked
                if not nc or (tag and nc ~= tag) then
                    hookNamecall()
                end
            end
            -- Đảm bảo Kick bị override
            if LocalPlayer and type(LocalPlayer.Kick) == "function" then
                local ok, err = pcall(function()
                    local test = tostring(LocalPlayer.Kick)
                    if not test:find("C function") and not test:find("function:") then
                        -- nghi ngờ hook bị đổi
                    end
                end)
            end
            if LocalPlayer then LocalPlayer.Kick = function() end end
            -- Bảo vệ Character hiện tại
            if BypassState.ProtectCharacter and LocalPlayer.Character then
                for _, part in ipairs(LocalPlayer.Character:GetDescendants()) do
                    if part:IsA("BasePart") and part.Name == "HumanoidRootPart" then
                        part:SetAttribute("ZenHubXProtected", true)
                    end
                end
            end
            BypassState.LastReapplyAt = os.clock()
        end)
    end
end)

-- Watchdog: nếu LocalPlayer sắp bị remove khỏi game thì cố gắng block
pcall(function()
    H.track(LocalPlayer.AncestryChanged:Connect(function(_, parent)
        if not running then return end
        if not BypassState.Enabled then return end
        if parent == nil then
            -- Player bị đẩy ra khỏi DataModel — không thể block hoàn toàn,
            -- nhưng log lại
            BypassState.KicksBlocked = BypassState.KicksBlocked + 1
            table.insert(BypassState.KickReasons, "AncestryChanged to nil")
            if H.notify then pcall(H.notify, "ZenHubX", "Blocked removal attempt", "Warning", 3) end
        end
    end))
end)

-- Re-protect on CharacterAdded
H.track(LocalPlayer.CharacterAdded:Connect(function(char)
    if not BypassState.Enabled then return end
    task.delay(0.5, function()
        pcall(function()
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if hrp then hrp:SetAttribute("ZenHubXProtected", true) end
        end)
    end)
end))
...
H.track(RunService.Heartbeat:Connect(function()
    ...
    if schedulerDue("dashboard", 2) then
        ...
        if H.isOn("WalkSpeedEnabled") then
            local humanoid = H.getHumanoid()
            if humanoid then humanoid.WalkSpeed = tonumber(H.optionValue("WalkSpeed", 32)) or 32 end
        end
...
H.notify("ZenHubX", "Loaded - Anti-Cheat Bypass ACTIVE", "Success", 5)
