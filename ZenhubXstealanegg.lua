--[[
    ...
    [MOD] Forest warm-up (BAY, fallback TELE) → RAGDOLL → TELE NGAY
    [MOD2] Anti-Knock CỰC MẠNH — không bay xa, không nẩy cao
    [MOD3] Đảm bảo BƯỚC 0 LUÔN chạy, có fallback khi thiếu trứng Forest
--]]
...
-- ============================================================
-- [MOD2] ANTI-KNOCK CỰC MẠNH
-- - Giới hạn velocity cực chặt
-- - Ép về vị trí anchor ngay khi drift quá nhỏ
-- - Khoá trục Y để không nẩy cao
-- - Dùng Heartbeat (post-physics) để clamp chính xác hơn Stepped
-- ============================================================
local AntiKnock = {
    Enabled = false,
    MaxHorizontal = 6,     -- giới hạn ngang cực mạnh (studs/s)
    MaxVertical   = 1.5,   -- gần như khoá trục Y
    DriftLimit    = 2.5,   -- drift tối đa trước khi kéo về anchor
    AnchorCF      = nil,
    AnchorSetAt   = 0,
    ResetAfter    = 5.0,
    LastClampAt   = 0,
...
-- Dùng Heartbeat (post-physics) để clamp mạnh sau khi guard knock
H.track(RunService.Heartbeat:Connect(function()
    ...
    -- 4) Anchor lock — ép vị trí về anchor mỗi khi drift vượt ngưỡng
    if AntiKnock.AnchorCF then
        ...
        if age > AntiKnock.ResetAfter then
            ...
            local verticalDist = math.abs(dy)
            -- Nếu drift vượt ngưỡng theo XZ hoặc Y quá cao/thấp → ép ngay về anchor
            if horizontalDist > AntiKnock.DriftLimit or verticalDist > AntiKnock.DriftLimit then
...
-- ============================================================
-- [MOD] stealEgg:
--   B0: LUÔN CHẠY — BAY tới Forest → nhặt trứng → RAGDOLL → TELE NGAY target
...
function H.stealEgg(slotEgg)
    ...
    -- Đảm bảo không đang mang trứng khi vào step 0
    ...
    -- ========================================================
    -- BƯỚC 0: FOREST WARM-UP — LUÔN CHẠY
    -- ========================================================
    do
        -- 1) Chọn trứng Forest
        local forestRecord = H.pickRandomForestEggRecord()
        local forestSlot, forestPos = nil, nil
        if forestRecord then
            forestSlot = H.findEggSlotInstance(forestRecord.Uid)
            if forestSlot then
                forestPos = H.getSlotEggPosition(forestSlot)
            end
        end

        -- 2) Fallback: nếu không có trứng Forest → dùng tâm Forest
        if not forestPos then
            forestPos = H.getZoneLaneCenter("Forest")
        end
        if not forestPos then
            forestPos = H.getEntryPosition()
        end

        -- 3) Nếu vẫn không có vị trí → bỏ qua step 0 an toàn (không chặn farm)
        if forestPos then
            AntiKnock.Enabled = false

            -- Bay tới (bypassMoveTo dùng BodyVelocity)
            local reached = H.bypassMoveTo(forestPos, H.stealingEnabled, BYPASS_SPEED)
            -- Fallback: nếu bay thất bại → tele
            if not reached then
                local r = H.getRoot()
                if r then
                    local ty = H.groundedY(forestPos.X, forestPos.Z, forestPos.Y)
                    H.placeRoot(r, CFrame.new(forestPos.X, ty, forestPos.Z))
                end
            end
            task.wait(0.15)

            -- Bật lại ragdoll cho guard đánh
            H.setRagdollStatesAllowed(true)

            -- Bật AntiKnock + neo vị trí tại Forest
            AntiKnock.Enabled = true
            root = H.getRoot()
            if root then AntiKnock.SetAnchor(root.Position) end

            -- Nhặt trứng Forest nếu có
            if forestSlot then
            ...
            -- Chờ ragdoll → TELE NGAY khi phát hiện
            ...
            -- Fallback: nếu timeout không có ragdoll → vẫn tele tới target
            ...
            -- Khoá ragdoll states lại để grab target an toàn
            H.setRagdollStatesAllowed(false)

            -- Ép đứng dậy nếu còn ragdoll
            local hum = H.getHumanoid()
            if hum then
                pcall(function()
    ...
    -- ========================================================
    -- BƯỚC 1: Đảm bảo đã ở target (tele phòng trường hợp chưa)
    -- ========================================================
    do
        root = H.getRoot()
        if root then
