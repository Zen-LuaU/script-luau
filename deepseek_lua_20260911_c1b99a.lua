--[[
    zen hubX steal an egg
    Farm Egg + Visual Pet + Spy ESP + Teleport Step + Anti-Cheat Metatable
]]
...
-- ============================================================
-- ANTI-CHEAT METATABLE LOCK (code người dùng cung cấp)
-- ============================================================
local function ApplyMetatableLock()
    if type(getgc) ~= "function" or type(getrawmetatable) ~= "function"
    or type(setmetatable) ~= "function" then return false end
    local locked = 0
    pcall(function()
...
local Window = WindUI:CreateWindow({
    ...
    Folder = "zenhubX_StealAnEgg",
    Icon = "solar:shield-bold",
    Theme = "Dark",
    NewElements = true,
    HideSearchBar = false,
    OpenButton = {
        Title = "Mở Menu zen hubX steal an egg",
...
Window:Tag({Title = "v14.0 Steal an Egg", Icon = "solar:star-bold", Color = PurpleColor, Border = true})
...
FarmTab:Space()
FarmTab:Button({Title = "Apply Metatable Lock", Desc = "Khóa metatable anti-cheat (dùng getgc)",
    Callback = function()
        local n = ApplyMetatableLock()
        WindUI:Notify({Title = "Anti-Cheat", Content = "Đã khóa "..tostring(n or 0).." bảng metatable."})
    end})
FarmTab:Space()
...
-- Auto apply metatable lock khi load
task.spawn(function()
    task.wait(1)
    pcall(ApplyMetatableLock)
end)

WindUI:Notify({Title = "zen hubX steal an egg", Content = "Đã load "..#AreaList.." khu vực! Mở tab Farm Egg.", Duration = 6})