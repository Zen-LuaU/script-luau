...
-- Hop to low-pop server (từ file steal egg zen hubX)
...
-- ============================================================
-- EXTRAS (từ file steal egg zen hubX.lua)
-- ============================================================
local ExtrasState = {
    AntiRagdoll = false,
    LookRocket = false,
    ...
    FixLag = false,
    AntiTrap = false,
    SpeedConfig = { Enabled = false, Value = 300 },
...
local function getRecords()
    if not EggApi.GetAreaEggSnapshot then return {} end
    local ok, snap = pcall(function() return EggApi.GetAreaEggSnapshot() end)
...
local function assetDir(cat)
    if not AssetsData or typeof(AssetsData.Directory) ~= "table" then return nil end
...
H.track(VisualPets.conn)

-- ============================================================
-- WINDUI INTERFACE - ZenHubX Neon Purple
-- ============================================================
-- WindUI latest loader (requested)
local WindUI = loadstring(game:HttpGet("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua"))()

-- Custom neon-purple theme
pcall(function()
    WindUI:AddTheme({
        Name = "ZenHubX Neon",
        Accent = Color3.fromHex("#B84CFF"),
        Background = Color3.fromHex("#09050F"),
        Outline = Color3.fromHex("#A83DFF"),
        Text = Color3.fromHex("#F7EFFF"),
        Placeholder = Color3.fromHex("#9B7CB8"),
        Button = Color3.fromHex("#35104D"),
        Icon = Color3.fromHex("#D28BFF"),
    })
end)
pcall(function() WindUI:SetTheme("ZenHubX Neon") end)
...
local COLORS = {
    accent   = Color3.fromHex("#B84CFF"),
    accentHi = Color3.fromHex("#E1A0FF"),
    success  = Color3.fromRGB(90, 220, 140),
    warning  = Color3.fromRGB(245, 180, 70),
    error    = Color3.fromRGB(245, 90, 100),
    info     = Color3.fromHex("#B57CFF"),
}

-- Keep the old unload code compatible with WindUI.
local Window = WindUI:CreateWindow({
    Title = "ZenHubX",
    Author = "Neon Purple Edition",
    ...
    OpenButton = {
        ...
        Color = ColorSequence.new({
            Color3.fromHex("#E6A6FF"),
            Color3.fromHex("#7A18FF"),
        }),
    },
    Topbar = {
        Height = 44,
...
pcall(function()
    Window:Tag({
        Title = "ZENHUBX",
        Icon = "sparkles",
        Color = Color3.fromHex("#8D2BFF"),
        Border = true,
    })
end)

-- Compatibility handles for the existing cleanup routine.
local screenGui = { Destroy = function() pcall(function() Window:Destroy() end) end }
local toggleGui = { Destroy = function() end }

local function conn(c)
    table.insert(UI.__connections, c)
    return c
end

function UI.GetState(id)
    return UI.__state[id]
end

function UI.SetState(id, value, fire)
    UI.__state[id] = value
    if fire and UI.__callbacks[id] then
        for _, cb in ipairs(UI.__callbacks[id]) do
            pcall(cb, value)
        end
    end
end
...
local function normalizeOptions(options)
    ...
    for _, v in ipairs(options or {}) do
        if type(v) == "table" then
            -- WindUI supports advanced dropdown entries, but the existing
            -- state layer expects strings, so keep the title as the value.
            table.insert(out, tostring(v.Title or v.Name or v.Value or "Option"))
        else
...
function UI.AddTab(opts)
    local tab = Window:Tab({
        Title = opts.Title or opts.Id or "Tab",
        Icon = opts.Icon or "solar:layers-bold",
        IconColor = COLORS.accentHi,
        IconShape = "Square",
        Border = true,
    })
    local wrapped = {
        ...
        SetActive = function(active)
            if active and tab.Select then pcall(function() tab:Select() end) end
        end,
    }
    UI.__tabs[opts.Id] = wrapped
    if not UI.__activeTab then UI.__activeTab = opts.Id end
    return wrapped
end

function UI.AddSection(tab, opts)
    local parent = tab.WindTab or tab.Page or tab
    local section = parent:Section({
        Title = opts.Title or "",
        Desc = opts.Description or opts.Desc,
        Box = true,
        BoxBorder = true,
        Opened = true,
    })
...
function UI.AddToggle(parent, opts)
    ...
    UI.__state[opts.Id] = default

    local element = parent:Toggle({
        Title = opts.Title or "Toggle",
        Desc = opts.Desc,
        Default = default,
...
function UI.AddSlider(parent, opts)
    local default = tonumber(opts.Default) or tonumber(opts.Min) or 0
    UI.__state[opts.Id] = default

    local element = parent:Slider({
        Title = opts.Title or "Slider",
        Desc = opts.Desc,
        Step = opts.Step or 1,
        Value = {
            Min = opts.Min or 0,
...
function UI.AddDropdown(parent, opts)
    ...
    if isMulti then
        ...
        if type(opts.Default) == "table" then
            for _, v in ipairs(opts.Default) do table.insert(default, tostring(v)) end
        end
        UI.__state[opts.Id] = default
    else
        default = opts.Default
        if default == nil then default = options[1] end
        if default ~= nil then default = tostring(default) end
        UI.__state[opts.Id] = default
    end

    local element = parent:Dropdown({
        Title = opts.Title or "Dropdown",
        Desc = opts.Desc,
        Values = options,
        Value = default,
        Multi = isMulti,
            ...
            if isMulti then
                ...
                if type(value) == "table" then
                    for _, item in ipairs(value) do
                        table.insert(normalized, tostring(item))
                    end
                end
                UI.SetState(opts.Id, normalized, true)
                if opts.Callback then pcall(opts.Callback, normalized) end
            else
    ...
    -- Preserve the old script's refreshPet handle.
...
function UI.AddButton(parent, opts)
    return parent:Button({
        ...
        Desc = opts.Desc,
        Icon = opts.Icon or "solar:play-bold",
        Color = opts.Color or COLORS.accent,
        Callback = function()
...
function UI.AddParagraph(parent, opts)
    return parent:Paragraph({
        Title = opts.Title or "",
        Desc = opts.Content or opts.Desc or "",
    })
end

function UI.AddDivider(parent, opts)
    if parent.Divider then
...
function UI.AddStatus(parent, opts)
    local currentValue = tostring(opts.Value or "")
    local status = parent:Paragraph({
    ...
    function api.SetValue(value)
        ...
        pcall(function() status:SetDesc(currentValue) end)
    end

    function api.SetStatus(state)
        local title = tostring(opts.Title or "Status")
        local value = currentValue
        local icon = "●"
        if state == "Success" then icon = "🟣"
        elseif state == "Warning" then icon = "🟡"
        elseif state == "Error" then icon = "🔴"
        elseif state == "Info" then icon = "🔵" end
        pcall(function() status:SetTitle(icon .. " " .. title) end)
        pcall(function() status:SetDesc(value) end)
    end
...
function UI.AddInput(parent, opts)
    UI.__state[opts.Id] = tostring(opts.Default or "")
    return parent:Input({
...
function H.notify(title, content, variant, duration)
    local icon = "sparkles"
    if variant == "Success" then icon = "circle-check"
    elseif variant == "Warning" then icon = "triangle-alert"
    elseif variant == "Error" then icon = "circle-x"
    elseif variant == "Info" then icon = "info" end

    return WindUI:Notify({
        Title = tostring(title or "ZenHubX"),
...
do
    ...
    local SessionSec = UI.AddSection(HomeTab, { Title = L("sectionSession") })
    local AccountSec = UI.AddSection(HomeTab, { Title = L("sectionAccount") })
    local QuickSec = UI.AddSection(HomeTab, { Title = L("sectionQuick") })
    ...
    local StealSec = UI.AddSection(FarmTab, { Title = L("sectionSteal") })
    local LifeSec = UI.AddSection(FarmTab, { Title = L("sectionLifecycle") })
    local ServerSec = UI.AddSection(FarmTab, { Title = L("sectionServer") })
    ...
    UI.AddDivider(StealSec, { Title = "Filters" })
    ...
    -- Visual Pets
    local petDrop = UI.AddDropdown(VisualPetsSec, { Id = "VisualPetName", Title = "Pet", Options = { "None" }, Default = "None",
        Callback = function(v) VisualPets.inputName = tostring(v) end })
    pillButtons.refreshPet = UI.AddButton(VisualPetsSec, { Title = "Refresh Pet List", Text = "Scan", Callback = function()
    ...
    -- EXTRAS (từ steal egg zen hubX + raw 4)
    ...
    local SettingsSec = UI.AddSection(SysTab, { Title = L("sectionSettings") })
    local SocialSec = UI.AddSection(SysTab, { Title = L("sectionSocial") })
