--[[
    ...
    TikTok: zenhub.z
    [MOD] Forest FLY-in → grab → RAGDOLL → TELE NGAY → grab target → walk STRAIGHT to base center
    [MOD2] Anti-Knock mạnh — clamp cả position + velocity, hard-reset về anchor
--]]
...
local Remotes = {
    Backpack = {
        EQUIP_BEST = H.remoteFrom(RemotesModule, "Haul", "WearBest") or H.findRemoteContains("WearBest") or H.findRemoteContains("EQUIP_BEST"),
    },
    Plots = {
        REQUEST_BASE_UPGRADE = H.remoteFrom(RemotesModule, "Homestead", "AskBaseTierRaise") or H.findRemoteContains("AskBaseTierRaise") or H.findRemoteContains("BaseUpgrade"),
    },
    Treadmills = {
    ...
    AssetInventory = {
        SELL_ASSET = H.remoteFrom(RemotesModule, "PetSatchel", "SellPet") or H.findRemoteContains("SellPet") or H.findRemoteContains("SELL_ASSET"),
    },
    OfflineAssets = {
...
-- ============================================================
-- [MOD] RAGDOLL + FOREST HELPERS
-- ============================================================
function H.isRagdoll()
...
function H.waitForRagdoll(timeout, onDetected)
    timeout = tonumber(timeout) or RAGDOLL_WAIT_TIMEOUT
    local deadline = os.clock() + timeout
    while running and os.clock() < deadline do
        if H.isRagdoll() then
            if onDetected then pcall(onDetected) end
            return true
        end
        RunService.Heartbeat:Wait()
    end
...
function H.setRagdollStatesAllowed(allowed)
    local hum = H.getHumanoid()
    if not hum then return end
    pcall(function()
        hum:SetStateEnabled(Enum.HumanoidStateType.Ragdoll, allowed)
        hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown, allowed)
        hum:SetStateEnabled(Enum.HumanoidStateType.Physics, allowed)
        hum:SetStateEnabled(Enum.HumanoidStateType.GettingUp, true)
    end)
end

function H.findEggSlotInstance(uid)
    if typeof(uid) ~= "string" or not AreaEggSlotsClient then return nil end
    return AreaEggSlotsClient:FindFirstChild(uid)
end

function H.pickRandomForestEggRecord()
    -- Force refresh snapshot để có data mới nhất
    if EggApi.RequestAreaEggSnapshot then pcall(EggApi.RequestAreaEggSnapshot) end
    local records = H.getAreaEggs()
    local candidates = {}
    for _, rec in ipairs(records) do
        if typeof(rec) == "table"
            and rec.AreaId == "Forest"
            and (rec.State == "Slot" or rec.State == "Dropped") then
            if H.findEggSlotInstance(rec.Uid) then
...
-- ============================================================
-- [MOD2] ANTI-KNOCK MẠNH — clamp position + velocity, hard-reset anchor
-- ============================================================
local AntiKnock = {
    Enabled = false,
    MaxHorizontal = 6,
...
H.track(RunService.Stepped:Connect(function()
    ...
    -- 2. Xoá mọi constraint/force gây văng
    for _, obj in ipairs(ch:GetDescendants()) do
        if obj:IsA("Constraint") or obj:IsA("BodyVelocity")
        or obj:IsA("BodyThrust") or obj:IsA("VectorForce")
        or obj:IsA("BodyAngularVelocity") or obj:IsA("BodyPosition")
    ...
    local clampedVel = Vector3.new(horizontal.X, vy, horizontal.Z)
    hrp.AssemblyLinearVelocity = clampedVel
    hrp.AssemblyAngularVelocity = Vector3.zero
    pcall(function()
        hrp.Velocity = clampedVel
        hrp.RotVelocity = Vector3.zero
    end)

    -- 4. HARD-CLAMP vị trí về anchor nếu văng quá xa
    if AntiKnock.AnchorCF then
        ...
        if age > AntiKnock.ResetAfter then
            ...
            local dy = curPos.Y - anchorPos.Y
            local horizontalDist = math.sqrt(dx * dx + dz * dz)
            if horizontalDist > AntiKnock.DriftLimit or math.abs(dy) > AntiKnock.DriftLimit then
    ...
    -- 5. Nếu ragdoll kéo dài > 3s → ép GettingUp
    local st = hum:GetState()
    if (st == Enum.HumanoidStateType.Ragdoll
        or st == Enum.HumanoidStateType.FallingDown
        or st == Enum.HumanoidStateType.Physics)
        and AntiKnock.AnchorCF then
        local ragdollAge = os.clock() - AntiKnock.AnchorSetAt
        if ragdollAge > 3.0 then
            pcall(function()
...
-- ============================================================
-- [MOD] stealEgg: B0 fly-in Forest → grab → ragdoll → TELE NGAY → grab target → walk STRAIGHT to base center
-- ============================================================
function H.stealEgg(slotEgg)
    ...
    -- ========================================================
    -- BƯỚC 0: FOREST WARM-UP — BẮT BUỘC, KHÔNG SKIP
    -- ========================================================
    local step0Completed = false
    local step0Attempts = 0
    local MAX_STEP0_ATTEMPTS = 3

    while step0Completed == false and step0Attempts < MAX_STEP0_ATTEMPTS do
        step0Attempts = step0Attempts + 1

        if EggApi.RequestAreaEggSnapshot then pcall(EggApi.RequestAreaEggSnapshot) end
        task.wait(0.1)

        local forestRecord = H.pickRandomForestEggRecord()
        local forestSlot = forestRecord and H.findEggSlotInstance(forestRecord.Uid) or nil

        local forestTargetPos = nil
        local forestTargetSlot = nil
        if forestSlot then
            forestTargetPos = H.getSlotEggPosition(forestSlot)
            forestTargetSlot = forestSlot
        else
            local zone = H.getZoneModel("Forest")
            if zone then
                local bounds = zone:FindFirstChild("Bounds")
                if bounds and bounds:IsA("BasePart") then
                    forestTargetPos = Vector3.new(
                        bounds.Position.X + (math.random() - 0.5) * 8,
                        bounds.Position.Y,
                        H.getLaneZ()
                    )
                else
                    local ok, cf = pcall(function() return zone:GetBoundingBox() end)
                    if ok and cf then
                        forestTargetPos = Vector3.new(
                            cf.Position.X + (math.random() - 0.5) * 8,
                            cf.Position.Y,
                            H.getLaneZ()
                        )
                    end
        ...
        if forestTargetPos then
            AntiKnock.Enabled = false
            H.bypassMoveTo(forestTargetPos, H.stealingEnabled, BYPASS_SPEED)
            task.wait(0.15)

            H.setRagdollStatesAllowed(true)
            AntiKnock.Enabled = true
            root = H.getRoot()
            if root then AntiKnock.SetAnchor(root.Position) end

            if forestTargetSlot then
                ...
                while running and H.stealingEnabled() and os.clock() < grabDeadline do
                    ...
                    H.tryCarryEgg(forestTargetSlot)
                    if IsCarryingEgg then break end
                    task.wait(0.05)
                end
            end

            -- Chờ ragdoll → TELE NGAY
            local ragdollTriggered = false
            H.waitForRagdoll(RAGDOLL_WAIT_TIMEOUT, function()
                ragdollTriggered = true
                AntiKnock.Enabled = false
                AntiKnock.ClearAnchor()
                local r = H.getRoot()
                if r then
                    local ty = H.groundedY(targetPosition.X, targetPosition.Z, targetPosition.Y)
                    H.placeRoot(r, CFrame.new(targetPosition.X, ty, targetPosition.Z))
                end
            end)

            if not ragdollTriggered then
                AntiKnock.Enabled = false
                AntiKnock.ClearAnchor()
                local r = H.getRoot()
                if r then
            ...
            H.setRagdollStatesAllowed(false)
            local hum = H.getHumanoid()
            if hum then
                pcall(function()
                    hum.PlatformStand = false
...
-- [MOD] getBasePosition ƯU TIÊN CenterPoint để đi thẳng về giữa base
...
function H.bypassMoveTo(position, keepGoing, speed)
    ...
    while running and os.clock() < deadline do
        ...
        -- Đi THẲNG: bay trực tiếp tới đích, không qua waypoint
...
function H.ensureAtPlot(keepGoing)
    ...
    return H.bypassMoveTo(basePos, keepGoing, BYPASS_SPEED)
end
...
-- ============================================================
local ExtrasState = {
    AntiRagdoll = false, InstantInteract = false, FixLag = false,
    AntiTrap = false, LookRocket = false, SpeedBypass = false, SpeedValue = 300,
}
...
function H.ensureEspEntry(id, color)
    ...
    EspEntries[id] = { anchor = anchor, billboard = billboard, label = label, highlight = nil }
...
function H.hopLowPop()
    task.spawn(function()
        ...
        if not page then H.notify("Hop", "Server list unavailable", "Error", 3); return end
        local candidates = {}
        ...
        if #candidates == 0 then H.notify("Hop", "No low-pop server found", "Warning", 3); return end
...
local AutomationJobs = {
    ["Auto Steal Egg"] = { Ready = H.canAutoSteal, Run = H.runAutoSteal, Interval = 0.05 },
...
do
    ...
    conn(toggleBtn.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            dragStart = input.Position; startPos = toggleBtn.Position
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then dragging = false end
            end)
        end
...
local function makeTabButton(tabId, tabTitle)
    ...
    btn.MouseLeave:Connect(function() if UI.__activeTab ~= tabId then setActive(false) end end)
...
do
    ...
    -- [MOD] Toggles với callback trigger ngay lập tức
    local function triggerImmediateSteal()
        AutomationLastRunAt["Auto Steal Egg"] = 0
        task.defer(function()
    ...
    UI.AddToggle(StealSec, {
        Id = "AutoStealSelected", Title = "Auto Steal Selected",
        Description = "Use filters below", Default = false,
        Callback = function(state)
            if state then
                if EggApi.RequestAreaEggSnapshot then pcall(EggApi.RequestAreaEggSnapshot) end
                triggerImmediateSteal()
            end
        end
    })
    UI.AddToggle(StealSec, {
        Id = "AutoStealAll", Title = "Auto Steal All",
        Description = "Ignore rarity/mutation", Default = false,
        Callback = function(state)
            if state then
                if EggApi.RequestAreaEggSnapshot then pcall(EggApi.RequestAreaEggSnapshot) end
                triggerImmediateSteal()
    ...
    UI.AddToggle(StealSec, {
        Id = "StealBigEggs", Title = "Steal Big Eggs", Default = false,
        Callback = function(state)
            if state then
                if EggApi.RequestAreaEggSnapshot then pcall(EggApi.RequestAreaEggSnapshot) end
                triggerImmediateSteal()
    ...
    UI.AddToggle(WorldSec, { Id = "FixLag", Title = "Fix Lag (Clay)",
        ...
        Callback = function(v)
            if v then ExtrasState.EnableFixLag() else ExtrasState.FixLag = false end
        end })
    UI.AddToggle(AntiTrapSec, { Id = "AntiTrap", Title = "Anti Trap (Delete)",
        Description = "Disable & hide traps", Default = false,
        Callback = function(v)
            if v then ExtrasState.EnableAntiTrap() else ExtrasState.AntiTrap = false end
        end })
    UI.AddButton(AntiTrapSec, { Title = "Hop Low-Pop Server (1-5 players)", Text = "Hop", Callback = function() H.hopLowPop() end })
    ...
    UI.AddButton(SpeedSec2, { Title = "Apply Speed Bypass (Clone Humanoid)", Text = "Apply",
        Callback = function()
            task.spawn(function()
                ...
                H.notify("Speed", ok and ("Bypass " .. sp .. " applied") or "Failed", ok and "Success" or "Error", 3)
            end)
        end })
...
H.track(RunService.Heartbeat:Connect(function()
    ...
    if schedulerDue("core", 0.15) then
        task.spawn(function()
            if H.stealingEnabled() then H.swapStealHumanoid() end
            runCarryJobs()
            runPriorityJobs()
...
H.notify("ZenHubX", "Ready - press avatar to open", "Success", 5)
