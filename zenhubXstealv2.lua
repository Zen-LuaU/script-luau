...
do
    ...
    UI.AddToggle(SellPetSec, {
        ...
        Callback = function(v)
            ...
            if v then
                H.notify("Auto Sell Pet", "Đã bật. Quét pet mỗi " .. PetSellConfig.SellPetInterval .. "s.", "Success", 3)
            else
                H.notify("Auto Sell Pet", "Đã tắt.", "Info", 3)
            end
        end
    })
    ...
    UI.AddButton(SellPetSec, {
        ...
        Callback = function()
            task.spawn(function()
                ...
                table.sort(names)
                if #names == 0 then
                    H.notify("Auto Sell Pet", "Không tìm thấy pet nào.", "Warning", 3)
                    return
                end
