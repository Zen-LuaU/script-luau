--[[
    ...
    TikTok: zenhub.z
    Anti-Kick Edition + Extras + Auto Take
--]]
...
-- ============================================================
-- EXTRAS (Anti-Ragdoll, Instant Interact, Fix Lag, Anti Trap,
--         Speed Bypass, Auto Take, Void Reset, AntiRagdollCamera)
-- ============================================================
local ExtrasState = {
    AntiRagdoll = false,
    AntiRagdollCameraMode = "Keep Camera",
...
-- Anti-Ragdoll + LookRocket + AntiRagdollCamera
H.track(RunService.RenderStepped:Connect(function()
    ...
    -- Anti-Ragdoll Camera: redirect camera to our humanoid if config requires
    if ExtrasState.AntiRagdoll and ExtrasState.AntiRagdollCameraMode == "Follow Humanoid" then
...
-- ============================================================
-- AUTO TAKE (ProximityPrompt based, from file 2)
-- ============================================================
local AUTO_TAKE_KEYWORDS = {"steal","grab","take","collect","rob","loot","pickpocket","snatch","open","interact"}
local AutoTakeCache = {}
...
local function isAutoTakePrompt(obj)
    if not obj:IsA("ProximityPrompt") then return false end
    local action = string.lower(tostring(obj.ActionText or ""))
    for _, kw in ipairs(AUTO_TAKE_KEYWORDS) do
        if string.find(action, kw, 1, true) then return true end
    end
    local n = string.lower(obj.Name or "")
    local pn = string.lower(obj.Parent and obj.Parent.Name or "")
    return string.find(n, "steal", 1, true) ~= nil or string.find(pn, "steal", 1, true) ~= nil
end

local function refreshAutoTakeCache()
    table.clear(AutoTakeCache)
    for _, d in ipairs(Workspace:GetDescendants()) do
        if isAutoTakePrompt(d) then table.insert(AutoTakeCache, d) end
    end
end
...
H.track(Workspace.DescendantRemoving:Connect(function(d)
    if d:IsA("ProximityPrompt") then
        local i = table.find(AutoTakeCache, d)
        if i then table.remove(AutoTakeCache, i) end
    end
end))

task.spawn(function()
    pcall(refreshAutoTakeCache)
    while running do
        task.wait(1.2)
        if ExtrasState.AutoTake then pcall(refreshAutoTakeCache) end
    end
end)

H.track(RunService.Heartbeat:Connect(function()
    if not running or not ExtrasState.AutoTake then return end
    local root = H.getRoot()
    if not root then return end
    local limit = ExtrasState.AutoTakeDistance
    for i = #AutoTakeCache, 1, -1 do
        local p = AutoTakeCache[i]
        if not p or not p.Parent then
            table.remove(AutoTakeCache, i)
        elseif p.Enabled then
            local part = getPromptPart(p)
            if part and part.Position then
                if (root.Position - part.Position).Magnitude <= limit then
                    pcall(function()
                        if fireproximityprompt then
                            fireproximityprompt(p, limit, 0, false)
                        else
                            p.HoldDuration = 0
                            p:InputHoldBegin()
                            p:InputHoldEnd()
...
function H.hopLowPop()
    task.spawn(function()
        ...
        if not page then
            H.notify("Hop", "Server list unavailable", "Error", 3)
            return
        end
        local candidates = {}
        ...
        if #candidates == 0 then
            H.notify("Hop", "No low-pop server found", "Warning", 3)
            return
        end
        ...
        H.notify("Hop", "Hopping to low-pop server", "Success", 3)
    end)
end

-- ============================================================
-- SERVER HOP GUI (from file 2)
-- ============================================================
function H.openServerHopGUI()
    ...
    HopBtn.BackgroundColor3 = Color3.fromRGB(138, 43, 226)
    ...
    task.spawn(function()
        while ScreenGui.Parent do
            NowLabel.Text = "Player ở server hiện tại: " .. #Players:GetPlayers()
            task.wait(1)
        end
    end)
    ...
    local function getRandomServer()
        ...
        for _, s in ipairs(data.data) do
            if s.id and tostring(s.id) ~= tostring(game.JobId) then
                if s.playing and s.playing < targetMax then
...
local LANG = {
    ...
    Strings = {
        vi = {
            ready = "Sẵn sàng - ấn avatar để mở",
            discordCopied = "Đã sao chép link Discord",
            tiktokCopied = "Đã sao chép TikTok ID: zenhub.z",
            languageChanged = "Đã chuyển sang Tiếng Việt",
        },
        en = {
            ready = "Ready - press avatar to open",
            discordCopied = "Discord link copied",
            tiktokCopied = "TikTok ID copied: zenhub.z",
            languageChanged = "Switched to English",
        },
    },
}
function L(key)
    local pack = LANG.Strings[LANG.Current] or LANG.Strings.vi
    return pack[key] or key
end

-- English -> Vietnamese map for UI text replacement
local VI_TITLES = {
["Home"]="Trang chủ",["Session"]="Phiên",["Account"]="Tài khoản",["Quick Actions"]="Thao tác nhanh",
["Automation"]="Tự động hóa",["Current Job"]="Tác vụ hiện tại",["Stolen Eggs"]="Trứng đã trộm",["Carrying Egg"]="Đang mang trứng",
["Runtime"]="Thời gian chạy",["Server"]="Máy chủ",["Egg Inventory"]="Kho trứng",["Money"]="Tiền",["Speed Power"]="Sức mạnh tốc độ",
["Rebirths"]="Tái sinh",["Pets Owned"]="Pet đang sở hữu",["Return to Base"]="Về căn cứ",["Place Eggs"]="Đặt trứng",
["Server Hop"]="Đổi server",["Fuse Now"]="Gộp ngay",["Farm"]="Farm",["Steal Eggs"]="Trộm trứng",["Egg Handling"]="Xử lý trứng",
["Task Order"]="Thứ tự tác vụ",["Auto Steal Selected"]="Tự trộm mục đã chọn",["Auto Steal All"]="Tự trộm tất cả",
["Steal Big Eggs"]="Trộm trứng lớn",["Target filters"]="Bộ lọc mục tiêu",["Areas"]="Khu vực",["Rarities"]="Độ hiếm",
["Mutations"]="Biến thể",["Target Priority"]="Ưu tiên mục tiêu",["Minimum Big Egg Size"]="Kích thước trứng lớn tối thiểu",
["Carry behavior"]="Cách mang trứng",["Auto Return to Base"]="Tự về căn cứ",["Auto Drop Held Egg"]="Tự thả trứng đang cầm",
["Auto Place Selected"]="Tự đặt mục đã chọn",["Auto Place All"]="Tự đặt tất cả",["Auto Hatch Ready"]="Tự ấp trứng sẵn sàng",
["Lifecycle Rarities"]="Độ hiếm vòng đời",["Lifecycle Mutations"]="Biến thể vòng đời",["Egg selling"]="Bán trứng",
["Auto Sell Eggs"]="Tự bán trứng",["Sell Rarities"]="Độ hiếm bán",["Sell Interval"]="Khoảng thời gian bán",
["Auto Server Hop"]="Tự đổi server",["Hop When"]="Đổi server khi",["Wait Before Hop"]="Chờ trước khi đổi",["Hop Now"]="Đổi server ngay",
["Pets"]="Pet",["Auto Fuse"]="Tự gộp",["Auto Sell Pets"]="Tự bán pet",["Auto Equip Best Pets"]="Tự trang bị pet tốt nhất",
["Hide Own Pet Renders"]="Ẩn pet của mình",["Auto Fuse Pets"]="Tự gộp pet",["Fuse Rarities"]="Độ hiếm gộp",["Fuse Mutations"]="Biến thể gộp",
["Pick Group By"]="Nhóm theo",["Never Fuse Mutated"]="Không gộp pet biến thể",["Never Fuse Equipped"]="Không gộp pet đang trang bị",
["Auto Complete Reveal"]="Tự hoàn tất khám phá",["Maximum Scale to Fuse"]="Scale tối đa để gộp",["Keep Per Pet Type"]="Giữ mỗi loại pet",
["Fuse Interval"]="Khoảng thời gian gộp",["Sell Mutations"]="Biến thể bán",["Never Sell Mutated"]="Không bán pet biến thể",
["Never Sell Equipped"]="Không bán pet đang trang bị",["Maximum Scale to Sell"]="Scale tối đa để bán",["Progress"]="Tiến trình",
["Upgrades"]="Nâng cấp",["Rewards"]="Phần thưởng",["Equipment"]="Trang bị",["Training"]="Huấn luyện",
["Auto Buy Upgrades"]="Tự mua nâng cấp",["Upgrade Types"]="Loại nâng cấp",["Auto Claim Index"]="Tự nhận Index",
["Auto Claim Group Reward"]="Tự nhận thưởng nhóm",["Claim Offline Earnings"]="Nhận tiền offline",["Auto Buy Trail"]="Tự mua Trail",
["Trails"]="Trail",["Auto Equip Best Trail"]="Tự trang bị Trail tốt nhất",["Auto Equip Best Gear"]="Tự trang bị Gear tốt nhất",
["Auto Treadmill Training"]="Tự tập máy chạy",["Player"]="Người chơi",["ESP"]="ESP",["Movement"]="Di chuyển",["Teleports"]="Dịch chuyển",
["World Egg ESP"]="ESP trứng trên map",["Carried and Dropped Egg ESP"]="ESP trứng đang cầm/thả",["Guard ESP"]="ESP bảo vệ",
["Pet ESP"]="ESP pet",["Player ESP"]="ESP người chơi",["Machine ESP"]="ESP máy",["Plot ESP"]="ESP khu đất",
["Render Distance"]="Khoảng cách hiển thị",["Walk Speed Override"]="Chỉnh tốc độ chạy",["Walk Speed"]="Tốc độ chạy",
["Jump Power Override"]="Chỉnh lực nhảy",["Jump Power"]="Lực nhảy",["Infinite Jump"]="Nhảy vô hạn",["NoClip"]="Xuyên tường",
["Fly"]="Bay",["Fly Speed"]="Tốc độ bay",["Waypoint"]="Điểm đánh dấu",["Teleport to Waypoint"]="Dịch chuyển tới điểm",
["Extras"]="Bổ sung",["Character"]="Nhân vật",["World"]="Thế giới",["Speed Bypass"]="Bypass tốc độ",["Anti Trap"]="Chống bẫy",
["Anti-Ragdoll"]="Chống ragdoll",["Instant Interact"]="Tương tác tức thì",["Tele Guiado (LookRocket)"]="Tele Guiado (LookRocket)",
["Fix Lag (Clay)"]="Giảm lag (Clay)",["Anti Trap (Delete)"]="Chống bẫy (Xóa)",["Hop Low-Pop Server (1-5 players)"]="Đổi server ít người (1-5)",
["Speed Value"]="Giá trị tốc độ",["Lock WalkSpeed"]="Khóa tốc độ chạy",["Apply Speed Bypass (Clone Humanoid)"]="Áp dụng bypass tốc độ",
["System"]="Hệ thống",["Performance"]="Hiệu năng",["Settings"]="Cài đặt",["Social"]="Liên kết",["Webhooks"]="Webhook",
["About"]="Thông tin",["Anti-AFK"]="Chống AFK",["No Gameplay Paused"]="Không tạm dừng gameplay",["Auto Reconnect"]="Tự kết nối lại",
["Rejoin Server"]="Vào lại server",["Copy Join Script"]="Sao chép script vào server",["FPS Boost"]="Tăng FPS",
["Disable 3D Rendering"]="Tắt render 3D",["FPS Cap"]="Giới hạn FPS",["Language / Ngôn ngữ"]="Ngôn ngữ",
["Copy Discord Link"]="Sao chép link Discord",["Copy TikTok ID"]="Sao chép TikTok ID",["Enable Webhooks"]="Bật Webhook",
["Webhook URL"]="URL Webhook",["Ping User ID"]="ID người nhận ping",["Summary Interval"]="Khoảng thời gian tổng kết",
["List Spawned Eggs"]="Liệt kê trứng đã spawn",["Disconnect Alerts"]="Cảnh báo mất kết nối",["Send Summary Now"]="Gửi tổng kết ngay",
["Script Dev"]="Nhà phát triển script",["UI"]="Giao diện",["Discord"]="Discord",["TikTok"]="TikTok",["Danger Zone"]="Khu vực nguy hiểm",
["Unload Script"]="Tắt script",["Visual Pet"]="Visual Pet",["Refresh Pet List"]="Làm mới danh sách pet",["Custom Name"]="Tên tùy chỉnh",
["Orbit Radius"]="Bán kính xoay",["Orbit Speed"]="Tốc độ xoay",["Spawn Pet"]="Spawn Pet",["Remove Last Pet"]="Xóa pet cuối",
["Remove All Pets"]="Xóa tất cả pet",["Void Reset"]="Về Void",["Anti-Ragdoll Camera"]="Camera chống ragdoll",
["Auto Take (Prompt)"]="Auto Take (Prompt)",["Auto Take Distance"]="Khoảng cách Auto Take",
["Open Server Hop GUI"]="Mở bảng đổi server",["Keep Camera"]="Giữ camera",["Follow Humanoid"]="Theo nhân vật",
["Return"]="Về",["Place"]="Đặt",["Hop"]="Đổi",["Fuse"]="Gộp",["Run"]="Chạy",["Send"]="Gửi",["Copy"]="Sao chép",
["Refresh"]="Làm mới",["Spawn"]="Triệu hồi",["Remove"]="Xóa",["Remove All"]="Xóa tất cả",["Rejoin"]="Vào lại",
["Apply"]="Áp dụng",["Go"]="Đi",["All"]="Tất cả",["Yes"]="Có",["No"]="Không",["Ready"]="Sẵn sàng",["Idle"]="Chờ",
}
...
-- ============================================================
-- LANGUAGE: applyLanguageToUI defined AFTER screenGui
-- (this fixes the previous bug where screenGui was nil)
-- ============================================================
    ...
    local ok = pcall(function()
        for _, obj in ipairs(screenGui:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                local original = obj:GetAttribute("ZenOriginalText")
                if original then
                    local text = tostring(original)
                    if LANG.Current == "vi" then
                        if text:sub(1, 9) == "Priority " then
                            text = "Ưu tiên " .. text:sub(10)
                        else
                            text = VI_TITLES[text] or text
                        end
                    else
...
-- Nếu ảnh không load, hiển thị chữ Z fallback
task.delay(2.5, function()
    pcall(function()
        -- ImageButton không có property ContentText; dùng TextButton fallback nếu cần.
        -- Nếu asset fail, người dùng vẫn click được vì ImageButton có thể có Image rỗng.
    end)
end)
...
discordBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard(DISCORD_LINK) end)
    H.notify("ZenHubX", L("discordCopied"), "Success", 3)
end)
...
do
    ...
    local SessionSec = UI.AddSection(HomeTab, { Title = "Session", Description = "Live status" })
    local AccountSec = UI.AddSection(HomeTab, { Title = "Account", Description = "Save data" })
    ...
    refs.jobRow = UI.AddStatus(SessionSec, { Title = "Current Job", Value = "Idle" })
    ...
    local StealSec = UI.AddSection(FarmTab, { Title = "Steal Eggs", Description = "Main egg farming" })
    ...
    UI.AddDivider(StealSec, { Title = "Target filters" })
    ...
    UI.AddSlider(StealSec, { Id = "StealBigEggScale", Title = "Minimum Big Egg Size", Min = 1, Max = 50, Default = 1.5, Step = 0.1, Suffix = "x" })
    ...
    UI.AddDivider(LifeSec, { Title = "Egg selling" })
    ...
    UI.AddButton(ServerSec, { Title = "Open Server Hop GUI", Text = "Open", Callback = function()
        H.openServerHopGUI()
    end })

    UI.AddParagraph(PrioritySec, { Content = "Runs the first ready task in this order." })
    ...
    local function spawnVisualPet(name)
        ...
        if not source then
            H.notify("Visual Pet", "Cannot find pet: " .. tostring(name), "Warning", 4)
            return
        end
        ...
        H.notify("Visual Pet", "Spawned " .. tostring(name) .. " (client only).", "Success", 3)
    end

    local VisualPetDropdown = UI.AddDropdown(PetsOverview, {
        Id = "VisualPetName", Title = "Visual Pet",
        Options = {"Scanning..."}, Default = "Scanning...",
        Callback = function(v)
    ...
    UI.AddButton(PetsOverview, {
        ...
        Callback = function()
            if VisualPetState.inputName == "" or VisualPetState.inputName == "Scanning..." then
                H.notify("Visual Pet", "Select or type a pet name.", "Warning", 4)
                return
            end
            spawnVisualPet(VisualPetState.inputName)
        end
    })
    UI.AddButton(PetsOverview, { Title = "Remove Last Pet", Text = "Remove", Callback = removeLastVisualPet })
    UI.AddButton(PetsOverview, { Title = "Remove All Pets", Text = "Remove All", Callback = removeAllVisualPets })
    ...
    task.delay(2, function()
        pcall(function()
            ...
            if #list > 0 then
                ...
                VisualPetDropdown:Refresh(list)
                VisualPetState.inputName = list[1]
                UI.SetState("VisualPetName", list[1], true)
            end
        end)
    ...
    UI.AddSlider(FuseSec, { Id = "FuseMaxScale", Title = "Maximum Scale to Fuse", Min = 0, Max = 10, Default = 10, Step = 0.1 })
    UI.AddSlider(FuseSec, { Id = "FuseKeepPerCategory", Title = "Keep Per Pet Type", Min = 0, Max = 20, Default = 0, Step = 1 })
    ...
    UI.AddSlider(SellPetSec, { Id = "SellMaxScale", Title = "Maximum Scale to Sell", Min = 0, Max = 10, Default = 10, Step = 0.1 })
    ...
    UI.AddToggle(EspSec, { Id = "EspCarriedEggs", Title = "Carried and Dropped Egg ESP", Default = false })
    ...
    -- EXTRAS
    local ExtrasTab = UI.AddTab({ Id = "extras", Title = "Extras" })
    local CharSec = UI.AddSection(ExtrasTab, { Title = "Character" })
    local WorldSec = UI.AddSection(ExtrasTab, { Title = "World" })
    local SpeedSec2 = UI.AddSection(ExtrasTab, { Title = "Speed Bypass" })
    local AntiTrapSec = UI.AddSection(ExtrasTab, { Title = "Anti Trap" })
    local AutoTakeSec = UI.AddSection(ExtrasTab, { Title = "Auto Take (Prompt)" })
    ...
    UI.AddDropdown(CharSec, { Id = "AntiRagdollCamera", Title = "Anti-Ragdoll Camera",
        Options = {"Keep Camera", "Follow Humanoid"}, Default = "Keep Camera",
        Callback = function(v)
            if type(v) == "table" then v = next(v) end
            ExtrasState.AntiRagdollCameraMode = tostring(v or "Keep Camera")
        end })

    UI.AddToggle(CharSec, { Id = "InstantInteract", Title = "Instant Interact",
        Description = "Skip hold duration for prompts", Default = false,
        Callback = function(v) ExtrasState.SetInstantHold(v) end })

    UI.AddToggle(CharSec, { Id = "LookRocket", Title = "Tele Guiado (LookRocket)",
        Description = "Redirect velocity to camera when thrown", Default = false,
        Callback = function(v) ExtrasState.LookRocket = v end })

    UI.AddButton(CharSec, { Title = "Void Reset", Text = "Drop",
        Callback = function()
            task.spawn(function()
    ...
    UI.AddToggle(WorldSec, { Id = "FixLag", Title = "Fix Lag (Clay)",
        Description = "Remove foliage + SmoothPlastic", Default = false,
        Callback = function(v)
            if v then ExtrasState.EnableFixLag()
            else ExtrasState.FixLag = false end
        end })

    UI.AddToggle(AntiTrapSec, { Id = "AntiTrap", Title = "Anti Trap (Delete)",
        Description = "Disable & hide traps", Default = false,
        Callback = function(v)
            if v then ExtrasState.EnableAntiTrap()
            else ExtrasState.AntiTrap = false end
        end })

    UI.AddButton(AntiTrapSec, { Title = "Hop Low-Pop Server (1-5 players)", Text = "Hop",
        Callback = function() H.hopLowPop() end })

    UI.AddSlider(SpeedSec2, { Id = "BypassSpeed", Title = "Speed Value",
        Min = 100, Max = 900, Default = 300, Step = 10,
        Callback = function(v) ExtrasState.SpeedValue = v end })

    UI.AddToggle(SpeedSec2, { Id = "SpeedBypassToggle", Title = "Lock WalkSpeed",
        Description = "Locks WalkSpeed every frame", Default = false,
        Callback = function(v) ExtrasState.SpeedBypass = v end })

    UI.AddButton(SpeedSec2, { Title = "Apply Speed Bypass (Clone Humanoid)", Text = "Apply",
        Callback = function()
            task.spawn(function()
                local sp = tonumber(H.optionValue("BypassSpeed", 300)) or 300
                ExtrasState.SpeedValue = sp
                local ok = ExtrasState.ApplySpeedBypass(sp)
                H.notify("Speed", ok and ("Bypass " .. sp .. " applied") or "Failed",
                    ok and "Success" or "Error", 3)
            end)
        end })

    -- Auto Take (ProximityPrompt)
    UI.AddToggle(AutoTakeSec, { Id = "AutoTake", Title = "Auto Take (Prompt)",
        Description = "Fire steal prompts within range", Default = false,
        Callback = function(v) ExtrasState.AutoTake = v end })

    UI.AddSlider(AutoTakeSec, { Id = "AutoTakeDistance", Title = "Auto Take Distance",
        Min = 5, Max = 100, Default = 35, Step = 1, Suffix = " studs",
        Callback = function(v) ExtrasState.AutoTakeDistance = tonumber(v) or 35 end })
    ...
    UI.AddButton(SessionSysSec, { Title = "Copy Join Script", Text = "Copy", Callback = function()
        pcall(function() setclipboard(string.format(
            'game:GetService("TeleportService"):TeleportToPlaceInstance(%d, "%s", game:GetService("Players").LocalPlayer)',
            game.PlaceId, CurrentJobId)) end)
        H.notify("Copied", "Join script copied", "Success", 3)
    end })
    ...
    -- Language
    UI.AddDropdown(SettingsSec, { Id = "Language", Title = "Language / Ngôn ngữ",
        Options = { "English", "Tiếng Việt" }, Default = "English",
        Callback = function(v)
            if type(v) == "table" then v = next(v) end
            LANG.Current = (v == "Tiếng Việt") and "vi" or "en"
            applyLanguageToUI()
            H.notify("ZenHubX", L("languageChanged"), "Success", 3)
        end })

    UI.AddButton(SocialSec, { Title = "Copy Discord Link", Text = "Copy", Callback = function()
        pcall(function() setclipboard(DISCORD_LINK) end)
        H.notify("Discord", L("discordCopied"), "Success", 3)
    end })
    UI.AddButton(SocialSec, { Title = "Copy TikTok ID", Text = "Copy", Callback = function()
        pcall(function() setclipboard(TIKTOK_ID) end)
        H.notify("TikTok", L("tiktokCopied"), "Success", 3)
    end })
    ...
    UI.AddParagraph(AboutSec, { Title = "UI", Content = "Standalone custom UI" })
...
-- Save original text for language switching
for _, obj in ipairs(screenGui:GetDescendants()) do
    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
        if obj.Text ~= "" then
            pcall(function() obj:SetAttribute("ZenOriginalText", obj.Text) end)
        end
    end
end
...
-- Fly handler (FIXED: VectorVelocity, not VectorVectorVelocity)
