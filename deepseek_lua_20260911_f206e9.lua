--[[
    ZEN HUB X - ULTIMATE MERGED
    Kết hợp: Apex Hub (Farm/Auto) + ThanhDuyHub (EggList/VisualPet) + ZenHubX UI (WindUI)
...
-- ============================================================
-- 🛡️ ANTI-CHEAT KICKER (tự động kích hoạt khi script chạy)
-- ============================================================
...
local function CreateDiscordVerificationUI()
    ...
    Desc.Text = "Bạn cần tham gia Server Discord của chúng tôi để tiếp tục sử dụng Script!"
    ...
    VerifyBtn.Text = "2. Xác Minh & Bắt Đầu Chơi"
    ...
    JoinBtn.MouseButton1Click:Connect(function()
        ...
        JoinBtn.Text = "✓ Đã Sao Chép Link / Mở Web Discord!"
    end)
    VerifyBtn.MouseButton1Click:Connect(function()
        if joinedClicked then
            ...
            VerifyBtn.Text = "⚠️ Hãy nhấp Nút 1 trước!"
            task.wait(1.5)
            VerifyBtn.Text = "2. Xác Minh & Bắt Đầu Chơi"
        end
    end)
...
function H.stealEgg(slotEgg)
    ...
    if not H.stealAlong(H.buildStealPath(root.Position, targetPos), H.stealingEnabled) then return false end
    root = H.getRoot()
    if root then
        local ty = H.groundedY(targetPos.X, targetPos.Z, targetPos.Y)
        H.placeRoot(root, CFrame.new(targetPos.X, ty, targetPos.Z))
    end
    ...
    local cd = os.clock() + 2.5
    while running and H.stealingEnabled() and not IsCarryingEgg and os.clock() < cd do
        root = H.getRoot()
        if root then
            local ty = H.groundedY(targetPos.X, targetPos.Z, targetPos.Y)
            H.placeRoot(root, CFrame.new(targetPos.X, ty, targetPos.Z))
    ...
    if running and H.stealingEnabled() then
        ...
        local rd = os.clock() + 1.5
        while running and H.stealingEnabled() and not IsCarryingEgg and os.clock() < rd do H.tryCarryEgg(slotEgg); task.wait(0.05) end
    end
    H.returnToBaseBypass(H.stealingEnabled)
    local cf = os.clock() + 3
    while running and H.stealingEnabled() and IsCarryingEgg and os.clock() < cf do task.wait(0.1) end
    return true
end
...
function H.runAutoReturn()
    ...
    if not H.returnToBaseBypass(keep) then return false end
    local root = H.getRoot()
...
Window:Tag({ Title = "v13.0 Merged Ultimate", Icon = "solar:star-bold", Color = PurpleColor, Border = true })

-- ============================================================
-- TAB 1: AUTO STEAL
-- ============================================================
local StealSec = Window:Tab({ Title = "Auto Steal", Icon = "solar:zap-bold", IconColor = PurpleColor, Border = true })

StealSec:Section({ Title = "Steal Eggs" })
StealSec:Toggle({ Title = "Auto Steal Selected", Desc = "Dùng filter bên dưới", Value = false, Callback = function(v) UIState["AutoStealSelected"] = v end })
StealSec:Toggle({ Title = "Auto Steal All", Desc = "Bỏ qua rarity/mutation", Value = false, Callback = function(v) UIState["AutoStealAll"] = v end })
StealSec:Toggle({ Title = "Steal Big Eggs", Value = false, Callback = function(v) UIState["StealBigEggs"] = v end })
StealSec:Dropdown({ Title = "Areas", Values = AreaNames, Multi = true, Value = {}, Callback = function(v) UIValues["StealZones"] = v end })
StealSec:Dropdown({ Title = "Rarities", Values = RARITIES, Multi = true, Value = {}, Callback = function(v) UIValues["StealRarities"] = v end })
StealSec:Dropdown({ Title = "Mutations", Values = MUTATIONS, Multi = true, Value = {}, Callback = function(v) UIValues["StealMutations"] = v end })
StealSec:Dropdown({ Title = "Target Priority", Values = STEAL_PRIORITIES, Value = "Rarest", Callback = function(v) UIValues["StealPriority"] = v end })
StealSec:Slider({ Title = "Minimum Big Egg Size", Min = 1, Max = 50, Default = 1.5, Step = 0.1, Callback = function(v) UIValues["StealBigEggScale"] = v end })
StealSec:Toggle({ Title = "Auto Return to Base", Value = true, Callback = function(v) UIState["AutoReturn"] = v end })
StealSec:Toggle({ Title = "Auto Drop Held Egg", Value = false, Callback = function(v) UIState["AutoDropEgg"] = v end })
StealSec:Button({ Title = "Steal Now", Callback = function() task.spawn(H.runAutoSteal) end })

StealSec:Section({ Title = "Anti-Cheat Bypass" })
StealSec:Button({ Title = "Bypass Anti-Cheat + Speed", Desc = "Clone Humanoid để vượt qua filter", Callback = function()
    task.spawn(function()
        local ok = H.applyNewBypassAndSpeed()
        WindUI:Notify({ Title = "ZenHubX", Content = ok and "✓ Bypass thành công" or "Lỗi: Không có nhân vật" })
    end)
end })
StealSec:Toggle({ Title = "Bật/Tắt WalkSpeed", Value = false, Callback = function(v)
    SpeedConfig.Enabled = v
    local hum = H.getHumanoid()
    if hum then hum.WalkSpeed = v and SpeedConfig.Value or OriginalWalkSpeed end
end })
StealSec:Slider({ Title = "Tốc độ di chuyển", Min = 100, Max = 900, Default = 300, Step = 10, Callback = function(v)
    SpeedConfig.Value = v
    if SpeedConfig.Enabled then local hum = H.getHumanoid(); if hum then hum.WalkSpeed = v end end
end })
StealSec:Toggle({ Title = "Speed Changer (Anti-Cheat)", Value = false, Callback = function(v) UIState["SpeedChanger"] = v end })
StealSec:Toggle({ Title = "Void Unstuck", Value = false, Callback = function(v) VoidUnstuck = v end })
StealSec:Button({ Title = "Set Base Here", Callback = function()
    local hrp = H.getRoot()
    if hrp then ManualBase = hrp.Position; WindUI:Notify({ Title = "Base", Content = "Đã pin base!" }) end
end })
StealSec:Button({ Title = "Clear Base Pin", Callback = function() ManualBase = nil; WindUI:Notify({ Title = "Base", Content = "Đã xoá pin." }) end })

-- ============================================================
-- TAB 2: AUTO STEAL FREE
-- ============================================================
local FreeSec = Window:Tab({ Title = "Auto Steal Free", Icon = "solar:flash-bold", IconColor = PurpleColor, Border = true })
FreeSec:Section({ Title = "Auto Steal Free (RUN!! + Drop Detection)" })
FreeSec:Toggle({ Title = "Enable Auto Steal Free", Desc = "Khi bạn grab egg → walk TP về base", Value = false, Callback = function(v) UIState["AutoStealFree"] = v end })
FreeSec:Toggle({ Title = "Anti-Cheat Bypass", Value = false, Callback = function(v)
    if v then task.spawn(H.applyNewBypassAndSpeed)
    else pcall(function() if LocalPlayer.Character then LocalPlayer.Character:BreakJoints() end end) end
end })
FreeSec:Toggle({ Title = "Speed Changer", Value = false, Callback = function(v) UIState["SpeedChanger"] = v end })
FreeSec:Slider({ Title = "Walk Speed", Min = 16, Max = 500, Default = 60, Step = 1, Callback = function(v) SpeedConfig.Value = v end })

-- ============================================================
-- TAB 3: EGG HANDLING
-- ============================================================
local LifeSec = Window:Tab({ Title = "Egg Handling", Icon = "solar:box-bold", IconColor = PurpleColor, Border = true })
LifeSec:Section({ Title = "Auto Place" })
LifeSec:Toggle({ Title = "Auto Place Selected", Value = false, Callback = function(v) UIState["AutoPlaceSelected"] = v end })
LifeSec:Toggle({ Title = "Auto Place All", Value = false, Callback = function(v) UIState["AutoPlaceAll"] = v end })
LifeSec:Dropdown({ Title = "Lifecycle Rarities", Values = RARITIES, Multi = true, Value = {}, Callback = function(v) UIValues["LifecycleRarities"] = v end })
LifeSec:Dropdown({ Title = "Lifecycle Mutations", Values = MUTATIONS, Multi = true, Value = {}, Callback = function(v) UIValues["LifecycleMutations"] = v end })
LifeSec:Button({ Title = "Place Now", Callback = function() task.spawn(function() H.runAutoPlaceEggs(true) end) end })

LifeSec:Section({ Title = "Auto Hatch" })
LifeSec:Toggle({ Title = "Auto Hatch Ready", Value = false, Callback = function(v) UIState["AutoOpenReadyEggs"] = v end })
LifeSec:Button({ Title = "Hatch Now", Callback = function() task.spawn(function() H.runAutoOpenReadyEggs() end) end })

LifeSec:Section({ Title = "Auto Sell Eggs" })
LifeSec:Toggle({ Title = "Auto Sell Eggs", Value = false, Callback = function(v) UIState["AutoSellEggs"] = v end })
LifeSec:Dropdown({ Title = "Sell Rarities", Values = RARITIES, Multi = true, Value = {}, Callback = function(v) UIValues["SellEggRarities"] = v end })
LifeSec:Slider({ Title = "Sell Interval", Min = 1, Max = 120, Default = 8, Step = 1, Callback = function(v) UIValues["SellEggInterval"] = v end })

-- ============================================================
-- TAB 4: PETS
-- ============================================================
local PetsSec = Window:Tab({ Title = "Pets", Icon = "solar:cat-bold", IconColor = PurpleColor, Border = true })
PetsSec:Section({ Title = "Pets Overview" })
PetsSec:Toggle({ Title = "Auto Equip Best", Value = false, Callback = function(v) UIState["AutoEquipBest"] = v end })
PetsSec:Toggle({ Title = "Hide Own Pet Renders", Value = false, Callback = function(v) UIState["AutoDeleteOwnPets"] = v end })

PetsSec:Section({ Title = "Auto Fuse" })
PetsSec:Toggle({ Title = "Auto Fuse Pets", Value = false, Callback = function(v) UIState["AutoFusePets"] = v end })
PetsSec:Dropdown({ Title = "Fuse Rarities", Values = RARITIES, Multi = true, Value = {}, Callback = function(v) UIValues["FuseRarities"] = v end })
PetsSec:Dropdown({ Title = "Fuse Mutations", Values = MUTATIONS, Multi = true, Value = {}, Callback = function(v) UIValues["FuseMutations"] = v end })
PetsSec:Dropdown({ Title = "Pick Group By", Values = FUSE_TARGET_MODES, Value = "Highest Rarity", Callback = function(v) UIValues["FuseTarget"] = v end })
PetsSec:Toggle({ Title = "Never Fuse Mutated", Value = true, Callback = function(v) UIState["FuseKeepMutated"] = v end })
PetsSec:Toggle({ Title = "Never Fuse Equipped", Value = true, Callback = function(v) UIState["FuseKeepEquipped"] = v end })
PetsSec:Toggle({ Title = "Auto Complete Reveal", Value = true, Callback = function(v) UIState["FuseAutoReveal"] = v end })
PetsSec:Slider({ Title = "Max Scale to Fuse", Min = 0, Max = 10, Default = 10, Step = 0.1, Callback = function(v) UIValues["FuseMaxScale"] = v end })
PetsSec:Slider({ Title = "Keep Per Pet Type", Min = 0, Max = 20, Default = 0, Step = 1, Callback = function(v) UIValues["FuseKeepPerCategory"] = v end })
PetsSec:Slider({ Title = "Fuse Interval", Min = 1, Max = 120, Default = 8, Step = 1, Callback = function(v) UIValues["FuseInterval"] = v end })
PetsSec:Button({ Title = "Fuse Now", Callback = function() task.spawn(function() H.runAutoFusePets(true) end) end })

PetsSec:Section({ Title = "Auto Sell Pets" })
PetsSec:Toggle({ Title = "Auto Sell Pets", Value = false, Callback = function(v) UIState["AutoSellPets"] = v end })
PetsSec:Dropdown({ Title = "Sell Rarities", Values = RARITIES, Multi = true, Value = {}, Callback = function(v) UIValues["SellRarities"] = v end })
PetsSec:Dropdown({ Title = "Sell Mutations", Values = MUTATIONS, Multi = true, Value = {}, Callback = function(v) UIValues["SellMutations"] = v end })
PetsSec:Toggle({ Title = "Never Sell Mutated", Value = true, Callback = function(v) UIState["SellKeepMutated"] = v end })
PetsSec:Toggle({ Title = "Never Sell Equipped", Value = true, Callback = function(v) UIState["SellKeepEquipped"] = v end })
PetsSec:Slider({ Title = "Max Scale to Sell", Min = 0, Max = 10, Default = 10, Step = 0.1, Callback = function(v) UIValues["SellMaxScale"] = v end })
PetsSec:Slider({ Title = "Sell Interval", Min = 1, Max = 120, Default = 6, Step = 1, Callback = function(v) UIValues["SellInterval"] = v end })

-- ============================================================
-- TAB 5: VISUAL PETS
-- ============================================================
local VisSec = Window:Tab({ Title = "Visual Pets", Icon = "solar:star-bold", IconColor = PurpleColor, Border = true })
VisSec:Section({ Title = "Visual Pets (Client Only)" })
local visPetList = VisSec:Dropdown({ Title = "Pet", Values = {"Scanning..."}, Value = 1, Callback = function(v)
    local name = type(v) == "table" and next(v) or v
    if type(name) == "string" then UIValues["VisualPetSelected"] = name end
end })
VisSec:Button({ Title = "Refresh Pet List", Callback = function()
...
VisSec:Input({ Title = "Custom Name", Placeholder = "e.g. Dragon", Callback = function(v) UIValues["VisualPetCustom"] = v end })
VisSec:Slider({ Title = "Orbit Radius", Min = 2, Max = 20, Default = 4, Step = 1, Callback = function(v) UIValues["VisualPetRadius"] = v end })
VisSec:Slider({ Title = "Orbit Speed", Min = 0, Max = 10, Default = 1, Step = 1, Callback = function(v) UIValues["VisualPetSpeed"] = v end })
VisSec:Button({ Title = "Spawn Pet", Callback = function()
...
VisSec:Button({ Title = "Remove Last", Callback = function()
    local v = table.remove(VisualPetsSpawned); if v then pcall(function() v.model:Destroy() end) end
end })
VisSec:Button({ Title = "Remove All", Callback = function() H.clearVisualPets(); WindUI:Notify({ Title = "Visual Pets", Content = "Đã xoá hết." }) end })
...
-- TAB 6: PROGRESS
-- ============================================================
local ProgSec = Window:Tab({ Title = "Progress", Icon = "solar:graph-up-bold", IconColor = PurpleColor, Border = true })
ProgSec:Section({ Title = "Upgrades" })
ProgSec:Toggle({ Title = "Auto Buy Upgrades", Value = false, Callback = function(v) UIState["AutoUpgrades"] = v end })
ProgSec:Dropdown({ Title = "Upgrade Types", Values = UPGRADE_TYPES, Multi = true, Value = {"Base","Treadmill"}, Callback = function(v) UIValues["UpgradeTypes"] = v end })

ProgSec:Section({ Title = "Rewards" })
ProgSec:Toggle({ Title = "Auto Claim Index", Value = false, Callback = function(v) UIState["AutoClaimIndex"] = v end })
ProgSec:Toggle({ Title = "Auto Claim Group Reward", Value = false, Callback = function(v) UIState["AutoClaimGroupReward"] = v end })
ProgSec:Toggle({ Title = "Claim Offline Earnings", Value = false, Callback = function(v) UIState["AutoClaimOffline"] = v end })

ProgSec:Section({ Title = "Equipment" })
ProgSec:Toggle({ Title = "Auto Buy Trail", Value = false, Callback = function(v) UIState["AutoBuyTrail"] = v end })
ProgSec:Dropdown({ Title = "Trails", Values = TrailNames, Multi = true, Value = {}, Callback = function(v) UIValues["TrailWanted"] = v end })
ProgSec:Toggle({ Title = "Auto Equip Best Trail", Value = false, Callback = function(v) UIState["AutoEquipBestTrail"] = v end })
ProgSec:Toggle({ Title = "Auto Equip Best Gear", Value = false, Callback = function(v) UIState["AutoEquipBestGear"] = v end })

ProgSec:Section({ Title = "Training" })
ProgSec:Toggle({ Title = "Auto Treadmill Training", Value = false, Callback = function(v) UIState["AutoTreadmill"] = v end })

-- ============================================================
-- TAB 7: PLAYER
-- ============================================================
local PlrSec = Window:Tab({ Title = "Player", Icon = "solar:user-bold", IconColor = PurpleColor, Border = true })
PlrSec:Section({ Title = "ESP" })
PlrSec:Toggle({ Title = "World Egg ESP", Value = false, Callback = function(v) UIState["EspWorldEggs"] = v end })
PlrSec:Toggle({ Title = "Carried/Dropped Egg ESP", Value = false, Callback = function(v) UIState["EspCarriedEggs"] = v end })
PlrSec:Toggle({ Title = "Guard ESP", Value = false, Callback = function(v) UIState["EspGuards"] = v end })
PlrSec:Toggle({ Title = "Pet ESP", Value = false, Callback = function(v) UIState["EspPets"] = v end })
PlrSec:Toggle({ Title = "Player ESP", Value = false, Callback = function(v) UIState["EspPlayers"] = v end })
PlrSec:Toggle({ Title = "Machine ESP", Value = false, Callback = function(v) UIState["EspMachines"] = v end })
PlrSec:Toggle({ Title = "Plot ESP", Value = false, Callback = function(v) UIState["EspPlots"] = v end })
PlrSec:Slider({ Title = "Render Distance", Min = 100, Max = 6000, Default = 2000, Step = 50, Callback = function(v) UIValues["EspDistance"] = v end })

PlrSec:Section({ Title = "Movement" })
PlrSec:Toggle({ Title = "Walk Speed Override", Value = false, Callback = function(v) UIState["WalkSpeedEnabled"] = v end })
PlrSec:Slider({ Title = "Walk Speed", Min = 16, Max = 500, Default = 32, Step = 1, Callback = function(v) UIValues["WalkSpeed"] = v end })
PlrSec:Toggle({ Title = "Jump Power Override", Value = false, Callback = function(v) UIState["JumpPowerEnabled"] = v end })
PlrSec:Slider({ Title = "Jump Power", Min = 10, Max = 500, Default = 50, Step = 1, Callback = function(v) UIValues["JumpPower"] = v end })
PlrSec:Toggle({ Title = "Infinite Jump", Value = false, Callback = function(v) UIState["InfJump"] = v end })
PlrSec:Toggle({ Title = "NoClip", Value = false, Callback = function(v) UIState["NoClip"] = v end })
PlrSec:Toggle({ Title = "Fly", Value = false, Callback = function(v)
    UIState["Fly"] = v
    if not v then local hum = H.getHumanoid(); if hum then hum.PlatformStand = false end end
end })
PlrSec:Slider({ Title = "Fly Speed", Min = 10, Max = 400, Default = 60, Step = 1, Callback = function(v) UIValues["FlySpeed"] = v end })

PlrSec:Section({ Title = "Teleports" })
PlrSec:Dropdown({ Title = "Waypoint", Values = WaypointNames, Value = "Base", Callback = function(v) UIValues["WaypointTarget"] = v end })
PlrSec:Button({ Title = "Teleport to Waypoint", Callback = function()
    task.spawn(function()
...
PlrSec:Button({ Title = "Drop to Void", Callback = function() H.teleportToVoid(); WindUI:Notify({ Title = "Void", Content = "Đã thả xuống void." }) end })

-- ============================================================
-- TAB 8: SERVER HOP
-- ============================================================
local HopSec = Window:Tab({ Title = "Server Hop", Icon = "solar:refresh-bold", IconColor = PurpleColor, Border = true })
HopSec:Toggle({ Title = "Auto Server Hop", Value = false, Callback = function(v) UIState["AutoServerHop"] = v end })
HopSec:Dropdown({ Title = "Hop When", Values = ServerHopModes, Value = "No Matching Eggs", Callback = function(v) UIValues["HopMode"] = v end })
HopSec:Slider({ Title = "Wait Before Hop", Min = 1, Max = 200, Default = 15, Step = 1, Callback = function(v) UIValues["HopValue"] = v end })
HopSec:Button({ Title = "Hop Now", Callback = function() task.spawn(function() ServerHopRetryAfter = 0; H.serverHop("Manual") end) end })
HopSec:Button({ Title = "Rejoin Server", Callback = function() H.rejoinServer() end })

-- ============================================================
-- TAB 9: TASK PRIORITY
-- ============================================================
local PriSec = Window:Tab({ Title = "Task Priority", Icon = "solar:list-bold", IconColor = PurpleColor, Border = true })
for i, name in ipairs(PrioritySlotOptionNames) do
    PriSec:Dropdown({ Title = "Priority " .. i, Values = PriorityTaskNames, Value = PriorityTaskNames[i], Callback = function(v) UIValues[name] = v end })
end

-- ============================================================
-- TAB 10: ANTI FEATURES
-- ============================================================
local AntiSec = Window:Tab({ Title = "Anti Features", Icon = "solar:shield-check-bold", IconColor = PurpleColor, Border = true })
AntiSec:Section({ Title = "Anti-Ragdoll / Anti-Knock" })
AntiSec:Toggle({ Title = "Anti-Ragdoll (Chống Văng)", Value = true, Callback = function(v) PeteAntiRagdoll = v end })
AntiSec:Toggle({ Title = "Anti Trap (Xóa & Ẩn Bẫy)", Value = false, Callback = function(v)
    IsTrapImmune = v
    if v then H.applyTrapImmunity(); WindUI:Notify({ Title = "Anti Trap", Content = "Đã bật!" }) end
end })
AntiSec:Toggle({ Title = "Bỏ Qua Ấn Giữ (Instant Interact)", Value = false, Callback = function(v) H.setInstantHold(v) end })
AntiSec:Button({ Title = "Bật Fix Lag Đất Sét", Callback = function()
    H.enableFixLag()
...
-- TAB 11: SYSTEM
-- ============================================================
local SysSec = Window:Tab({ Title = "System", Icon = "solar:settings-bold", IconColor = PurpleColor, Border = true })
SysSec:Section({ Title = "Session" })
SysSec:Toggle({ Title = "Anti-AFK", Value = true, Callback = function(v) UIState["AntiAfk"] = v end })
SysSec:Toggle({ Title = "No Gameplay Paused", Value = true, Callback = function(v)
    UIState["AntiGameplayPause"] = v
    H.applyAntiGameplayPause(v)
end })
SysSec:Toggle({ Title = "Auto Reconnect", Value = false, Callback = function(v) UIState["AutoReconnect"] = v end })

SysSec:Section({ Title = "Performance" })
SysSec:Toggle({ Title = "FPS Boost", Value = false, Callback = function(v)
    UIState["FpsBoost"] = v
    if v then H.enableFpsBoost() else H.disableFpsBoost() end
end })
SysSec:Toggle({ Title = "Disable 3D Rendering", Value = false, Callback = function(v) H.applyRendering(v) end })
SysSec:Slider({ Title = "FPS Cap", Min = 15, Max = 360, Default = 60, Step = 1, Callback = function(v) H.applyFpsCap(v) end })

SysSec:Section({ Title = "Webhooks" })
SysSec:Toggle({ Title = "Enable Webhooks", Value = false, Callback = function(v) UIState["WebhookEnabled"] = v end })
SysSec:Input({ Title = "Webhook URL", Placeholder = "https://discord.com/api/webhooks/...", Callback = function(v) UIValues["WebhookUrl"] = v end })
SysSec:Input({ Title = "Ping User ID", Placeholder = "123456789012345678", Callback = function(v) UIValues["WebhookPingId"] = v end })
SysSec:Slider({ Title = "Summary Interval (min)", Min = 1, Max = 180, Default = 15, Step = 1, Callback = function(v) UIValues["WebhookInterval"] = v end })
SysSec:Toggle({ Title = "Disconnect Alerts", Value = false, Callback = function(v) UIState["WebhookDisconnectAlerts"] = v end })
SysSec:Button({ Title = "Send Summary Now", Callback = function() task.spawn(function() H.sendSummary() end) end })

SysSec:Section({ Title = "About" })
SysSec:Button({ Title = "Copy Discord Link", Callback = function()
    if setclipboard then setclipboard(DISCORD_LINK) end
    WindUI:Notify({ Title = "ZenHubX", Content = "Đã copy Discord link!" })
end })
SysSec:Button({ Title = "Unload Script", Callback = function()
    running = false
...
WindUI:Notify({ Title = "ZenHubX Ultimate", Content = "Đã load! Mở menu bằng nút bên trái." })