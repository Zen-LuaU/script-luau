--[[
    zen hubXsteal - Fixed Death Issue + Key Gate + Language Switch
    Based on Apex Hub -> zen hubXsteal
--]]
...
local TRANSLATIONS = {
    vi = {
        -- Key Gate
        gate_title = "zen hubXsteal - XÁC MINH KEY",
        gate_desc = "Vui lòng nhập KEY để sử dụng script.\nTham gia Discord để lấy KEY miễn phí!",
        gate_placeholder = "Nhập key tại đây...",
        gate_submit = "XÁC MINH",
        gate_join = "1. Vào Discord Lấy Key",
        gate_invalid = "KEY KHÔNG ĐÚNG! Vào Discord để lấy key: zenhubXsteal",
        gate_success = "XÁC MINH THÀNH CÔNG! Đang tải script...",
        -- Tabs
        tab_home = "Trang chủ", tab_farm = "Nông trại", tab_pets = "Thú cưng",
        tab_progress = "Tiến trình", tab_player = "Người chơi", tab_extras = "Tính năng thêm",
        tab_system = "Hệ thống",
        -- Sections
        sec_session = "Phiên làm việc", sec_account = "Tài khoản", sec_quick = "Hành động nhanh",
        sec_steal = "Trộm trứng", sec_egg_handling = "Xử lý trứng", sec_server = "Đổi server",
        sec_task = "Thứ tự nhiệm vụ", sec_pets = "Thú cưng", sec_fuse = "Auto Fuse",
        sec_sell_pet = "Auto Bán Pet", sec_upgrade = "Nâng cấp", sec_reward = "Phần thưởng",
        sec_equip = "Trang bị", sec_train = "Luyện tập", sec_esp = "ESP", sec_move = "Di chuyển",
        sec_tp = "Dịch chuyển", sec_combat = "Chiến đấu / Sinh tồn", sec_interact = "Tương tác",
        sec_speed = "Speed Changer", sec_perf = "Hiệu năng", sec_visual = "Visual Pets",
        sec_lang = "Ngôn ngữ", sec_webhook = "Webhook", sec_about = "Giới thiệu",
        sec_danger = "Vùng nguy hiểm",
        -- Language options
        lang_label = "Ngôn ngữ / Language",
        -- Common buttons
        btn_return = "Về Base", btn_place = "Đặt trứng", btn_hop = "Đổi server",
        btn_fuse = "Fuse ngay", btn_unload = "Tắt Script", btn_copy_discord = "Copy Discord",
        btn_send = "Gửi", btn_apply = "Áp dụng", btn_spawn = "Triệu hồi",
        btn_clear = "Xóa", btn_go = "Đi", btn_rejoin = "Vào lại", btn_copy_join = "Copy",
        -- Kill/extras
        toggle_anti_ragdoll = "Chống Văng / Anti-Ragdoll",
        toggle_anti_trap = "Chống Bẫy (Anti-Trap)",
        toggle_instant = "Bỏ Qua Ấn Giữ",
        toggle_free_steal = "Auto Trộm Free (Prompt)",
        toggle_speed_lock = "Khóa WalkSpeed",
    },
    en = {
        gate_title = "zen hubXsteal - KEY VERIFICATION",
        gate_desc = "Please enter your KEY to use the script.\nJoin Discord to get a free KEY!",
        gate_placeholder = "Enter key here...",
        gate_submit = "VERIFY",
        gate_join = "1. Join Discord For Key",
        gate_invalid = "INVALID KEY! Join Discord: zenhubXsteal",
        gate_success = "VERIFIED! Loading script...",
        tab_home = "Home", tab_farm = "Farm", tab_pets = "Pets",
        tab_progress = "Progress", tab_player = "Player", tab_extras = "Extras",
        tab_system = "System",
        sec_session = "Session", sec_account = "Account", sec_quick = "Quick Actions",
        sec_steal = "Steal Eggs", sec_egg_handling = "Egg Handling", sec_server = "Server Hop",
        sec_task = "Task Order", sec_pets = "Pets", sec_fuse = "Auto Fuse",
        sec_sell_pet = "Auto Sell Pets", sec_upgrade = "Upgrades", sec_reward = "Rewards",
        sec_equip = "Equipment", sec_train = "Training", sec_esp = "ESP", sec_move = "Movement",
        sec_tp = "Teleports", sec_combat = "Combat / Survival", sec_interact = "Interaction",
        sec_speed = "Speed Changer", sec_perf = "Performance", sec_visual = "Visual Pets",
        sec_lang = "Language", sec_webhook = "Webhooks", sec_about = "About",
        sec_danger = "Danger Zone",
        lang_label = "Language / Ngôn ngữ",
        btn_return = "Return to Base", btn_place = "Place Eggs", btn_hop = "Hop",
        btn_fuse = "Fuse Now", btn_unload = "Unload Script", btn_copy_discord = "Copy Discord",
        btn_send = "Send", btn_apply = "Apply", btn_spawn = "Spawn",
        btn_clear = "Clear", btn_go = "Go", btn_rejoin = "Rejoin", btn_copy_join = "Copy",
        toggle_anti_ragdoll = "Anti-Ragdoll / Anti-Knock",
        toggle_anti_trap = "Super Anti-Trap",
        toggle_instant = "Instant Interact (No Hold)",
        toggle_free_steal = "Auto Steal Free (Proximity)",
        toggle_speed_lock = "Enable WalkSpeed Lock",
    }
}
local function tr(key)
    local dict = TRANSLATIONS[CurrentLang] or TRANSLATIONS.vi
    return dict[key] or TRANSLATIONS.en[key] or key
end
H_tr = tr

-- ============================================================
-- KEY GATE - BẮT BUỘC NHẬP KEY
-- ============================================================
local VALID_KEYS = {
    ["zenhubXsteal"] = true,
}
...
local function CreateKeyGateUI()
    local gui = Instance.new("ScreenGui")
    gui.Name = "ZenXKeyGate"
    ...
    panel.Position = UDim2.new(0.5, -200, 0.5, -170)
    ...
    local ps = Instance.new("UIStroke"); ps.Thickness = 2; ps.Color = Color3.fromRGB(180, 80, 255); ps.Parent = panel
    ...
    title.Text = "zen hubXsteal - KEY"
    title.Parent = panel
    local tg = Instance.new("UIGradient")
    tg.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(200, 100, 255)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(140, 50, 230)),
    })
    tg.Parent = title
    ...
    desc.TextSize = 12
    desc.TextColor3 = Color3.fromRGB(200, 180, 220)
    desc.TextWrapped = true
    desc.Text = tr("gate_desc")
    desc.Parent = panel

    local keyBox = Instance.new("TextBox")
    keyBox.Position = UDim2.fromOffset(20, 118)
    keyBox.Size = UDim2.new(1, -40, 0, 40)
    keyBox.BackgroundColor3 = Color3.fromRGB(30, 20, 48)
    keyBox.BorderSizePixel = 0
    keyBox.Font = Enum.Font.GothamBold
    keyBox.TextSize = 14
    keyBox.TextColor3 = Color3.fromRGB(240, 220, 255)
    keyBox.PlaceholderColor3 = Color3.fromRGB(140, 120, 175)
    keyBox.PlaceholderText = tr("gate_placeholder")
    keyBox.Text = ""
    keyBox.ClearTextOnFocus = false
    keyBox.Parent = panel
    local kbc = Instance.new("UICorner"); kbc.CornerRadius = UDim.new(0, 8); kbc.Parent = keyBox
    local kbs = Instance.new("UIStroke"); kbs.Color = Color3.fromRGB(100, 50, 160); kbs.Thickness = 1.5; kbs.Parent = keyBox
    local kbp = Instance.new("UIPadding"); kbp.PaddingLeft = UDim.new(0, 12); kbp.PaddingRight = UDim.new(0, 12); kbp.Parent = keyBox

    local discordBtn = Instance.new("TextButton")
    discordBtn.Position = UDim2.fromOffset(20, 172)
    ...
    discordBtn.Text = tr("gate_join")
    discordBtn.Parent = panel
    local dbc = Instance.new("UICorner"); dbc.CornerRadius = UDim.new(0, 8); dbc.Parent = discordBtn

    local submitBtn = Instance.new("TextButton")
    submitBtn.Position = UDim2.fromOffset(20, 220)
    submitBtn.Size = UDim2.new(1, -40, 0, 44)
    submitBtn.BackgroundColor3 = Color3.fromRGB(170, 60, 255)
    submitBtn.BorderSizePixel = 0
    submitBtn.Font = Enum.Font.GothamBold
    submitBtn.TextSize = 15
    submitBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    submitBtn.Text = tr("gate_submit")
    submitBtn.Parent = panel
    local sbc = Instance.new("UICorner"); sbc.CornerRadius = UDim.new(0, 10); sbc.Parent = submitBtn

    local status = Instance.new("TextLabel")
    status.Position = UDim2.fromOffset(20, 272)
    status.Size = UDim2.new(1, -40, 0, 50)
    status.BackgroundTransparency = 1
    status.Font = Enum.Font.Gotham
    status.TextSize = 12
    status.TextColor3 = Color3.fromRGB(255, 100, 180)
    ...
    submitBtn.MouseButton1Click:Connect(function()
        local input = tostring(keyBox.Text or ""):gsub("%s+", "")
        if VALID_KEYS[input] then
            status.TextColor3 = Color3.fromRGB(150, 255, 150)
            status.Text = tr("gate_success")
            keyVerified = true
...
local keyGui = CreateKeyGateUI()

-- Chờ người dùng nhập key
while running and not keyVerified do
    task.wait(0.2)
end
if not keyVerified then return end

-- ============================================================
-- ANTI-CHEAT (kích hoạt sau khi xác minh)
-- ============================================================
pcall(function()
...
local LastCharacterSpawnAt = 0  -- FIX DEATH
...
function H.swapStealHumanoid()
    ...
    -- FIX: chỉ chạy khi nhân vật còn sống
...
-- ============================================================
-- FIX DEATH: Humanoid swap an toàn, chỉ 1 lần / mạng
-- ============================================================
function H.prepareStealHumanoid()
    ...
    -- Nếu nhân vật đang chết -> không swap
    if humanoid.Health <= 0 or humanoid:GetState() == Enum.HumanoidStateType.Dead then
        return nil
    end

    -- Chờ nhân vật mới spawn ổn định (tránh chết khi vừa respawn)
    if LastCharacterSpawnAt > 0 and tick() - LastCharacterSpawnAt < 1.2 then
        task.wait(1.2 - (tick() - LastCharacterSpawnAt))
    end

    -- Đã swap cho nhân vật này rồi -> không swap nữa
    if character:GetAttribute("ZenStealHumanoidReady") == true then
        return humanoid
    end
    ...
    local savedMaxHealth = humanoid.MaxHealth
    ...
    if newHumanoid then
        ...
        if newHumanoid.Health <= 0 then
            newHumanoid.Health = savedHealth > 0 and savedHealth or savedMaxHealth
        end
    end
    ...
    character:SetAttribute("ZenStealHumanoidReady", true)
    return newHumanoid
end
...
function H.canAutoSteal()
    ...
    if not hum or hum.Health <= 0 then return false end
    -- FIX: chờ 2s sau respawn để tránh chết
    if LastCharacterSpawnAt > 0 and tick() - LastCharacterSpawnAt < 2 then return false end
...
function H.stealEgg(slotEgg)
    ...
    if not hum or hum.Health <= 0 then return false end
    if not H.swapStealHumanoid() then return false end
...
-- ============================================================
-- EXTRA FEATURES (từ 2 file đầu, không dính egg farm)
-- ============================================================

-- Anti-Ragdoll / Anti-Knock (FIXED DEATH)
...
RunService.Stepped:Connect(function()
    ...
    -- FIX DEATH: bỏ qua hoàn toàn khi nhân vật đã chết
    if humanoid.Health <= 0 then return end
    local state = humanoid:GetState()
    if state == Enum.HumanoidStateType.Dead then return end
    -- FIX DEATH: chỉ set khi cần
...
-- Auto Steal Free (Proximity Prompt)
local PromptCache = {}
...
-- Visual Pets
local VisualPets = { spawned = {}, orbitRadius = 4, orbitSpeed = 0.8, petNames = {}, inputName = "" }
...
-- ============================================================
-- STANDALONE UI (tím neon) + LANGUAGE SYSTEM
-- ============================================================
local UI = {}
UI.__state = {}
UI.__callbacks = {}
...
UI.__activeTab = nil
UI.__connections = {}
UI.__textRegistry = {}  -- { [instance] = { vi = "...", en = "..." } }
...
-- Register text element so it can be retranslated
local function registerText(instance, viText, enText)
    UI.__textRegistry[instance] = { vi = viText, en = enText }
    instance.Text = CurrentLang == "vi" and viText or enText
end

local function applyLanguage()
    for instance, texts in pairs(UI.__textRegistry) do
        if instance and instance.Parent then
            instance.Text = CurrentLang == "vi" and texts.vi or texts.en
        end
    end
end
...
discordBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard(DISCORD_LINK) end)
    H.notify("zen hubXsteal", "Discord link copied", "Success", 3)
end)
...
local function makeTabButton(tabId, tabVi, tabEn)
...
function UI.AddTab(opts)
    local id = opts.Id
    local btn, setActive = makeTabButton(id, opts.Title or id, opts.TitleEN or opts.Title or id)
...
function UI.AddSection(tab, opts)
    ...
    sectionPad.PaddingTop = UDim.new(0, 10); sectionPad.PaddingBottom = UDim.new(0, 10)
    sectionPad.PaddingLeft = UDim.new(0, 12); sectionPad.PaddingRight = UDim.new(0, 12)
    ...
    if opts.Title then
        ...
        h.Text = opts.Title
        h.Parent = section
        registerText(h, opts.Title, opts.TitleEN or opts.Title)
    end
    if opts.Description then
        ...
        d.Parent = section
        registerText(d, opts.Description, opts.DescriptionEN or opts.Description)
    end
    return section
end
...
function UI.AddToggle(parent, opts)
    ...
    name.Parent = row
    registerText(name, opts.Title or "Toggle", opts.TitleEN or opts.Title or "Toggle")
    if opts.Description then
        local d = Instance.new("TextLabel")
        d.BackgroundTransparency = 1
...
function UI.AddSlider(parent, opts)
    ...
    label.Parent = row
    registerText(label, opts.Title or "Slider", opts.TitleEN or opts.Title or "Slider")
...
function UI.AddDropdown(parent, opts)
    ...
    label.Parent = row
    registerText(label, opts.Title or "Dropdown", opts.TitleEN or opts.Title or "Dropdown")
    local valueLabel = Instance.new("TextLabel")
    valueLabel.BackgroundTransparency = 1
    ...
    for _, option in ipairs(opts.Options or {}) do
        ...
        local function refresh()
            local active
            if isMulti then active = selected[option] == true else active = (selected == option) end
...
function UI.AddButton(parent, opts)
    ...
    btn.TextXAlignment = Enum.TextXAlignment.Left
    btn.Parent = row
    registerText(btn, opts.Title or "Button", opts.TitleEN or opts.Title or "Button")
...
function UI.AddParagraph(parent, opts)
    ...
    t.Parent = row
    registerText(t, opts.Title or "", opts.TitleEN or opts.Title or "")
    ...
    body.Parent = row
    registerText(body, opts.Content or "", opts.ContentEN or opts.Content or "")
    return row
end
...
function UI.AddDivider(parent, opts)
    ...
    if opts and opts.Title then
        ...
        lbl.Text = opts.Title
        lbl.Parent = holder
        registerText(lbl, opts.Title, opts.TitleEN or opts.Title)
    end
    return holder
end
...
function UI.AddStatus(parent, opts)
    ...
    name.Parent = row
    registerText(name, opts.Title or "Status", opts.TitleEN or opts.Title or "Status")
...
function UI.AddInput(parent, opts)
    ...
    label.Parent = row
    registerText(label, opts.Title or "Input", opts.TitleEN or opts.Title or "Input")
...
do
    -- HOME
    local HomeTab = UI.AddTab({ Id = "home", Title = tr("tab_home"), TitleEN = "Home" })
    local SessionSec = UI.AddSection(HomeTab, { Title = tr("sec_session"), TitleEN = "Session" })
    local AccountSec = UI.AddSection(HomeTab, { Title = tr("sec_account"), TitleEN = "Account" })
    local QuickSec = UI.AddSection(HomeTab, { Title = tr("sec_quick"), TitleEN = "Quick Actions" })

    refs.statusRow = UI.AddStatus(SessionSec, { Title = "Automation", TitleEN = "Automation", Value = "Ready" })
    refs.statusRow:SetStatus("Success")
    refs.stolenRow = UI.AddStatus(SessionSec, { Title = "Stolen Eggs", TitleEN = "Stolen Eggs", Value = "0" })
    refs.carryingRow = UI.AddStatus(SessionSec, { Title = "Carrying Egg", TitleEN = "Carrying Egg", Value = "No" })
    refs.runtimeRow = UI.AddStatus(SessionSec, { Title = "Runtime", TitleEN = "Runtime", Value = "0m" })
    UI.AddStatus(SessionSec, { Title = "Server", TitleEN = "Server", Value = DisplayJobId })

    refs.inventoryProgress = UI.AddStatus(AccountSec, { Title = "Egg Inventory", TitleEN = "Egg Inventory", Value = tostring(H.eggInventoryCount()) })
    refs.moneyRow = UI.AddStatus(AccountSec, { Title = "Money", TitleEN = "Money", Value = "0" })
    refs.speedRow = UI.AddStatus(AccountSec, { Title = "Speed Power", TitleEN = "Speed Power", Value = "0" })
    refs.rebirthRow = UI.AddStatus(AccountSec, { Title = "Rebirths", TitleEN = "Rebirths", Value = "0" })
    refs.petsOwnedRow = UI.AddStatus(AccountSec, { Title = "Pets Owned", TitleEN = "Pets Owned", Value = "0" })

    UI.AddButton(QuickSec, { Title = tr("btn_return"), TitleEN = "Return to Base", Text = "Return", Callback = function()
        task.spawn(function()
    ...
    UI.AddButton(QuickSec, { Title = tr("btn_place"), TitleEN = "Place Eggs", Text = "Place", Callback = function()
        task.spawn(function() H.runAutoPlaceEggs(true) end)
    end })
    UI.AddButton(QuickSec, { Title = tr("btn_hop"), TitleEN = "Server Hop", Text = "Hop", Callback = function()
        task.spawn(function() ServerHopRetryAfter = 0; H.serverHop("Manual") end)
    end })
    UI.AddButton(QuickSec, { Title = tr("btn_fuse"), TitleEN = "Fuse Now", Text = "Fuse", Callback = function()
        task.spawn(function() H.runAutoFusePets(true) end)
    end })

    -- FARM
    local FarmTab = UI.AddTab({ Id = "farm", Title = tr("tab_farm"), TitleEN = "Farm" })
    local StealSec = UI.AddSection(FarmTab, { Title = tr("sec_steal"), TitleEN = "Steal Eggs" })
    local LifeSec = UI.AddSection(FarmTab, { Title = tr("sec_egg_handling"), TitleEN = "Egg Handling" })
    local ServerSec = UI.AddSection(FarmTab, { Title = tr("sec_server"), TitleEN = "Server Hop" })
    local PrioritySec = UI.AddSection(FarmTab, { Title = tr("sec_task"), TitleEN = "Task Order" })

    UI.AddToggle(StealSec, { Id = "AutoStealSelected", Title = "Auto Steal Selected", TitleEN = "Auto Steal Selected", Default = false })
    UI.AddToggle(StealSec, { Id = "AutoStealAll", Title = "Auto Steal All", TitleEN = "Auto Steal All", Default = false })
    UI.AddToggle(StealSec, { Id = "StealBigEggs", Title = "Steal Big Eggs", TitleEN = "Steal Big Eggs", Default = false })
    UI.AddDropdown(StealSec, { Id = "StealZones", Title = "Areas", TitleEN = "Areas", Options = AreaNames, Multi = true, Default = {} })
    UI.AddDropdown(StealSec, { Id = "StealRarities", Title = "Rarities", TitleEN = "Rarities", Options = RARITIES, Multi = true, Default = {} })
    UI.AddDropdown(StealSec, { Id = "StealMutations", Title = "Mutations", TitleEN = "Mutations", Options = MUTATIONS, Multi = true, Default = {} })
    UI.AddDropdown(StealSec, { Id = "StealPriority", Title = "Target Priority", TitleEN = "Target Priority", Options = STEAL_PRIORITIES, Default = "Rarest" })
    UI.AddSlider(StealSec, { Id = "StealBigEggScale", Title = "Minimum Big Egg Size", TitleEN = "Minimum Big Egg Size", Min = 1, Max = 50, Default = 1.5, Step = 0.1, Suffix = "x" })
    UI.AddToggle(StealSec, { Id = "AutoReturn", Title = "Auto Return to Base", TitleEN = "Auto Return to Base", Default = true })
    UI.AddToggle(StealSec, { Id = "AutoDropEgg", Title = "Auto Drop Held Egg", TitleEN = "Auto Drop Held Egg", Default = false })

    UI.AddToggle(LifeSec, { Id = "AutoPlaceSelected", Title = "Auto Place Selected", TitleEN = "Auto Place Selected", Default = false })
    UI.AddToggle(LifeSec, { Id = "AutoPlaceAll", Title = "Auto Place All", TitleEN = "Auto Place All", Default = false })
    UI.AddToggle(LifeSec, { Id = "AutoOpenReadyEggs", Title = "Auto Hatch Ready", TitleEN = "Auto Hatch Ready", Default = false })
    UI.AddDropdown(LifeSec, { Id = "LifecycleRarities", Title = "Lifecycle Rarities", TitleEN = "Lifecycle Rarities", Options = RARITIES, Multi = true, Default = {} })
    UI.AddDropdown(LifeSec, { Id = "LifecycleMutations", Title = "Lifecycle Mutations", TitleEN = "Lifecycle Mutations", Options = MUTATIONS, Multi = true, Default = {} })
    UI.AddToggle(LifeSec, { Id = "AutoSellEggs", Title = "Auto Sell Eggs", TitleEN = "Auto Sell Eggs", Default = false })
    UI.AddDropdown(LifeSec, { Id = "SellEggRarities", Title = "Sell Rarities", TitleEN = "Sell Rarities", Options = RARITIES, Multi = true, Default = {} })
    UI.AddSlider(LifeSec, { Id = "SellEggInterval", Title = "Sell Interval", TitleEN = "Sell Interval", Min = 1, Max = 120, Default = 8, Step = 1, Suffix = " s" })

    UI.AddToggle(ServerSec, { Id = "AutoServerHop", Title = "Auto Server Hop", TitleEN = "Auto Server Hop", Default = false })
    UI.AddDropdown(ServerSec, { Id = "HopMode", Title = "Hop When", TitleEN = "Hop When", Options = ServerHopModes, Default = "No Matching Eggs" })
    UI.AddSlider(ServerSec, { Id = "HopValue", Title = "Wait Before Hop", TitleEN = "Wait Before Hop", Min = 1, Max = 200, Default = 15, Step = 1 })
    UI.AddButton(ServerSec, { Title = "Hop Now", TitleEN = "Hop Now", Text = "Hop", Callback = function()
        task.spawn(function() ServerHopRetryAfter = 0; H.serverHop("Manual") end)
    end })

    for i, name in ipairs(PrioritySlotOptionNames) do
        UI.AddDropdown(PrioritySec, { Id = name, Title = "Priority " .. i, TitleEN = "Priority " .. i, Options = PriorityTaskNames, Default = PriorityTaskNames[i] })
    end

    -- PETS
    local PetsTab = UI.AddTab({ Id = "pets", Title = tr("tab_pets"), TitleEN = "Pets" })
    local PetsOverview = UI.AddSection(PetsTab, { Title = tr("sec_pets"), TitleEN = "Pets" })
    local FuseSec = UI.AddSection(PetsTab, { Title = tr("sec_fuse"), TitleEN = "Auto Fuse" })
    local SellPetSec = UI.AddSection(PetsTab, { Title = tr("sec_sell_pet"), TitleEN = "Auto Sell Pets" })

    UI.AddToggle(PetsOverview, { Id = "AutoEquipBest", Title = "Auto Equip Best Pets", TitleEN = "Auto Equip Best Pets", Default = false })
    UI.AddToggle(PetsOverview, { Id = "AutoDeleteOwnPets", Title = "Hide Own Pet Renders", TitleEN = "Hide Own Pet Renders", Default = false })
    UI.AddToggle(FuseSec, { Id = "AutoFusePets", Title = "Auto Fuse Pets", TitleEN = "Auto Fuse Pets", Default = false })
    UI.AddDropdown(FuseSec, { Id = "FuseRarities", Title = "Fuse Rarities", TitleEN = "Fuse Rarities", Options = RARITIES, Multi = true, Default = {} })
    UI.AddDropdown(FuseSec, { Id = "FuseMutations", Title = "Fuse Mutations", TitleEN = "Fuse Mutations", Options = MUTATIONS, Multi = true, Default = {} })
    UI.AddDropdown(FuseSec, { Id = "FuseTarget", Title = "Pick Group By", TitleEN = "Pick Group By", Options = FUSE_TARGET_MODES, Default = "Highest Rarity" })
    UI.AddToggle(FuseSec, { Id = "FuseKeepMutated", Title = "Never Fuse Mutated", TitleEN = "Never Fuse Mutated", Default = true })
    UI.AddToggle(FuseSec, { Id = "FuseKeepEquipped", Title = "Never Fuse Equipped", TitleEN = "Never Fuse Equipped", Default = true })
    UI.AddToggle(FuseSec, { Id = "FuseAutoReveal", Title = "Auto Complete Reveal", TitleEN = "Auto Complete Reveal", Default = true })
    UI.AddSlider(FuseSec, { Id = "FuseMaxScale", Title = "Maximum Scale to Fuse", TitleEN = "Maximum Scale to Fuse", Min = 0, Max = 10, Default = 10, Step = 0.1 })
    UI.AddSlider(FuseSec, { Id = "FuseKeepPerCategory", Title = "Keep Per Pet Type", TitleEN = "Keep Per Pet Type", Min = 0, Max = 20, Default = 0, Step = 1 })
    UI.AddSlider(FuseSec, { Id = "FuseInterval", Title = "Fuse Interval", TitleEN = "Fuse Interval", Min = 1, Max = 120, Default = 8, Step = 1, Suffix = " s" })
    UI.AddButton(FuseSec, { Title = "Fuse Now", TitleEN = "Fuse Now", Text = "Fuse", Callback = function() task.spawn(function() H.runAutoFusePets(true) end) end })

    UI.AddToggle(SellPetSec, { Id = "AutoSellPets", Title = "Auto Sell Pets", TitleEN = "Auto Sell Pets", Default = false })
    UI.AddDropdown(SellPetSec, { Id = "SellRarities", Title = "Sell Rarities", TitleEN = "Sell Rarities", Options = RARITIES, Multi = true, Default = {} })
    UI.AddDropdown(SellPetSec, { Id = "SellMutations", Title = "Sell Mutations", TitleEN = "Sell Mutations", Options = MUTATIONS, Multi = true, Default = {} })
    UI.AddToggle(SellPetSec, { Id = "SellKeepMutated", Title = "Never Sell Mutated", TitleEN = "Never Sell Mutated", Default = true })
    UI.AddToggle(SellPetSec, { Id = "SellKeepEquipped", Title = "Never Sell Equipped", TitleEN = "Never Sell Equipped", Default = true })
    UI.AddSlider(SellPetSec, { Id = "SellMaxScale", Title = "Maximum Scale to Sell", TitleEN = "Maximum Scale to Sell", Min = 0, Max = 10, Default = 10, Step = 0.1 })
    UI.AddSlider(SellPetSec, { Id = "SellInterval", Title = "Sell Interval", TitleEN = "Sell Interval", Min = 1, Max = 120, Default = 6, Step = 1, Suffix = " s" })

    -- PROGRESS
    local ProgressTab = UI.AddTab({ Id = "progress", Title = tr("tab_progress"), TitleEN = "Progress" })
    local UpgradesSec = UI.AddSection(ProgressTab, { Title = tr("sec_upgrade"), TitleEN = "Upgrades" })
    local RewardsSec = UI.AddSection(ProgressTab, { Title = tr("sec_reward"), TitleEN = "Rewards" })
    local EquipmentSec = UI.AddSection(ProgressTab, { Title = tr("sec_equip"), TitleEN = "Equipment" })
    local TrainingSec = UI.AddSection(ProgressTab, { Title = tr("sec_train"), TitleEN = "Training" })
    UI.AddToggle(UpgradesSec, { Id = "AutoUpgrades", Title = "Auto Buy Upgrades", TitleEN = "Auto Buy Upgrades", Default = false })
    UI.AddDropdown(UpgradesSec, { Id = "UpgradeTypes", Title = "Upgrade Types", TitleEN = "Upgrade Types", Options = UPGRADE_TYPES, Multi = true, Default = { "Base", "Treadmill" } })
    UI.AddToggle(RewardsSec, { Id = "AutoClaimIndex", Title = "Auto Claim Index", TitleEN = "Auto Claim Index", Default = false })
    UI.AddToggle(RewardsSec, { Id = "AutoClaimGroupReward", Title = "Auto Claim Group Reward", TitleEN = "Auto Claim Group Reward", Default = false })
    UI.AddToggle(RewardsSec, { Id = "AutoClaimOffline", Title = "Claim Offline Earnings", TitleEN = "Claim Offline Earnings", Default = false })
    UI.AddToggle(EquipmentSec, { Id = "AutoBuyTrail", Title = "Auto Buy Trail", TitleEN = "Auto Buy Trail", Default = false })
    UI.AddDropdown(EquipmentSec, { Id = "TrailWanted", Title = "Trails", TitleEN = "Trails", Options = TrailNames, Multi = true, Default = {} })
    UI.AddToggle(EquipmentSec, { Id = "AutoEquipBestTrail", Title = "Auto Equip Best Trail", TitleEN = "Auto Equip Best Trail", Default = false })
    UI.AddToggle(EquipmentSec, { Id = "AutoEquipBestGear", Title = "Auto Equip Best Gear", TitleEN = "Auto Equip Best Gear", Default = false })
    UI.AddToggle(TrainingSec, { Id = "AutoTreadmill", Title = "Auto Treadmill Training", TitleEN = "Auto Treadmill Training", Default = false })

    -- PLAYER
    local PlayerTab = UI.AddTab({ Id = "player", Title = tr("tab_player"), TitleEN = "Player" })
    local EspSec = UI.AddSection(PlayerTab, { Title = tr("sec_esp"), TitleEN = "ESP" })
    local MoveSec = UI.AddSection(PlayerTab, { Title = tr("sec_move"), TitleEN = "Movement" })
    local TeleportSec = UI.AddSection(PlayerTab, { Title = tr("sec_tp"), TitleEN = "Teleports" })
    UI.AddToggle(EspSec, { Id = "EspWorldEggs", Title = "World Egg ESP", TitleEN = "World Egg ESP", Default = false })
    UI.AddToggle(EspSec, { Id = "EspCarriedEggs", Title = "Carried and Dropped Egg ESP", TitleEN = "Carried and Dropped Egg ESP", Default = false })
    UI.AddToggle(EspSec, { Id = "EspGuards", Title = "Guard ESP", TitleEN = "Guard ESP", Default = false })
    UI.AddToggle(EspSec, { Id = "EspPets", Title = "Pet ESP", TitleEN = "Pet ESP", Default = false })
    UI.AddToggle(EspSec, { Id = "EspPlayers", Title = "Player ESP", TitleEN = "Player ESP", Default = false })
    UI.AddToggle(EspSec, { Id = "EspMachines", Title = "Machine ESP", TitleEN = "Machine ESP", Default = false })
    UI.AddToggle(EspSec, { Id = "EspPlots", Title = "Plot ESP", TitleEN = "Plot ESP", Default = false })
    UI.AddSlider(EspSec, { Id = "EspDistance", Title = "Render Distance", TitleEN = "Render Distance", Min = 100, Max = 6000, Default = 2000, Step = 50, Suffix = " studs" })
    UI.AddToggle(MoveSec, { Id = "WalkSpeedEnabled", Title = "Walk Speed Override", TitleEN = "Walk Speed Override", Default = false })
    UI.AddSlider(MoveSec, { Id = "WalkSpeed", Title = "Walk Speed", TitleEN = "Walk Speed", Min = 16, Max = 500, Default = 32, Step = 1 })
    UI.AddToggle(MoveSec, { Id = "JumpPowerEnabled", Title = "Jump Power Override", TitleEN = "Jump Power Override", Default = false })
    UI.AddSlider(MoveSec, { Id = "JumpPower", Title = "Jump Power", TitleEN = "Jump Power", Min = 10, Max = 500, Default = 50, Step = 1 })
    UI.AddToggle(MoveSec, { Id = "InfJump", Title = "Infinite Jump", TitleEN = "Infinite Jump", Default = false })
    UI.AddToggle(MoveSec, { Id = "NoClip", Title = "NoClip", TitleEN = "NoClip", Default = false })
    UI.AddToggle(MoveSec, { Id = "Fly", Title = "Fly", TitleEN = "Fly", Default = false, Callback = function(v)
        if not v then local h = H.getHumanoid(); if h then h.PlatformStand = false end end
    end })
    UI.AddSlider(MoveSec, { Id = "FlySpeed", Title = "Fly Speed", TitleEN = "Fly Speed", Min = 10, Max = 400, Default = 60, Step = 1 })
    UI.AddDropdown(TeleportSec, { Id = "WaypointTarget", Title = "Waypoint", TitleEN = "Waypoint", Options = WaypointNames, Default = "Base" })
    UI.AddButton(TeleportSec, { Title = "Teleport to Waypoint", TitleEN = "Teleport to Waypoint", Text = "Go", Callback = function()
        task.spawn(function()
            local position = H.resolveWaypoint(H.optionValue("WaypointTarget", "Base"))
            if not position then H.notify("Waypoint", "Unavailable", "Warning", 3); return end
            if not H.bypassMoveTo(position, nil, BYPASS_SPEED) then H.notify("Waypoint", "Failed", "Error", 3) end
        end)
    end })

    -- EXTRAS
    local ExtraTab = UI.AddTab({ Id = "extras", Title = tr("tab_extras"), TitleEN = "Extras" })

    local CombatSec = UI.AddSection(ExtraTab, { Title = tr("sec_combat"), TitleEN = "Combat / Survival" })
    UI.AddToggle(CombatSec, { Id = "AntiRagdoll", Title = tr("toggle_anti_ragdoll"), TitleEN = "Anti-Ragdoll / Anti-Knock", Default = true,
        Callback = function(v) PeteSettings.AntiRagdoll = v end })
    UI.AddDropdown(CombatSec, { Id = "AntiRagdollMode", Title = "Camera Mode", TitleEN = "Camera Mode",
    ...
    UI.AddToggle(CombatSec, { Id = "AntiTrap", Title = tr("toggle_anti_trap"), TitleEN = "Super Anti-Trap", Default = false,
        Callback = function(v) H.setTrapImmune(v) end })

    local InteractSec = UI.AddSection(ExtraTab, { Title = tr("sec_interact"), TitleEN = "Interaction" })
    UI.AddToggle(InteractSec, { Id = "InstantHold", Title = tr("toggle_instant"), TitleEN = "Instant Interact (No Hold)", Default = false,
        Callback = function(v) H.setInstantHold(v) end })
    UI.AddToggle(InteractSec, { Id = "FreeAutoSteal", Title = tr("toggle_free_steal"), TitleEN = "Auto Steal Free (Proximity)", Default = false,
        Callback = function(v) FreeAutoSteal.Enabled = v end })

    local SpeedSec = UI.AddSection(ExtraTab, { Title = tr("sec_speed"), TitleEN = "Speed Changer" })
    UI.AddToggle(SpeedSec, { Id = "SpeedChanger", Title = tr("toggle_speed_lock"), TitleEN = "Enable WalkSpeed Lock", Default = false,
        Callback = function(v) SpeedConfig.Enabled = v end })
    UI.AddSlider(SpeedSec, { Id = "SpeedValue", Title = "WalkSpeed Value", TitleEN = "WalkSpeed Value", Min = 16, Max = 900, Default = 300, Step = 10,
        Callback = function(v) SpeedConfig.Value = v end })

    local PerfSec = UI.AddSection(ExtraTab, { Title = tr("sec_perf"), TitleEN = "Performance" })
    UI.AddButton(PerfSec, { Title = "Fix Lag (Clay + Remove Foliage)", TitleEN = "Fix Lag (Clay + Remove Foliage)", Text = "Apply",
        Callback = function() H.enableFixLag(); H.notify("Fix Lag", "Applied", "Success", 3) end })

    local VisualSec = UI.AddSection(ExtraTab, { Title = tr("sec_visual"), TitleEN = "Visual Pets (Client Only)" })
    UI.AddInput(VisualSec, { Id = "VisualPetName", Title = "Pet Name", TitleEN = "Pet Name", Placeholder = "Dragon", Default = "" })
    UI.AddSlider(VisualSec, { Id = "VisualPetRadius", Title = "Orbit Radius", TitleEN = "Orbit Radius", Min = 2, Max = 20, Default = 4, Step = 1,
        Callback = function(v) VisualPets.orbitRadius = v end })
    UI.AddSlider(VisualSec, { Id = "VisualPetSpeed", Title = "Orbit Speed", TitleEN = "Orbit Speed", Min = 0, Max = 10, Default = 4, Step = 1,
        Callback = function(v) VisualPets.orbitSpeed = v * 0.2 end })
    UI.AddButton(VisualSec, { Title = "Spawn Visual Pet", TitleEN = "Spawn Visual Pet", Text = "Spawn", Callback = function()
        local name = H.optionValue("VisualPetName", "")
        if name == "" then H.notify("Visual Pets", "Enter a pet name", "Warning", 3); return end
        H.spawnVisualPet(name)
    end })
    UI.AddButton(VisualSec, { Title = "Clear All Visual Pets", TitleEN = "Clear All Visual Pets", Text = "Clear", Callback = function() H.clearVisualPets() end })

    -- SYSTEM
    local SysTab = UI.AddTab({ Id = "system", Title = tr("tab_system"), TitleEN = "System" })
    local LangSec = UI.AddSection(SysTab, { Title = tr("sec_lang"), TitleEN = "Language" })
    UI.AddDropdown(LangSec, { Id = "Language", Title = tr("lang_label"), TitleEN = "Language / Ngôn ngữ",
        Options = { "Tiếng Việt", "English" },
        Default = CurrentLang == "vi" and "Tiếng Việt" or "English",
        Callback = function(v)
            if v == "English" then CurrentLang = "en" else CurrentLang = "vi" end
            applyLanguage()
            H.notify("zen hubXsteal", CurrentLang == "vi" and "Đã đổi sang Tiếng Việt" or "Changed to English", "Success", 3)
        end })

    local SessionSysSec = UI.AddSection(SysTab, { Title = tr("sec_session"), TitleEN = "Session" })
    local PerformanceSec = UI.AddSection(SysTab, { Title = tr("sec_perf"), TitleEN = "Performance" })
    local WebhookSec = UI.AddSection(SysTab, { Title = tr("sec_webhook"), TitleEN = "Webhooks" })
    local AboutSec = UI.AddSection(SysTab, { Title = tr("sec_about"), TitleEN = "About" })
    UI.AddToggle(SessionSysSec, { Id = "AntiAfk", Title = "Anti-AFK", TitleEN = "Anti-AFK", Default = true })
    UI.AddToggle(SessionSysSec, { Id = "AntiGameplayPause", Title = "No Gameplay Paused", TitleEN = "No Gameplay Paused", Default = true,
        Callback = function(v) H.applyAntiGameplayPause(v) end })
    UI.AddToggle(SessionSysSec, { Id = "AutoReconnect", Title = "Auto Reconnect", TitleEN = "Auto Reconnect", Default = false })
    UI.AddButton(SessionSysSec, { Title = tr("btn_rejoin"), TitleEN = "Rejoin Server", Text = "Rejoin", Callback = function() H.rejoinServer() end })
    UI.AddToggle(PerformanceSec, { Id = "FpsBoost", Title = "FPS Boost", TitleEN = "FPS Boost", Default = false,
        Callback = function(v) if v then H.enableFpsBoost() else H.disableFpsBoost() end end })
    UI.AddSlider(PerformanceSec, { Id = "FpsCap", Title = "FPS Cap", TitleEN = "FPS Cap", Min = 15, Max = 360, Default = 60, Step = 1, Suffix = " fps",
        Callback = function(v) H.applyFpsCap(v) end })
    UI.AddToggle(WebhookSec, { Id = "WebhookEnabled", Title = "Enable Webhooks", TitleEN = "Enable Webhooks", Default = false })
    UI.AddInput(WebhookSec, { Id = "WebhookUrl", Title = "Webhook URL", TitleEN = "Webhook URL", Placeholder = "https://discord.com/api/webhooks/...", Default = "" })
    UI.AddSlider(WebhookSec, { Id = "WebhookInterval", Title = "Summary Interval", TitleEN = "Summary Interval", Min = 1, Max = 180, Default = 15, Step = 1, Suffix = " min" })
    UI.AddButton(WebhookSec, { Title = tr("btn_send"), TitleEN = "Send Summary Now", Text = "Send", Callback = function()
        task.spawn(function()
            local sent = H.sendSummary()
            H.notify("Webhook", sent and "Sent" or "Failed", sent and "Success" or "Error", 3)
        end)
    end })
    UI.AddParagraph(AboutSec, { Title = "Script Dev", TitleEN = "Script Dev", Content = "zen hubXsteal" })
    UI.AddParagraph(AboutSec, { Title = "Discord", TitleEN = "Discord", Content = DISCORD_LINK })
    UI.AddButton(AboutSec, { Title = tr("btn_copy_discord"), TitleEN = "Copy Discord Link", Text = "Copy", Callback = function()
        pcall(function() setclipboard(DISCORD_LINK) end)
        H.notify("Copied", "Discord link copied", "Success", 3)
    end })
    UI.AddDivider(AboutSec, { Title = tr("sec_danger"), TitleEN = "Danger Zone" })
    UI.AddButton(AboutSec, { Title = tr("btn_unload"), TitleEN = "Unload Script", Text = "Unload", Callback = function() H.unload() end })
end
...
local function refreshHomeDashboard()
    ...
    pcall(function()
        if refs.carryingRow then
            refs.carryingRow:SetValue(IsCarryingEgg and "Yes" or "No")
...
-- FIX DEATH: theo dõi thời điểm spawn nhân vật, clear flag humanoid swap
H.track(LocalPlayer.CharacterAdded:Connect(function(char)
    if not running then return end
    LastCharacterSpawnAt = tick()
    pcall(function() char:SetAttribute("ZenStealHumanoidReady", nil) end)
    -- Reset trạng thái carry khi respawn
    IsCarryingEgg = false
...
local AutomationJobs = {
    ["Auto Steal Egg"] = { Ready = H.canAutoSteal, Run = H.runAutoSteal, Interval = 0.4 },
    ...
    if schedulerDue("dashboard", 2) then
        ...
        if hum and hum.Health > 0 then
            if H.isOn("WalkSpeedEnabled") then
                hum.WalkSpeed = tonumber(H.optionValue("WalkSpeed", 32)) or 32
            end
            if H.isOn("JumpPowerEnabled") then
                hum.UseJumpPower = true
                hum.JumpPower = tonumber(H.optionValue("JumpPower", 50)) or 50
            end
        end
...
H.notify("zen hubXsteal", "Ready - press Z button", "Success", 5)
